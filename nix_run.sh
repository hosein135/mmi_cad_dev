#!/usr/bin/env bash
# =============================================================================
# nix_run.sh — Auto setup-or-start for Micro Magic CAD (x86_64 Linux / WSL2)
#
...
#
# Usage:
#   ./nix_run.sh                # start CAD shell (auto setup if first time)
#   ./nix_run.sh max            # start a tool
#   ./nix_run.sh --force-setup  # re-run Nix prep even if ready
#   ./nix_run.sh --force-install  # re-extract vendor tarball
#   ./nix_run.sh --prep-only    # ensure env only, do not start CAD
#   ./nix_run.sh --clean        # drop local prefix/fonts/ready marker
#   ./nix_run.sh --help
# =============================================================================
set -euo pipefail

RED='\033[0;31m'; YELLOW='\033[1;33m'; GREEN='\033[0;32m'; CYAN='\033[0;36m'; NC='\033[0m'
info()  { echo -e "${GREEN}[nix_run]${NC}  $*"; }
warn()  { echo -e "${YELLOW}[nix_run]${NC}  $*"; }
error() { echo -e "${RED}[nix_run]${NC} $*" >&2; }
step()  { echo -e "${CYAN}[nix_run]${NC}  $*"; }
...
main() {
    if grep -q $'\r' "$0" 2>/dev/null; then
        ...
        error "  sed -i 's/\\\\r\$//' nix_run.sh run.sh && chmod +x nix_run.sh run.sh"
        exit 1
    fi
