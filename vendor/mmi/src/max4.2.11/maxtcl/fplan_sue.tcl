...
set RCSVERSION(fplan_sue.tcl) { $Revision: 1.4 $ }
...
proc UNUSED_fplan_sue_get_icon_name {id} {
  global FPLAN_SUE_INFO
...
proc fplan_read_sue {{-menu 0} {filename ""}} {
  ...
  foreach sue_id [array names FPLAN_SUE_INFO] {
    ...
    if {$type == $cellname} {
      # Stupid special case.  A schematic can contain its own icon as a comment.
      # Ignore it.
      continue
    }
  ...
  foreach thingy $sue_list {
	...
	fplan_place_ports -center_unplaced 1 $type
  ...
  db_paint prb 0 0 $x_size $y_size
...
proc _fplan_sue_ports_int {portlist cx cy sepx sepy default_layer region} {
  ...
  foreach thing $portlist {
	...
	fplan_db_pin -cell $cell setprop $bit bitloc $bitloc
      ...
      if {$loc != ""} {
	fplan_db_pin -cell $cell setprop $bit loc $loc
      } else {
	fplan_db_pin -cell $cell setprop $bit loc $region
      }
