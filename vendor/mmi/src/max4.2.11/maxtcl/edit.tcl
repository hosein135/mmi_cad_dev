...
set RCSVERSION(edit.tcl) { $Revision: 1.39 $ }
...
proc edit_push {args} -desc {
  ...
  if {[memq $flags override]} { set override 1 }

  if { $cell == "" } {
    # Edit selected cell

    if { [memq $flags paint_in_place] } {
    ...
    # There is not currently selected cell in the max frame
...
proc edit_this_cell_only {} -desc {
  ...
  set cell_info [sel_what_l cells]
...
# replaces the buggy [lay_editcell -path]

proc lay_path {} -desc {
  returns the edit cell path relative to the root for edit in place
} {
...
#  if {[db_search paint -area $x $y $x $y $layer] != ""}
  ...
  foreach list [split [sel_what cells] \n] {
    ...
    if { ![memq [lindex $list 7] expanded] } {
      # not expanded
      continue
    }
    ...
    push EDIT(stack) [list $inst_path [lay_path] \
			  ...
			  [layt_box exact] \
		          "$xbot $ybot $xtop $ytop"] 
    # Pats note: I was going to change this to cell_edit,
    # but I noticed that this code cant possibly work anyway,
    # because the cell we want is not selected.
    :edit
