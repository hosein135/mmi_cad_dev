...
set RCSVERSION(cell.tcl) { $Revision: 1.65 $ }
...
proc cell_info {filename} -desc {
  ...
  setl {name flags file} [db_cells [cell_cellname $filename]]
  if {$name == ""} {
...
proc cell_cellname {filename} -desc {
...
proc cell_in_memory {cellName} -desc {
  ...
  setl {name flags file} [db_cells [cell_cellname $cellName]]
...
proc cell_empty {cellName} -desc {
  ...
  This does not always do what you think.  It does not actually
  ...
  regardless of its contents.  Observe:
    db_cell_new foo dir/foo.max
    cell_empty
    -> returns 0
  Also, if the cell is called out as an instance, but the cell has
  never been loaded, this returns empty.  You need to do lay_interals -area
  to make sure the cell is loaded, if it exists, before calling this.
} {
...
proc cell_load {{-search} {cell ""} {dir ""} } -desc {
  ...
  If dir is not given, but cell has a directory, that directory is used.
...
# Except I was overruled, on the somewhat specious grounds that
# we want max to resemble MacDraw as much as possible.
#
# TODO:
...
proc cell_make_instance {cell {x ""} {y ""}} -desc {
  ...
  if {$y == ""} {
	...
	return
      ...
      if { [msg_catch [list db_instance $cell $x $y] cellname errmsg] ||
	  $cellname == "" } {
	# Try another spot
...
proc cell_id2cell {{-cell ""} cellid} -desc {
  ...
  foreach subcellid [split $cellid "/"] {
    if {$subcellid != "."} {
      struct max_cell c [lindex [db_instances_l -cell $cell -id $subcellid] 0]
      set cell ${c.def}
    }
  }
  return $cell
...
proc cell_load_hook {name unused set_file} -desc {
  This is called whenever max loads a file.
} -doc {
  name = cell def;
  unused = unused now.
  set_file = binary.  If 1, max will set the filename associated
  with this cell buffer to this file, which is where it would be written.
  This does not need to be used by this proc.
...
proc cellinfo_loc {cell_info} -desc {
  Return current cell location {x1 y1 x2 y2} from cell info returned by db_search or sel_what.
} {
  return [lrange $cell_info 2 5]
}

proc cellinfo_path {cell_info} -desc {
  Return path from cell info returned by db_search or sel_what.
} -doc {
  Path may be empty.  If non-empty, it already contains the trailing slash.
} {
