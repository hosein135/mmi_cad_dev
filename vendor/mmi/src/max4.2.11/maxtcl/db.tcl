...
set RCSVERSION(db.tcl) { $Revision: 1.5 $ }
...
proc dbt_long_name {name} -desc {
  ...
  global db_type_table

  #initialize type table if necessary
  if {! [info exists db_type_table] } {
    _db_gen_type_table
  }

  return [use_first db_type_table($name.longname)]
}

proc dbt_short_name {names} -desc {
    map (list of) long layer names to (list of) short names
} {
  global PAL db_type_table
  ...
  foreach name $names {
    ...
    set long [use_first db_type_table($name.longname)]
...
proc db_plane {name} -desc {
  ...
  return [use_first db_type_table($name.plane)]
}
...
proc _db_gen_type_table {} -desc {
  ...
  foreach type [split [db_types] \n] {
    ...
    foreach name $names {
      set db_type_table($name.longname) $longName
      set db_type_table($name.plane) $plane
    }
  }
}
