...
set RCSVERSION(utils.tcl) { $Revision: 1.36 $ }
...
proc round_list_scale {x scale} {
  ...
  set out ""
  foreach y $x {
    lappend out [expr round($y/$scale)*$scale]
  }
...
proc db_search_label {labelpath} -desc {
  ...
  if {$slash == -1} {
    ...
    if {0} {
      ...
      sel_cell3 $path
      lay_internals
      # It returns labels from the current cell as well as labels in the
      # cell that we lay_internaled.  So have to check the path.
      set labels [db_search_l labels -any_cell -exact $text]
      set label_path_index [struct_index max_label path]
      return [lsearch2 -index $label_path_index -value -exact $labels $path]
    } else {
...
db_search_l labels [-cell <def>] [-hier <type>] [-limit <n>] [-layers <commalist>]
      [-area <xbot> <ybot> <xtop> <ytop>] [-kind <kind>] [-exact] [-glop] [labelname]
...
db_search_l cells [-no_fets] [-no_vias] ...
  ...
  switch $action {
    "labels" {
      # This is slower than shit, but mha says he'll quit if this
      # is written in C, and mark says "This is a prototype, who cares?"
	...
	set cell_def_index [struct_index max_cell "def"]

	set new_result ""
	foreach cell_info $result {
	  set def [lindex $cell_info $cell_def_index]
...
proc util_load_pkg {pkg_file {pkg_name ""}} -desc {
  ...
  # New way: Just look on PATH.
  set filepath [util_find_path $pkg_file]
  if {$filepath == ""} {
    error "Could not find file $pkg_file on PATH"
  } else {
    msg "Loading $pkg_file from $filepath\n"
    eval load $filepath $pkg_name
