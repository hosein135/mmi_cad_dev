...
set RCSVERSION(cursor.tcl) { $Revision: 1.18 $ }
...
Notify cursor module that max is busy (bool=1) or not (bool=0) 
  ...
  global CURSOR
  if { $CURSOR(wait) == "" } {
    set CURSOR(busy,main) $bool
  } else {
    set CURSOR(busy,$CURSOR(wait)) $bool
  }
  cursor_update
}
...
proc cursor_mode {submode {redisplay_disable 0}} -desc {
  ...
  that the redisplay will not be a problem.
    ...
    if { $submode == "0" } {
	set CURSOR(mode) 0
	# Added 3/6/01: Make busy cursor is reset when re-entering main mode from submode
	set CURSOR(busy,main) 0
    } elseif { $submode == "1" } {
...
if {0} {
    # No no no.  Do not redefine the TCL language to produce hidden
    # side-effects.  Instead, use cursor_wait explicitly.

    # wrap tkwait to call cursor_wait on "tkwait window ..."
    if {[info commands _cursor_tkwait_orig] == ""} {
	rename tkwait _cursor_tkwait_orig
    }

    proc tkwait {type name} -desc {
	tkwait (with wrapper to call cursor_wait on "tkwait window name") 
    } {
	if {$type == "window"} {
	    cursor_wait $name 1
	    _cursor_tkwait_orig $type $name
	    cursor_wait $name 0
	} else {
	    _cursor_tkwait_orig $type $name
	}
