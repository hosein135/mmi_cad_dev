...
private	char	*filename;		/* current input file */
	...
	if( in == stdin )
	  {
	    ...
	    (void) fprintf( stdout, "%s> %s", filename, line );
...
private void prtime( col )
  int  col;
  {
    if( col != 0 )
	lprintf( stdout, "\n" );
    lprintf( stdout, "time = %.1fns", d2ns( cur_delta ) );
    if( npending )
	...
	if( strcmp( b->name, targv[1] ) == 0 )
	  {
	    if( undefseq( (nptr) b, &slist, &maxsequence ) or
	      undefseq( (nptr) b, &clock, &maxclock ) )
	      {
...
private int setvector()
  ...
  got_it :
    ...
    for( i = 0; i < b->nbits; i++ )
      {
	if( (val[i] = potchars[ ch2pot( val[i] ) ]) == '.' )
	    return( 0 );
      }
    for( i = 0; i < b->nbits; i++ )
...
private int doAssert()
  {
	...
	name = pnode( f.node );
	UnAlias( f.node );
	CompareVector( &f.node, name, 1, mask, value );
      }
    else if( f.vec != NULL )
	CompareVector( f.vec->nodes, f.vec->name, f.vec->nbits, mask, value );
...
public void disp_watch_vec( which )
  long  which;
  {
    ...
    (void) sprintf( temp, " @ %.1fns ", d2ns( cur_delta ) );
...
private void cpath( n, level )
  ...
  int            level;
  {
	 ...
	 */
      ...
      {
	lprintf( stdout, "  %s -> %c @ %.1fns , node was an input\n",
	...
	n->nflags &= ~VISITED;
	if( delta_t < 0 )
	    lprintf( stdout, "  %s -> %c @ %.1fns   (??)\n", pnode( n ),
		vchars[ n->npot ], d2ns( n->c.time ) );
	else
	    lprintf( stdout, "  %s -> %c @ %.1fns   (%.1fns)\n", pnode( n ),
...
#   define	SIZE_ST		( sizeof( st ) - 1 )
    ...
    lprintf( stdout,
      "Histogram of circuit activity: %.1f -> %.1fns (bucket size = %.1f)\n",
      d2ns( ac.begin ), d2ns( ac.end ), d2ns( ac.size ) );

    for( i = 0; i < NBUCKETS; i += 1 )
	lprintf( stdout, " %10.1f -%10.1f%6d  %s\n",
...
private int dochanges()
  {
    ...
    column = 0;
    lprintf(stdout,"Nodes with last transition in interval %.1f -> %.1fns:\n",
...
private int print_list( n, l, eolist )
  ...
  int    n;
  {
    ...
    for( eolist = eolist->flink; l != eolist and n != 0; l = l->flink, n-- )
      {
	lprintf( stdout, "Node %s -> %c @ %.1fns (%.1fns)\n",
