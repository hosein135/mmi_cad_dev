     ...
     va_list args)
{
	...
	Tcl_DStringAppend(stack->mcs_textDS, buf, -1);
      }
    }
 ...
 * msgPushCxtStack --
 *
 * Start new message diversion.
 *
 * Used by MsgBeginCmd and msgCatchTclCmd below
 *
 * ----------------------------------------------------------------------------
 */
static void msgPushCxtStack(MsgCxtStack **stack)
{
  MsgCxtStack *new;

  MALLOC(MsgCxtStack *, new, sizeof(MsgCxtStack));
  new->mcs_textDS = 0;
  new->mcs_next = *stack;
  *stack = new;
}
 ...
 * ----------------------------------------------------------------------------
 */
static Tcl_DString *msgPopCxtStack(MsgCxtStack **stack)
{
  Tcl_DString *textDS;
  MsgCxtStack *top = *stack;

  ASSERT(top,"msgPopCxtStack");
...
{
  Tcl_DString *errorDS = msgPopCxtStack(&msgErrorStack);
...
msgCatchTclCmd(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[])
{
   int completion;
    ...
    if(argc>=4 && *argv[3]!='\0') msgPushCxtStack(&msgInfoStack);

    /* divert warn messages */
    if(argc>=5 && *argv[4]!='\0') msgPushCxtStack(&msgWarnStack);
    ...
    if(argc>=4 && *argv[3]!='\0') 
    {
      Tcl_DString *msgDS = msgPopCxtStack(&msgInfoStack);
    ...
    if(argc>=5 && *argv[4]!='\0') 
    {
      Tcl_DString *msgDS = msgPopCxtStack(&msgWarnStack);
