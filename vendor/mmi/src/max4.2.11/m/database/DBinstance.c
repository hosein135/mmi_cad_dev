 ...
 *   2. linking it into the database with DBInstanceAdd() (in DBinstance.c) 
 *
 *     ********************************************************************* 
...
DBInstancePlace(CellUse *use, 
                        	/* new celluse to add to subcell tile plane */
		CellDef *targetcell)
                                /* parent cell's definition */
{
    ASSERT(use != (CellUse *) NULL, "DBInstancePlace");
    ASSERT(!targetcell->cd_cellPlaneLock,"DBInstancePlace");

    use->cu_parent = targetcell;
    ...
    */
    ...
    BPAdd(targetcell->cd_cellPlane, use);
...
DBInstanceUnplace (CellUse *use)
{
    ...
    ASSERT(!use->cu_parent->cd_cellPlaneLock,"DBInstanceUnplace");
	       ...
	       int flags)
{
    ...
    */
     ...
     */
    if(!(flags & DBIA_DUP_OK) && dbInstanceFindDup(use, parentDef))
    {
    ...
    /* link use to parent */
    use->cu_parent = parentDef;

    /* handle versions */
    DBChangeNewInstance(use);
    ...
    if(!use->cu_id)
    {
      ...
      /* get last part of def name */
      lastName = strrchr(def->cd_name, '/');
      if (lastName == NULL) 
      {
	lastName = def->cd_name;
      }
      else
      {
	lastName++;
      ...
      do
      {
	sprintf(useId, "%s_%d", lastName, ++kid->ck_idLastSuffix);
      }
    ...
    /* place instance (in celltile plane) */
    DBInstancePlace(use, parentDef);
 ...
 */
void
DBInstanceUnlink(CellUse *use, CellDef *parentDef)
{
    HashEntry *he;
    CellKid *kid = IHashLookUp(parentDef->cd_kidHash, &(use->cu_def)); 

    /*
    fprintf(stderr, "DEBUG DBInstanceUnlink(): unlinking '%s' from '%s'\n",
	    use->cu_id, parentDef->cd_name);
    */

    ASSERT(kid!=NULL,"DBInstanceUnlink");

    /* unlink use from kid structure */
    {
      CellUse **pp;
      for(pp=&(kid->ck_sibUses); (*pp)!=use; pp=&((*pp)->cu_nextSib));
      (*pp) = use->cu_nextSib;
    }
    ...
    if(!kid->ck_sibUses)
    {
      ...
      /*** DEBUG
      fprintf(stderr,
	      "DEBUG DBInstanceUnlink, deleteing %s from %s->cd_kidHash\n",
	      use->cu_def->cd_name, 
	      parentDef->cd_name);
      ***/
...
DBInstanceRename(CellUse *cellUse, char *newName)
{
    ...
    if (DBInstanceFindByName(newName, cellUse->cu_parent))
	return (FALSE);

    /* Old id (may be NULL) */
    if (cellUse->cu_id) dbUseIdDelete(cellUse, cellUse->cu_parent);
    DBUndoCellUse(cellUse, UNDO_CELL_CLRID);

    /* New id */
    (void) StrDup(&cellUse->cu_id, newName);
    dbUseIdSet(cellUse, cellUse->cu_parent);
                              ...
                              /* File to output to */
{
    ...
    else
    {
      ...
      sprintf(cstring, "vBBOX %d %d\n",
	      def->cd_vBBOX.vs_time,def->cd_vBBOX.vs_rev);
      FPRINTR(f,cstring);
    }
            		...
            		/* Input file */
{
 ...
 nextdef:
   ...
   * make a skeleton def that's not marked CDAVAILABLE. 
   */
  ...
  while(line[0]!='}')
  {
    ...
    /* create celluse */
    subCellUse = DBCellNewUse(subCellDef, id[0] ? id : (char *) NULL);
    DBMakeArray(subCellUse, &GeoIdentityTransform,
			    xlo, ylo, xhi, yhi, xsep, ysep);
    subCellUse->cu_vMAIN = vMAIN;
    subCellUse->cu_vDRC = vDRC;
    subCellUse->cu_vBBOX = vBBOX;
