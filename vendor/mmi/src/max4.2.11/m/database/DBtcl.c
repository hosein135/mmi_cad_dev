...
cell buffers.
    ...
    <cellname> <flaglist> <filename> {<changes> <changesPending>}
	  ...
	  ClientData clientdata)
{
    ...
    */
    ...
    if(cellsUser && def->cd_flags&CDINTERNAL) return 0;
...
#define dbtclDOFLAG(f,p) if(def->cd_flags & f) Tcl_AppendResult(interp,p, NULL);
    dbtclDOFLAG(CDAVAILABLE," available");
    dbtclDOFLAG(CDMODIFIED," modified");
    dbtclDOFLAG(CD_GENERATED," generated");
    dbtclDOFLAG(CDNOTFOUND," notFound");
    dbtclDOFLAG(CDINTERNAL," internal");
    dbtclDOFLAG(CDREADONLY," readOnly");
    dbtclDOFLAG(CD_BBOX_BAD," bBoxBad");
    dbtclDOFLAG(CD_DRC_PENDING," drcPending");
    dbtclDOFLAG(CD_DRC_ALL," drcAll");
...
#undef dbtclDOFLAG
    ...
    /* DEBUG changes and changesPending flags */
    if(!cellsUser)
    {
      char buf[1000];
      sprintf(buf,"\t{ %d %d }", def->cd_changes, def->cd_changesPending);
...
dbTclCmdCellDeleteFunc1(CellDef *def, ClientData cd)
{
  ...
  if(!(def->cd_flags&CDAVAILABLE)) return 0;
...
dbTclCmdCellNew(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
{
    ...
    if (internal) def->cd_flags |= CDINTERNAL;
    ...
    if(def->cd_uses) DBChangedArea(def, NULL, NULL, 0);
...
dbTclCmdCellRename(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
{
    ...
    if (CmdIllegalChars(cellName, "[],", "Cell name"))
    {
      CMD_RETURN(interp);
    }
    if (cellName=="")
		    ...
		    int argc, char **argv)
{
    ...
    if(def->cd_uses)
    {
      DBChangedArea(def, NULL, NULL, DBCF_DEFREAD|DBCF_INSTANCE);
    }
    else
    {
	       ...
	       int argc, char **argv)
{
    ...
    DBChangedArea(def, NULL, NULL, DBCF_INSTANCE);
    CMD_RETURN(interp);
...
#define db_label_DESC "add a label"
...
dbTclCmdLabel(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
{
    ...
    if(text[0] == '\0')
    {
	MsgErrorF("Can't have null label name.\n");
	goto usage;
    }
    if (CmdIllegalChars(text, " /", "Label name")) goto usage;
    argv++; argc--;
    ...
    /* doit */
    (void) DBLabelAdd(def, &r, pos, text, type, kind);

    /* change notification */
    if(notify)
    {
      DBChangedArea(def, &r, NULL, DBCF_LABEL);          
    }
...
dbTclCmdPaint(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
{
    ...
    if(!(def->cd_flags&CDINTERNAL) && !(def->cd_flags&CD_GENERATED))
    {
      SelectClear();
    }
...
int dbTclCmdInstance(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
{
    ...
    if(!subCellDef)
    {
      ...
      /* generated? */
      if(*subCellDefName == '#') DBCellRead(subCellDef, NULL, FALSE);
    }

    /* create celluse */
    subCellUse = DBCellNewUse(subCellDef, id);
    ...
    /* create instance */
    /* ( on failure DBInstanceAdd() calls DBCellDeleteUse() )*/
    {
...
#define db_instance_delete_DESC "create a cell instance"
			   ...
			   char **argv)
{
    ...
    /* delete instance */
    DBInstanceUnlink(cu, def);
    DBInstanceUnplace(cu);
    DBCellDeleteUse(cu);
		      ...
		      char **argv)
{
	...
	for(use=kid->ck_sibUses; use; use=use->cu_nextSib)
        {
	  numUses++;
	  memUses += UtlsStatMallocMem(sizeof(CellKid));
		   ...
		   char **argv)
{
    ...
    /* label loc hash */
    {
      ...
      mem = IHashStats2(def->cd_labelLocHash, &entries, &buckets);
...
dbTclCmdMHA(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
{
  ...
  initial=UtlsStatHeapSize();
  MALLOC(char *, c, 100*1000*1000);
  final=UtlsStatHeapSize();

  fprintf(stderr,"heap size:  initial=%d  final=%d  delta=%d \n",
