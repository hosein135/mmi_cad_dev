                                ...
                                /* If set, copy active group only */
{
		...
		|| TTMaskHasType(mask, L_LABEL)))
	{
	    ...
	    (void) DBLabelsEraseByContent(def, &labTargetRect, targetPos,
		    -1, lab->lab_text);
	    (void) DBLabelAdd(def, &labTargetRect, targetPos,
		    ...
		    Transform *trans)  /* if not null, transform while copying. */
{
  ...
  DBCellInitTempUse(srcDef,&srcUse);
  DBCellInitTempUse(destDef,&destUse);
