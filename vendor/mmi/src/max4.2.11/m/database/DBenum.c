                     		...
                     		/* Client data for above function */
{
    ...
    for (parentUse = baseDef->cd_uses;  parentUse != NULL;
	...
	else
	{
		...
		for (y = parentUse->cu_ylo; y <= parentUse->cu_yhi; y++)
		{
		    ...
		    if (DBEnumRoots(parentUse->cu_parent, &baseToParent,
			func, cdarg)) return 1;
		}
	}
    }
...
int
DBEnumArrayElements(CellUse *use, Rect *searchArea, int (*func) (/* ??? */), ClientData cdarg)
