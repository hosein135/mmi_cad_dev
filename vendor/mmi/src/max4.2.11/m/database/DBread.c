 ...
 * version 3 - save gcell layout             (N/Y/I)
 */  
int DBReadMaxFormat;
...
dbReadOpen(CellDef *cellDef, 
                     	/* Def being read */
	   char *name, 
               		/* Name if specified, or NULL 
			 *
			 * OBSOLETE name always NULL!
			 */
	   int setFileName)
                     	/* If TRUE then cellDef->cd_file should be updated
			 ...
			 */
{
    ...
    if (name != (char *) NULL)
    {
	f = PaOpen(name, "r", DBSuffix, MnPathCell, NULL, &filename);
    }
    ...
    else
    {
      ...
      /* cell name may contain ".", so can't rely on PaOpen() to tack
       * on extension
       */
      sprintf(nameBuf, "%s%s", cellDef->cd_name, DBSuffix);
      f = PaOpen(nameBuf, "r", NULL, MnPathCell, NULL, &filename);
    }
	...
	if (name != (char *) NULL)
	    MsgErrorF("File %s%s couldn't be found\n", name, DBSuffix);
	else if (cellDef->cd_file != (char *) NULL)
    ...
    {
      cellDef->cd_flags |= CDREADONLY;

      if(DBVerbose)
	MsgInfoF("Warning -- cell %s not writable\n", cellDef->cd_name);
    }
...
char *
DBGetTech(char *cellName)
                   			/* Name of cell whose technology
					 * is desired.
					 */
{
    FILE *f;
    ...
    f = PaOpen(cellName, "r", DBSuffix, MnPathCell, NULL, (char **) NULL);
            		...
            		/* Input file */
{
  ...
  if (sscanf(line, "vBBOX %d %d", 
	     &cellDef->cd_vBBOX.vs_time, 
	     &cellDef->cd_vBBOX.vs_rev) != 2)
  {
 ...
 * dbReadSections --
 *
 * called by DBCellRead for each DEF in file, to read data SECTIONS
...
/* change notification for DBCellReadSections() */
static void dbReadChangeNotify(CellDef *cellDef)
{
  ...
  if(cellDef->cd_uses)
  {
    DBChangedArea(cellDef, NULL, NULL, DBCF_DEFREAD|DBCF_INSTANCE);
  }
  else
  {
...
skipToEndOfSection:
    ...
    { 
      ...
      cellDef->cd_vBBOX = stamp;
      cellDef->cd_vDISPLAY = stamp;
    }
 ...
 * and set CDAVAILABLE
 * 
 * Otherwise, call tcl procedure cell_load_hook, and then read in the paint 
 * for a cell from its associated disk file.
			 ...
			 */
{
    ...
    /* TODO, remove name arg, or at least ASSERT to NULL */
    if(name!=NULL) 
    {
      fprintf(stderr,
	      "DEBUG, DBCellRead() called with non-null name, "
	      "'%s' Tell mha!!!\n",
	      name);
    }
    ...
    if (cellDef->cd_flags & CD_GENERATED)
    {
      ...
      loadResult = MnTclEvalBg(script,
			       "gcell_load call from DBCellRead");
     ...
     */
    {
      ...
      /* TODO:  note "name" always NULL - rm? */
      sprintf(cmd,"cell_load_hook {%s} {%s} %s\n",
	      cellDef->cd_name, 
	      name ? name : "",
	      setFileName ? "1" : "0");

      loadResult = MnTclEvalBg(cmd,
			       "cell_load_hook call from DBCellRead");
    ...
    if(DBReadMaxFormat<0 || DBReadMaxFormat>3)
    {
      MsgErrorF("Unsupported Max Format Version '%d', "
		"(format versions 0 through 3 are read by this Max)\n",
		DBReadMaxFormat); 
	goto badfile;
    } 
	...
	MsgErrorF("Expected timestamp but got: %s", line);
      ...
      cellDef->cd_vMAIN.vs_time = cellStamp;
      cellDef->cd_vMAIN.vs_rev = 0;
      cellDef->cd_vDRC.vs_time = cellStamp;
      cellDef->cd_vDRC.vs_rev = 0;
      cellDef->cd_vBBOX.vs_time = cellStamp;
      cellDef->cd_vBBOX.vs_rev = 0;
    }
...
DBCellReadTree(CellDef *def) 
{
  ...
  {
    DBCellRead(def, (char *) NULL, TRUE);
  }

  /* now recursively read-in descendents */
  for (kid=def->cd_kids; kid; kid=kid->ck_next) DBCellReadTree(kid->ck_def);
}
