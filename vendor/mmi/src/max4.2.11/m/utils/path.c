...
int
PaConvertTilde(char **psource, char **pdest, int size)
                   		...
                   		/* pathname to tilde expand */
{
  ...
  if(PaConvertTilde(&src, &dest, 1000) == -1) return NULL;
...
PaOpen(char *file, 
       ...
       char *path, 
				 ...
				 */
       char *library, 
                  		/* A 2nd path containing library names.  Can be
...
char *
PaSubsWD(char *path, char *newWD)
           			/* Path in which to substitute. */
            			/* New working directory to be used.  Must
				 * end in a slash.
				 */
...
int PaEnum(char *path, 
               		/* Search path */
	   char *file, 
               		/* Each element of the search path is prepended to
