 ...
 *  without need for parallel hash entry strucs.  
 *
 *  pointers involving users strucs are cast to (void *) 
...
/* Microsoft compiler barfs on arithmetic on (void*) pointers,
 * which I consider a bug.  This works around it.
 */
#if _MSC_VER
#define DEREF(ptr,offset) ( (void*) ((char*)(ptr)+(offset)) )
#else
		      ...
		      )
{
  ...
  MALLOC(IHashTable *, table, sizeof(IHashTable));
  CALLOC(void**, table->iht_table, sizeof(void *)*nBuckets);
...
{
  FREE((void *) table->iht_table);  /* free buckets */
  FREE((void *) table);                      
}
...
void IHashClear(IHashTable *table)
{
  ...
  FREE((void *) table->iht_table);  
  CALLOC(void**, table->iht_table, sizeof(void *)*table->iht_nBucketsInit);
...
static void iHashResize(IHashTable *table)
{
  ...
  /* alloc a new table */
  CALLOC(void**, table->iht_table, sizeof(void *)*newSize);
