...
#ifndef lint
static char rcsid[] = "$Header: dqueue.c,v 6.0 90/08/28 19:00:33 mayo Exp $";
#endif  not lint
...
void
DQInit(DQueue *q, int capacity)
{
    if (capacity < 1) capacity = 1;
    MALLOC(ClientData *, q->dq_data, (capacity+1) * sizeof (ClientData));
    q->dq_size = 0;
...
void
DQPushFront(DQueue *q, ClientData elem)
{
...
void
DQPushRear(DQueue *q, ClientData elem)
{
    if (q->dq_size == q->dq_maxSize)  DQChangeSize(q, 2 * q->dq_maxSize);
...
void
DQChangeSize(DQueue *q, int newSize)
{
    DQueue newq;
