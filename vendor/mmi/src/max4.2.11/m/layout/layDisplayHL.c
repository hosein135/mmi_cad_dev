                      		...
                      		/* Window in which to redisplay. */
{
    ...
    /* draw bounding box */
    GeoTransRect(&scx->scx_trans, DBBBoxCellDef(scx->scx_use->cu_def), &rect);
				 ...
				 */
{
    ...
    {
      /*** ZOOMED OUT ***/
      layDrawStyle(STYLE_SELECTION_SOLID);
	...
	!(window->lay_labelMarkSize <= 0 &&
	  window->lay_labelSize < 0 &&
	  DBLabelMaxDim * window->lay_pixelsPerDB < 1))
    {
      Label *label;
...
/* linked to tcl var LAY_FLYLINE_TEXT_SIZE */
/* -1=off, 0=small, 1=medium, 2=large, 3=extra large. */
int layFlyLineTextSize = GR_TEXT_MEDIUM;

/* display a single flyline
 * called by layDisplayHLFlylinesFunc()) 
 */
static void layDisplayFlyline(SearchContext *scx, FlyLine *fl)
{
  PointFloat p1, p2;
  PointFloat p1W, p2W;
  double length;
...
#define y2 (p2W.pf_y)
  ...
  if(fl->fl_width !=1 || (fl->fl_text && layFlyLineTextSize>=0))
  {
     ...
     */
    {
      ...
      if(fl->fl_text && layFlyLineTextSize>=0) delta += layFlyLineTic;
  ...
  if(fl->fl_text && layFlyLineTextSize>=0)
  {
    /* make center tic */
...
layDisplayHLFlylinesFunc(SearchContext *scx, ClientData arg)
{
  ...
  {  
    if (!DBCellRead(def, (char *) NULL, TRUE)) return 1;
  }
				 ...
				 */
{
    ...
    if (!(w->lay_flags & Lay_SEEFLYLINES)) return;
				 ...
				 */
{
    ...
    for (ta = w->lay_textAnnotations; ta; ta = ta->ta_next)
    {
      PointFloat pixelLoc;
