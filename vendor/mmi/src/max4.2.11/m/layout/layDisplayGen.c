...
static bool layNeedStyle;	/* Do we need to set the display style? */
 ...
 * children, rather than an area search.  This is more efficient, and
 * groups together instances of same def.
 *
                     	...
                     	/* Argument to pass to function */
{
  ...
  for(kid = def->cd_kids; kid; kid=kid->ck_next)
  {
	...
	continue;
      ...
      for (newScx.scx_y = use->cu_ylo; 
	   newScx.scx_y <= use->cu_yhi; 
	     newScx.scx_y++)
      {
	for (newScx.scx_x = use->cu_xlo; 
	     newScx.scx_x <= use->cu_xhi; 
	     newScx.scx_x++)
	{
	  int result;
...
*/
  ...
  */
  ...
  if(GEO_SURROUND(&layDisplayAreaDB,&rootBBoxDB))
  {
    /* The cell is entirely within the redisplay area, so
     * use kid strucs to enumerate subcells, 
     * (rather than area search).
     *
     * This is more efficient, and more importantly it groups
...
{
  /* DEBUG
    fprintf(stderr,"DEBUG, layDisplayUnexpandedSubcell %s\n",
	    scx->scx_use->cu_id);
  */
...
layDisplayUnexpandedSubcellsFunc(SearchContext *scx, ClientData cd)
{
  ...
  /* special case subcells only a few pixels across */
  if(loc.rf_xtop - loc.rf_xbot <= laySinglePixelThreshold && 
     loc.rf_ytop - loc.rf_ybot <= laySinglePixelThreshold)
  {
		  ...
		  ClientData notUsed)
{
    ...
    */
    ...
    scx.scx_area = rootArea;
    layDisplayUnexpandedSubcells(&scx);
