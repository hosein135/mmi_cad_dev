...
#define lay_text_DOC "
Usage:  lay_text [-tag tag] [-size n] [-align pos] x y text
        or 
        lay_text [-tag tag] [-delete n | -clear]
	   ...
	   char **argv)
{
    ...
    if(delete)
    {
      ...
      layAnnotateTextDelete(layCurrent,deleteNum, TRUE /* notify */);
      CMD_RETURN(interp);
    }
    ...
    if (argc==0)
    {
      ...
      for(ta = layCurrent->lay_textAnnotations;
	  ta;
	  ta = ta->ta_next)
      {
	       ...
	       Tcl_Interp *interp, int argc, char **argv)
{
    ...
    /* free current values */
    if(def->cd_showName) FREE(def->cd_showName);
    if(def->cd_showInst) FREE(def->cd_showInst);
    ...
    DBChangedArea(def, NULL, &DBAllButSpaceBits, DBCF_DISPLAY);
