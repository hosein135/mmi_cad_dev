...
layAnnotateTextAdd(Layout *w,
		   PointFloat location,
		   int pos, /* alignment */
		   int fontSize, /* 0 (small) through 3 (xlarge) */
		   char *text,
		   char *tag)
{
  TextAnnotation *new;

  ASSERT(0 <= fontSize && fontSize <= GR_MAX_FONT_SIZE, 
	 "layAnnotateTextAdd");
  ...
  /* link */
  new->ta_next = w->lay_textAnnotations;
  w->lay_textAnnotations = new;
...
/*
 * ----------------------------------------------------------------------------
 * layAnnotateTextDelete -
 *
 *   delete n+1th text annotation
 *
 * ----------------------------------------------------------------------------
 */

void
layAnnotateTextDelete(Layout *w, int n, bool notify)
{
  TextAnnotation **nextpp;

  nextpp = &w->lay_textAnnotations;

  for(; n>0 && *nextpp; n--) nextpp = &(*nextpp)->ta_next;

  if(*nextpp)
  {
    TextAnnotation *ta = *nextpp;
...
{
  TextAnnotation *ta;
  TextAnnotation **nextpp = &w->lay_textAnnotations;
