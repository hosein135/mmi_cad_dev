...
void ntlPrintItem(FILE * f, char * str, bool inComment)
{
...
void
ntlOutputSbcktDecl(CellDef * cd, FILE * f)
{
    Port * port;
 ...
 */
int
ntlOutputInstancesFn(CellUse * use, FILE * f)
{
    int i, x, y, xlen, xoff, yoff;
    int element;
...
if(ntlVerbose)
    MsgInfoF("OutputInstances: use %s of celldef %s, connector = %x, number of def ports = %d, number of connections = %x\n", 
...
if(ntlVerbose)
    MsgInfoF("output port %d\n", i);
	     ...
	     */
	    if((port->po_flags & NTL_PORT_FT_ELEM )
	    || ((labType == LAB_GLOBAL) && ntlUseGlobals))
		continue;
...
void
ntlOutputGlobals(Stack * stack, FILE * f)
{
 ...
 */
void 
ntlOutputNodes(CellDef *cd, FILE * f)
{
    Port * port;
    NodeRecord * node;
...
/* dont print ports, they are declared in the cell definition
    ...
    {
	(void) fprintf(f, "*        %s", ntlNodeName(port->po_node));
...
ntlNodeName(NodeRecord * node)
{
    ...
    static char name[80];	/* max name length !! */
