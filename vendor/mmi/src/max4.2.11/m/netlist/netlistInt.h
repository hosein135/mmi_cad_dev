 ...
 * region search.
 */
extern ClientData ntlUnInit;

/* needed in NtlLabel.c, left over from extract code */
/* extern  ClientData ntlUnInit ; */
...
extern ClientData ntlDebugID;   /* Identifier returned by the debug module */
...
extern int ntlTile2ElementFunc( CellUse * use, 
				Transform * trans, 
				int x, 
				int y, 
				HierArg * ha
...
extern PortConnector * ntlAllocPortCons (
		    PortConnector * curConns,
		    int numConns
		    );

TileTypeBitMask *ntlConnectTable;
Transform *ntlTransUse1toParent;
Transform *ntlTransUse2toParent;
...
extern bool ntlCommentTopDef;
extern bool ntlOutputAsComment;
extern bool ntlVerbose;

extern int ntlNumWarnings;
extern char * ntlNodeName();

extern NRegion *ntlTransFirst(Tile *tile, NFindRegion *arg);
extern int ntlTransEach(Tile *tile, int pNum, NFindRegion *arg);
 ...
 */
#define ntlConnectsTo(r, s, tbl)        ( TTMaskHasType(&(tbl)[(r)], (s)) )
#define ntlTileConn(tp1, tp2, tbl) \
        ntlConnectsTo(DBgetTileType(tp1), DBgetTileType(tp2), (tbl))

/* -------------------Debug -------------------------------------------- */


extern int ntlDebAreaEnum;
extern int ntlDebHierCap;
extern int ntlDebHierAreaCap;
extern int ntlDebLabel;
extern int ntlDebNeighbor;
extern int ntlDebNoFeedback;
extern int ntlDebNoSubcell;
extern int ntlDebLength;
extern int ntlDebPerim;
extern int ntlDebResist;
extern int ntlDebVisOnly;
