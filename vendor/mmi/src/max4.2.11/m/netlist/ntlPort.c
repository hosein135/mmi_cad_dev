...
int ntEraseImplPorts(CellDef * cd)
{
}

/* ignores effect of uniq suffix on names */
int ntl2ndNodeNameBetter(NodeRecord * node0, NodeRecord * node1)
{
char * str0, * str1;
 ...
 */
void ntlMergeNodes(NodeRecord * node0, NodeRecord * node1)
{
    NodeRecord ** node_ptr;

    if(ntlVerbose)
    {
	MsgInfoF("ntlMergeNodes: node0 = %s\n", ntlNodeName(node0));
	MsgInfoF("ntlMergeNodes: node1 = %s\n", ntlNodeName(node1));
    }

    if((node0->nrec_alias == NULL) && (node1->nrec_alias == NULL))
    {
	/* allocate storage for common alias */
	MALLOC(NodeRecord **, node_ptr, sizeof (NodeRecord *));

	/* set alias field of both nodes to point to new pointer */
	node0->nrec_alias = node1->nrec_alias = node_ptr;

	/* store pointer to winning node in common address */
	if(ntl2ndNodeNameBetter(node0, node1))
	{
	    * node_ptr = node1 ;
/* set flag in  node1 of owning alias storage*/
	}
	else
	{
	    * node_ptr = node0 ;
/* set flag in  node0 of owning alias storage*/
	}
    }
    else if(node0->nrec_alias == NULL)	/* Node 1 will be used */
	node0->nrec_alias = node1->nrec_alias;

    else if(node1->nrec_alias == NULL)  /* Node 0 will be used */
	node1->nrec_alias = node0->nrec_alias;

    else /* if((node0->nrec_alias != NULL) && (node1->nrec_alias != NULL)) */
    /* both non-NULL, only one will survive, the other is freed */
    {
	if(ntl2ndNodeNameBetter(*node0->nrec_alias, *node1->nrec_alias))
	{ 	/* node 1 winner */
MsgInfoF("FREEing node0 = %x\n", node0->nrec_alias);
	    FREE ((NodeRecord **) node0->nrec_alias);
/* clear flag in node0 of owning alias */
	    node0->nrec_alias = node1->nrec_alias;
	}
	else	/* node 0 winner */
	{
MsgInfoF("FREEing node1 = %x\n", node0->nrec_alias);
	    FREE ((NodeRecord **) node1->nrec_alias);
/* clear flag in node1 of owning alias */
	    node1->nrec_alias = node0->nrec_alias;
	}
    }
}
...
int ntlMergeDefNodesFn(CellUse * use, CellDef * def)
{
	     ...
	     port1 = port1->po_next)
	{
		...
		if (  (port2->po_portnum >= connector->pc_numAlloced)
		  ...
		  || (node1 == node2))	// nodes are the same
		    continue;
 ...
 * 1) the head of a linked listlist of all of the ports 
 ...
 */
void ntlMarkFTPorts(CellDef * def)
{
    NodeRecord * node1, * node2;
