...
outputCalma:
    ASSERT(namep,"CmdCalma");
    f = PaOpen(namep, "w", gdsSuffix, ".", (char *) NULL, (char **) NULL);
    if (f == (FILE *) NULL)
    {
	MsgErrorF("Cannot open %s to write Calma stream output\n", 
		PaExtendedName(namep, gdsSuffix));
	return;
    }
...
void CmdCheckPoint(Layout *w, TxCommand *cmd)
{
    ...
    /* Check that a legal filename was given */
    if (CmdIllegalChars(cmd->tx_argv[1], "[],", "Cell name")) return;

    /* save the cell but leave cd_flags (e.g. MODIFIED) unchanged  */
    {
      int flags;
      CellDef *cd = EditCellUse->cu_def;

      DBUpdate(cd); /* update to "stablilize" flags */
      flags = cd->cd_flags;
      cmdSaveCell(cd, cmd->tx_argv[1], FALSE);
      cd->cd_flags = flags;
    }
}
