...
SelectInit()
{
    ...
    DBCellUseSetTrans(SelectUse, &GeoIdentityTransform);
    ...
    DBCellUseSetTrans(Select2Use, &GeoIdentityTransform);
...
SelectClear(void)
{
	...
	{
	    DBInstanceUnlink(selDeleteUses[i], SelectDef);
	    DBInstanceUnplace(selDeleteUses[i]);
				    ...
				    */
{
    ...
    /* Notifiy redisplay and undo */
    {
       ...
       LayChangedSelection(SelectRootDef, &modified, TRUE);
	     ...
	     int less)
{
...

    {
      ...
      LayChangedSelection(SelectRootDef, sel2BBox, TRUE);
				 ...
				 */
{
	...
	selUndoBracket(TRUE, (CellDef *) NULL, (Rect *) NULL);
	DBInstanceUnlink(selectLastUse, SelectDef);
	DBInstanceUnplace(selectLastUse);
    ...
    DBCellUseSetArray(use, newUse);
    DBCellUseSetTrans(newUse, trans);
					 ...
					 */
{
    ...
    while(editInstances)
    {
      ...
      selUse = DBCellNewUse(editUse->cu_def, editUse->cu_id);
      selUse->cu_expandMask = editUse->cu_expandMask;
      DBCellUseSetArray(editUse, selUse);
      GEOTRANSTRANS(&editUse->cu_transform,&EditToRootTransform,&trans);
      DBCellUseSetTrans(selUse, &trans);
