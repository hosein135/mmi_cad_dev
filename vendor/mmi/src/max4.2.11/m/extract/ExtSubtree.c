...
#ifndef lint
static char rcsid[] = "$Header: ExtSubtree.c,v 1.3 90/12/12 15:20:25 stark Exp $";
#endif  not lint
...
extern int extHierYankFunc(CellUse *use, Transform *trans, int x, int y, HierYank *hy);
extern LabRegion *extSubtreeHardNode(Tile *tp, ExtTree *et, HierExtractArg *ha);
...
char *extSubtreeTileToNode(register Tile *tp, ExtTree *et, HierExtractArg *ha, int doHard);
int extSubtreeFunc(SearchContext *scx, HierExtractArg *ha);
int extConnFindFunc(Tile *tp, LabRegion **preg);
int extSubtreeHardUseFunc(CellUse *use, Transform *trans, int x, int y, HardWay *arg);
int extHardProc(SearchContext *scx, HardWay *arg);
...
extSubtree(CellUse *parentUse, FILE *f)
{
...
extSubtreeInteraction(HierExtractArg *ha)
                       	/* Context for hierarchical extraction */
{
     ...
     * to NULL.
     */
    (void) DBSrChildren(&scx, extSubtreeFunc, (ClientData) ha);
...
extSubtreeAdjustInit(HierExtractArg *ha)
{
...
extSubtreeOutputCoupling(HierExtractArg *ha)
{
    int cap;
...
int
extSubtreeFunc(SearchContext *scx, HierExtractArg *ha)
{
     ...
     * belongs to.
     */
    ha->ha_subArea = *DBBBoxCellUseNoUp(use);
    GEOCLIP(&ha->ha_subArea, &ha->ha_interArea);
    hy.hy_area = &ha->ha_subArea;
    hy.hy_target = oneFlat->et_use;
    hy.hy_prefix = TRUE;
    (void) DBEnumArrayElements(use, &ha->ha_subArea, extHierYankFunc, (ClientData) &hy);
...
char *
extSubtreeTileToNode(register Tile *tp, ExtTree *et, HierExtractArg *ha, int doHard)
                      	/* Tile whose node name is to be found */
                	/* Yank buffer to search */
                       	/* Extraction context */
			 ...
			 */
{
	 ...
	 * can cause problems.
	 */
	pNum = DBPlane(DBgetTileType(tp));
	if (pNum >= PL_PAINTBASE &&
		DBPlaneEnumAreaPaint((Tile *) NULL,
		    et->et_lookNames->cd_planes[DBPlane(DBgetTileType(tp))],
	...
	{
	    LayFeedbackAdd(&r, warningStr, parentDef, 1, STYLE_FEEDBACK_PALE);
	    extNumWarnings++;
	}
	return (extNodeName(reg));
    }
	...
	LayFeedbackAdd(&r, errorStr, parentDef, 1, STYLE_FEEDBACK_MEDIUM);
    return ("(none)");
}
...
int
extConnFindFunc(Tile *tp, LabRegion **preg)
{
    if (extHasRegion(tp, extUnInit))
    {
...
LabRegion *
extSubtreeHardNode(Tile *tp, ExtTree *et, HierExtractArg *ha)
{
...
extSubtreeHardSearch(ExtTree *et, HardWay *arg)
{
	...
	for (oneFlat = extSubList; oneFlat; oneFlat = oneFlat->et_next)
	{
	    ...
	    {
		if (DBEnumArrayElements(oneFlat->et_realuse, &arg->hw_area,
	...
	(void) DBEnumArrayElements(ha->ha_subUse, &arg->hw_area,
		extSubtreeHardUseFunc, (ClientData) arg);
    }
}
 ...
 */
    ...
    /*ARGSUSED*/
extSubtreeHardUseFunc(CellUse *use, Transform *trans, int x, int y, HardWay *arg)
                 	/* Use that is the root of the subtree being searched */
                     	/* Transform from coordinates of use->cu_def to those
			 * in use->cu_parent, for the array element (x, y).
			 */
             		/* Indices of this array element */
                 	/* Context passed down to filter functions */
{
    SearchContext scx;
    Transform tinv;
