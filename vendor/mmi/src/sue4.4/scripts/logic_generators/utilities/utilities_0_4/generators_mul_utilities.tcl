...
proc generators_mul_compute_ppg_alg { NBITS_A NBITS_B A B Algorithm} {
  ...
  if {[string toupper $Algorithm] == "SIMPLE"} {
	...
	# regular bits
	if {($j >= $ishift) && ($j < [expr $ishift + $NBITS_B])} {
	  set PPGALG($i,$j) b
	}

	# bits above the msb may be zero or sign extended
	if {$j >= [expr $ishift + $NBITS_B]} {
	  set PPGALG($i,$j) c
	}
      }
    }
...
proc generators_mul_dump_ppg { Algorithm } {
...
proc generators_mul_compute_boxcount { NPPs bit zeroopt rowcount_in boxcount_in } {
