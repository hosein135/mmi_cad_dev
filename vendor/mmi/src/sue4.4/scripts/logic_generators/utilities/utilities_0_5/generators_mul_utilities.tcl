  ...
  if {[string toupper $Algorithm] == "SIMPLE"} {
	...
	if {$j >= [expr $ishift + $NBITS_B]} {
	  if {([string toupper $Optimize] == "MSB") || ([string toupper $Optimize] == "BOTH")} {
	    # MSB optimization
	    if {($i == 0)} {
	      if {($j >= $NBITS_B) && ($j < [expr $NBITS_B + $bitmultiple])} {
		set PPGALG($i,$j) h
	      }
	      if {$j == [expr $NBITS_B + $bitmultiple]} {
		set PPGALG($i,$j) g
	      }
	      if {$j > [expr $NBITS_B + $bitmultiple]} {
		set PPGALG($i,$j) z
		...
		set PPGALG($i,$j) g
	      ...
	      if {($j > [expr $ishift + $NBITS_B]) && ($j < [expr $ishift + $NBITS_B + $bitmultiple])} {
		set PPGALG($i,$j) 1
	      }
	      if {$j >= [expr $ishift + $NBITS_B + $bitmultiple]} {
		set PPGALG($i,$j) z
	      }
	    }
	  } else {
