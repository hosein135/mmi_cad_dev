set RCSVERSION(tk.tcl) { $Revision: 1.1 $ }
# 4/01: MMI Modification to fix tk bug:
# The tkPriv(afterId) was used by multiple files
# that depend on tkCancelRepeat in tk.tcl to cancel
# the after commands.  Fixed so each after command
# has a unique entry in the tkPriv array.
...
if {[info exists tk_library] &&
	! [file exists "$tk_library/tclIndex"]} {
	puts "warning: Missing file: $tk_library/tclIndex"
}
...
proc tkScreenChanged screen {
    ...
    set tkPriv(afterId,entry) {}
    set tkPriv(afterId,listbox) {}
    set tkPriv(afterId,scale) {}
    set tkPriv(afterId,scrollbar) {}
    set tkPriv(afterId,textk) {}
    set tkPriv(afterId,fbox) {}
...
proc tkCancelRepeat {} {
    global tkPriv
    # MMI Modification:  each widget needs its own tkPriv(afterId)
    # variable, or they can get stuck in infinite loops
    # when another widget over-writes their "after" command.
    foreach widget {entry listbox scale scrollbar text fbox} {
	catch {after cancel $tkPriv(afterId,$widget)}
	set tkPriv(afterId,$widget) {}
    }
}
