...
set RCSVERSION(prop_menu.tcl) { $Revision: 1.57 $ }
...
proc _prop_invoke {w pn} -desc {
  ...
  global _PROP_INFO _PROP_RETURN$w
  if {$_PROP_INFO($w,bg)} {
    # Invoke it now.
    switch [lindex  [_prop_run_invoke $w $pn] 0] {
      "return" {
	_prop_menu_free $w
	_prop_menu_destroy .prop_menu$w
      }
      "reload" {
	# TODO!!!!
      }
    ...
    # Signal prop_menu2 to invoke this widget in its call stack.
    set _PROP_RETURN$w "invoke $pn"
  }
}


proc _prop_run_invoke {w pn} -desc {
  Actually do a widget invocation.
} {
  global _PROP_INFO
  set level $_PROP_INFO($w,level)
	...
	[_prop_getopt $w $pn -reload 1]} {
	  set _PROP_RETURN$w [list invoke $pn]
      }
    }
    ...
    set button_cmd [_prop_getopt $w $pn -button]
    if {$button_cmd != "" } {
      # Exec -button command in level 0.
      # This is deprecated; should use -command.
      uplevel #$level $button_cmd
      # This raise fails in the unlikely case that some one used the
      # X menu "Quit" command to kill the parent prop menu.
    ...
    set command [_prop_getopt $w $pn -command]
    if { $command != "" } {
      _prop_update $w value
    ...
    # The -return returns instantly, without doing prop_list value
    # verification, so the caller can repost a new menu if desired.
    if {[set retval [_prop_getopt $w $pn -return]] != ""} {
      _prop_update $w value
      return "return $retval"
    }
    
    if {[_prop_getopt $w $pn -reload 1]} {
      _prop_update $w value
...
proc _prop_bind {w pn entry type} -desc {
  Used by prop_menu to add bindings to a prop menu item
    ...
    # Add the entry itself onto the front of the list, so that user
    # specified bindings on the entry (added in this proc, below)
    # over-ride prop_menu bindings, for example, <Return>.
    regexp {^\.[^.]*} $entry prop_menu
    set tags [bindtags $entry]
    set tags [lremove $tags $entry]
    set tags [lremove $tags $prop_menu]
    bindtags $entry [concat $entry $prop_menu $tags]
	...
	for {set i 0} {[llength $wlist($i)]} {incr i} {
	    set wlist([expr $i+1]) ""
	    foreach sw $wlist($i) {
		bind $sw <ButtonRelease-1> $cmd
		append wlist([expr $i+1]) [winfo children $sw]
	    }
	}
    }
    ...
    # Optional bindings
    set bindings [_prop_getopt $w $pn -bind]
    while {[llength $bindings] >= 2} {
      bind $entry [lindex $bindings 0] [lindex $bindings 1]
      set bindings [lrange $bindings 2 end]
    }
...
proc _prop_menu_destroy {win} -desc {
  ...
  if {[info exists _PROP_SAVE(oldFocus,$win)] && $_PROP_SAVE(oldFocus,$win) != ""} {
    catch {focus $_PROP_SAVE(oldFocus,$win)}
  }

  if {[winfo exists $win]} {

    if {![catch {wm geometry $win} geom]} {
      regexp {\+([0-9]+)\+([0-9]+)$} $geom junk x y
      set new_pos "$x $y"
    }
...
proc _prop_menu_free {w} -desc {
  Free storage used by prop menu.
} {

  global _PROP_INFO
  catch {
      # Recover prop_menu memory.
...
proc _prop_menu_done {w retval} {
    ...
    if {$_PROP_INFO($w,bg)} {
      _prop_update $w value
      if {$_PROP_INFO($w,apply) != ""} {
	eval $_PROP_INFO($w,apply)
      }
      _prop_menu_free $w
      _prop_menu_destroy .prop_menu$w
    } else {
      # Must call prop_update using the calling stack of prop_menu,
      # so set _PROP_RETURN to restart prop_menu
      global _PROP_RETURN$w
      set _PROP_RETURN$w "done $retval"
    }
}

proc _prop_menu_cancel {w} {
    # The if here protects you when you cancel twice.
    #if {[winfo exists .prop_menu$w]} { _prop_update $w origvalue }
    _prop_menu_free $w
    _prop_menu_destroy .prop_menu$w
    global _PROP_RETURN$w
    set _PROP_RETURN$w cancel
}

proc _prop_menu_run {w {cmd ""}} -desc {
  Run specified cmd.
} {
    global _PROP_INFO
    if {$cmd == ""} {set cmd $_PROP_INFO($w,apply)}
...
proc _prop_menu_apply {w {cmd ""}} -desc {
  Bound to apply button.
} {
    global _PROP_INFO
    if {$_PROP_INFO($w,bg)} {
      # If in background, run cmd now.
      _prop_menu_run $w $cmd
    } else {
      # If not in background, run cmd using call stack of prop_menu2.
      # Must signal it to restart to run this command.
      global _PROP_RETURN$w
      set _PROP_RETURN$w [list apply $cmd]
    }
}

# NEW OPTIONS, NOT IN DOC YET:
# -options {stuff} : only for -button now; add stuff to the widget options.
#		Dont like this.  Either rename it to something specific,
#		or make it applicable to more widgets, or both.
# -align center: center the widget, instead of putting on right.
# -font <fontname> : a prop_menu2 option to set the default font, except for listboxes.
# 
...
Options are:
  ...
  -apply applyproc
  -buttons button_list
  -bg 1              Background window.  prop_menu2 returns immediately,
		     but the window stays up until closed by the user.
		     If this option is specified, all variables in the prop_list
		     must be global variables; the calling procedure might
		     return before the prop_menu is closed, so variables local
		     to the calling procedure would not exist.
		     TODO: verification is not working in background mode.
  ...
  selected from the following list:
	...
	-listbox		Create a listbox with a vertical scroll bar.
				If a variable is given, it is the initial
  ...
  If a -entry has a TRIGGER property, then a Return pressed while the
  focus is in the entry causes the trigger; otherwise, a Return
  pressed while the focus is in the entry does the default action,
  which normally closes the window.
  ...
  The following additional -options may appear after the property type.
	...
	-inline  For -radio, put the radio buttons in the same line,
		rather than one per line.
	-fg color  backgr  Foreground color for the values of some options,
		notably labels.
	-hide   The property is not displayed.  Useful for development.
		Equivalent to -when 0
	-bind <list>  A list of 2*n strings representing bindings, where each
		pair is an event and a command to execute on that event.
		The binding is attached to the active part of the menu item.
		For a listbox, bindings are on the listbox itself.  To get the 
		window name of a listbox in the prop_menu, use:
		[list "" "" -listbox -bind {<Map> "saveit %W"}]
		where saveit is a proc that saves the listbox window name for later use.
  ...
  global _PROP_RETURN$w;   # Return value for tkwait
  ...
  set keywords "{message {}} {title {}} {x {}} {y {}} \
	{atmouse {}} {apply {}} {buttons {}} {font {}} {bg 0}"
  ...
  if { $font == "" } { set font $DIALOG_FONT }

  set _PROP_INFO($w,font) $font
  set _PROP_INFO($w,buttons) $buttons
  set _PROP_INFO($w,apply) $apply
  set _PROP_INFO($w,title) $title
  set _PROP_INFO($w,message) $message

  # Kill off existing prop_menu with this title, if any.
  if {[info exists _PROP_SAVE(win,$title)]} {
    _prop_menu_destroy $_PROP_SAVE(win,$title)
  }
  set _PROP_SAVE(win,$title) $prop_menu
  ...
  # If in background mode, the calling stack is going to disappear as
  # soon as we return, so all variables better be global.
  set _PROP_INFO($w,level) [expr {$bg ? 0 : [info level] - 1}]
  set _PROP_INFO($w,bg) $bg
  set _PROP_INFO($w,apply) $apply
  ...
  # If in background mode, we do NOT restore old focus on exit.
  if {$bg} {
    set _PROP_SAVE(oldFocus,$prop_menu) ""
  } else {
  ...
  while {$prop_menu_repost} {
    ...
    # If in background mode, we are done.
    if {$bg} {return}
	...
	tkwait variable _PROP_RETURN$w
	catch {cursor_wait $prop_menu 0}
      }

      set prop_return [set _PROP_RETURN$w]

      # TODO: Must do apply!!
      switch [lindex $prop_return 0] {
	"done" {
	    set retval [lindex $prop_return 1]
	    _prop_update $w value
	    break
	}
	"continue" {
	...
	"invoke" {
	  set result [_prop_run_invoke $w [lindex $prop_return 1]]
	  switch [lindex $result 0] {
	    return {
	      set retval [lindex $result 1]
	      break
	    }
	    reload {
	      set prop_menu_repost 1
	...
	"apply" {
	  _prop_menu_run $w [lindex $prop_return 1]
	  # Break verification "while" loop.  Parent loop will
	  # destroy and then repost prop_menu.
	  set prop_menu_repost 1
	  break
	...
	default {
	  puts "prop_menu2 unrecognized _PROP_RETURN$w: $prop_return"
	  break  ;# Die
	}
      }
    ...
    if {0 && $prop_menu_repost && [winfo exists $prop_menu]} {
      # If the user has moved the window, try to repost it in the same spot.
      if {![catch {wm geometry $prop_menu} geom]} {
	#regexp {\+([0-9]+)\+([0-9]+)$} $geom junk x y
	set _PROP_INFO($w,geom) $geom
      }
    }
  }  ;# while {$prop_menu_repost}

  update ;# Get the window off the screen.
...
proc _prop_menu_post {w} {
  global _PROP_INFO
    ...
    set font $_PROP_INFO($w,font)
    set buttons $_PROP_INFO($w,buttons)
    set apply $_PROP_INFO($w,apply)
    set title $_PROP_INFO($w,title)
    set message $_PROP_INFO($w,message)
    set level $_PROP_INFO($w,level)

    # By not destroying the whole window the second time through here,
    # we cut down on the flashing a little, but not always for unknown reasons.
    if {[winfo exists $prop_menu]} {
      # If you save the full geometry  and restore it, the window does not flash as much,
      # but then if the window contents grows, the window wont expand to make it fit.
      #set geom [wm geometry $prop_menu]
      bind $prop_menu <Destroy> ""
      eval destroy [winfo children $prop_menu]
      #wm geometry $prop_menu $geom
      set do_warp 0
    } else {
      toplevel $prop_menu 
      ...
      set do_warp 1
      wm geometry $prop_menu $_PROP_INFO($w,geom)
    }
    ...
    # Removed 2/6/02: Seems like this binding is repeated below.
    ...
    set panel $prop_menu.panel$panelnumber
    frame $panel
	...
	if {($when != "" && ![uplevel #$level "expr { $when }"]) || \
	...
	# Default fill options for this frame.
	...
	set fg [_prop_getopt $w $pn -fg]
	if {$fg == ""} {set fg black}
	set relief [_prop_getopt $w $pn -relief]
	...
	"button" {
	    ...
	    button $frame.button -font $font -text $proptext \
		  -padx 1 -pady 1 -anchor w -fg $fg \
		  -command "_prop_invoke $w $pn"
	    set but_opts [_prop_getopt $w $pn -options]
	    bind $frame.button <Return> "_prop_invoke $w $pn"
	...
	"entry" {
	    ...
	    entry $frame.entry \
		  ...
		  -relief [use_first relief 'sunken] -bd 1 \
		  -highlightthickness 1
	    pack $frame.entry -side left -fill x -ipady 1 -expand 1

	    # If there is a -command or -reload, convert this entry into a trigger
	    # when the user presses Enter key.
	    if {[_prop_getopt $w $pn -command 1] || [_prop_getopt $w $pn -reload 1]} {
	      bind $frame.entry <Return> "_prop_invoke $w $pn;break"
	    }
	...
	"popup" {
	    ...
	    frame $frame.f -bd 2 -relief [use_first relief 'sunken]
	...
	"choice" {
	    ...
	    frame $frame.f -bd 2 -relief [use_first relief 'raised]
	...
	"radio" {
	    ...
	    frame $frame.f -relief [use_first relief 'sunken] -bd 1
	    set values [_prop_getopt $w $pn -values]
	    set inline [_prop_getopt $w $pn -inline 1]
	    #set specwidth [_prop_getopt $w $pn -maxwidth]
	    #if {$specwidth==""} {set specwidth 0}
	    #set curwidth 0
		...
		#if {! [set inline [expr {$curwidth < $specwidth}]]} { set curwidth 0 }
		radiobutton $frame.f.c_$ii -variable "_PROP_INFO($w,$pn,value)" \
		    -font $font \
		    -text "$choice" -value [lindex $values $ii] -anchor w \
		    -command "_prop_invoke $w $pn"
		pack $frame.f.c_$ii  \
		    -side [expr {$inline ? "left" : "top"}] \
		    -fill [expr {$inline ? "y" : "x"}]
		_prop_bind $w $pn $frame.f.c_$ii "radio"
		#incr curwidth [expr [string length [lindex $values $ii]]+3]
		incr ii
	    }
	...
	"label" {
	    ...
	    if { $_PROP_INFO($w,$pn,varname) != "" } {
	      if {$proptext != ""} {
		label $frame.label -font $font -text $proptext -width $namewidth \
		-anchor w -bd 3 -fg $fg -relief [use_first relief 'flat]
		pack $frame.label -side left -ipady 1
	      }
	      label $frame.label2  \
		  -width $entrywidth -font $font -anchor w \
		  -textvariable _PROP_INFO($w,$pn,value) -fg $fg -relief [use_first relief 'flat]
		  # -relief sunken -bd 1 -highlightthickness 1 -anchor w
	      pack $frame.label2 -side left -fill x -ipadx 2 -ipady 1 -expand 1
	      _prop_bind $w $pn $frame.label2 "label"
	    } else {
	      label $frame.label -font $font -text $proptext -bd 3 -fg $fg
	      pack $frame.label -side left -fill x
	      _prop_bind $w $pn $frame.label "label"
	    }
	}
	"-listbox" {
	  global LISTBOX_FONT
	  ...
	  frame $frame.items \
		  -borderwidth 0 \
		  -relief [use_first relief 'raised]

	  scrollbar $frame.items.vscroll \
		  -relief [use_first relief 'raised] \
		  -command "$frame.items.list yview"

	  listbox $frame.items.list \
		  -selectmode extended \
		  -font $LISTBOX_FONT \
		  -exportselection false \
		  -relief [use_first relief 'raised] \
		  -yscrollcommand "$frame.items.vscroll set"

	  pack $frame.items.list -expand 1 -fill both -side left
	  pack $frame.items.vscroll -fill y -side right
	  pack $frame.items -expand 1 -fill both -side top
	  set fill_option "-expand 1 -fill both"
	  set fnd_listbox($panelnumber) 1

	  # Optionally set initial contents of listbox from variable.
	  if { $_PROP_INFO($w,$pn,varname) != "" } {
	    foreach thing $_PROP_INFO($w,$pn,value) {
	      $frame.items.list insert end $thing
	    }
	  }
	  ...
	  bind $frame.items.list <B1-Motion> {
	      #set tkPriv(x) %x
	      #set tkPriv(y) %y
	  ...
	  # Optional bindings.
	  _prop_bind $w $pn $frame.items.list "listbox"
	}
	default {
	...
	# 2/6/02: Expand only listboxes.  Dont know if this change will break
	# existing prop menus.
	#eval pack $frame -side top -in $parent_frame -expand 1 -pady 1 $fill_option
	eval pack $frame -side top -in $parent_frame -pady 1 $fill_option
	...
	if {[_prop_getopt $w $pn -break 1]} {
	    ...
	    set panel $prop_menu.panel[incr panelnumber]
	    frame $panel
	    set namewidth $panel_namewidth($panelnumber)
	}
    }; #for pn
	...
	set but_cmd "_prop_menu_cancel $w"
    ...
    bind $prop_menu <Escape> "_prop_menu_cancel $w"
    bind $prop_menu <Any-Control-c> "_prop_menu_cancel $w"
    ...
    # window ourselves.
    bind $prop_menu <Destroy> "_prop_menu_free $w;set _PROP_RETURN$w cancel"
	...
	if {[info exists fnd_listbox($i)]} {
	  pack $prop_menu.panel$i -side left -anchor n -fill both -expand 1
	} else {
	  pack $prop_menu.panel$i -side left -anchor n -fill x -expand 1
	}
	if {$i < $panelnumber} {
    ...
    if { $do_warp && $default_button_widget != "" } {
...
proc prop_dialog {args} -desc {
  ...
  -bg 1              Background window.  prop_dialog returns immediately,
		     but the window stays up until closed by the user.
