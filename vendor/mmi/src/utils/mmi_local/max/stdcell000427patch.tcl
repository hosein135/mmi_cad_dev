set RCSVERSION(stdcell.tcl) { $Revision: 1.29 $ }
...
proc _stdcell_locate_fet {id} {
	...
	[techinfo2 width v0] + [techinfo2 sep v0 gate] + $l ]
    ...
    if { $fet_info($id,share_diff_right) } {
        ...
        set fetlen [expr $fetlen + [techinfo2 sep v0 gate] + \
	...
	set gx1 [expr $x + [techinfo2 overlap diff v0] + \
	    [techinfo2 width v0] + [techinfo2 sep v0 $fettype]]
    }
    if { $fet_info($id,long_cont_left) } {
	...
	# if { $LAYINFO(option:draw_fets_using) == "subcells" } {
	#     set rootcell [lay_rootcell]
	#     # Create a unique name.
	#     if {! [info exists fet_info(unique_id)] } {
	# 	set fet_info(unique_id) 0
...
proc _stdcell_draw_fets {type} {
	...
	_stdcell_draw_fet $id $LAYINFO(option:draw_fets_using)
...
proc stdcell_route {} {
    ...
    if { [use_first STDCELL(edit_cell)] != [lay_editcell] } {
      max_error "Can only call Layout Router immediately after \
...
proc _stdcell_draw_via {x1 y1 layer1 layer2} {
	...
	max_error "drawing via using gcell failed"
    }
	...
	max_error "can not find via subcell named $vialayer"
    }
...
proc _stdcell_fold_fets {type forder} {
	...
	set newwidth [uusnap [expr $fet_info($id,w) / $nfolds]]
...
proc _stdcell_calculate_stuff {fetlist {warn 0}} {
    ...
    ############################################################################
    # Other options.
    # Designated by LAYINFO(option:<name>)
    ...
    # Fill in default value, so user can see it.
    ...
    _stdcell_init LAYINFO(option:draw_fets_using) gcells
    set LAYINFO_HELP(option:draw_fets_using) {-choice {paint subcells gcells} \
      ...
      WARNING: If you set this option to "gcells", and also \
      set the preroute_inner or preroute_outer option to "metal+poly", \
      a runtime error will occur unless you also have\
      an updated fet gcell that supports this combination of options. \
      To avoid the error, set this option to "paint". \
      }}
    _stdcell_init LAYINFO(option:draw_vias_using) gcells
    set LAYINFO_HELP(option:draw_vias_using) {-choice {paint subcells gcells} \
...
proc _stdcell_fet_info_init {id type l w g s d} {
    ...
    set fet_info($id,l) [uusnap $l]
    set fet_info($id,w) [uusnap $w]
    set fet_info($id,g) $g
    set fet_info($id,s) $s
    set fet_info($id,d) $d
...
proc _stdcell_check_license {check} -desc {
    Check for layout generator license available, 
} -doc {
    If license found, return 1.
    If option is -check, just return, do not check out license.
    If option is -return, check out license.
} {
    # See if we already checked out a license earlier.
    # Use an innocent sounding variable name and set it to an
    # an obscure value to indicate whether
    # we have checked out the license or not yet.
    # By deriving it from pid, it changes on each max invocation.
    global STDCELL
    set license_key [expr ( [pid] * 3 ) % 97]
    if {[info exists STDCELL(x_info)]} {
	if { $STDCELL(x_info) == $license_key} {
	    return 1  ;# License already checked out.
	} else {
	    # The user is screwing around trying to set this to
	    # some value to see if they can work around the license
	    # software.  Punish them.  No warning, just fail.
	    return 0
	}
    }

    # No license already checked out.  See if one is available.
    # Build up name so it is non-trivial to change it in the binary
    # when someone wants to get a max_laygen license illegally by changing
    # it to say, "max" in the binary.
    set name max
    append name "_" \
	[string range layout 0 2] [string range regenerator 2 4]
    if { [license_check $check $name] == 0 } {
	warning "Could not acquire '$name' license\n"
	return 0
    } else {
	return 1
    }
...
proc stdcell_load_direct {filename} -desc {
  reads in a sim file and places fets and creates fly lines for circuit
    ...
    # The stupid unset stops at the first variable that does not exist.
	...
	"P" {
	    ...
	    if {! $fnd_comment } {
		puts "Invalid .sim file: no device location comments found"
		puts "exiting..."
		return 0
	    }
	    ...
	    if { $l > 100000 || $w > 10000 } {
		puts "Invalid .sim file: fets too huge!"
		puts "exiting..."
		return 0
	    }
	    if {! [_stdcell_is_on_grid $w]  || ! [_stdcell_is_on_grid $l] } {
		puts "error: FET width or length not divisible by resolution = [res]"
		puts "exiting..."
		return 0
	    }
...
if { $iotype == "io" } { set iotype "inout" }
	...
	eval lay_box [uusnap $x1 0 $x1 0]
	:label -kind global $STDCELL(gnd) n
	lay_box 0 [expr $cell_height - $tmp] $cell_width [expr $cell_height + $tmp]
	:paint [techinfo2 layer m1]
	eval lay_box [uusnap $x1 $cell_height $x1 $cell_height]
	:label -kind global $STDCELL(vdd) n
    }
...
# Edit the LAYINFO array.
# The type is the prefix of LAYINFO entries to edit,
# or 0 for a top-level menu.
# Return 0 if user canceled, 1 if ok.
proc stdcell_setup { {type 0} } {
...
proc stdcell_reload {} -desc {
  Regenerate the current cell with current options.
    ...
    return [stdcell_load_sim ${newpath} no]
}

# If verbose is 1, you get to edit everything in LAYINFO.
proc stdcell_load_sim { {filename ""} {verbose 0} } {
    global STDCELL
	...
	set list [_cell_file_box "Select sim file to load" *.sim $filename]
	if {$list == ""} {
	  # user hit cancel key
	  return 0
	}
    
	setl {dir file suffix} $list
	set filename "$dir/$file$suffix"
    }
    ...
    regsub {\.sim$} $filename "" STDCELL(output_filename)
    ...
    if { $verbose != "no" } {
      if {! [stdcell_setup $verbose] } { return }
    }
...
proc load_spice {{filename ""}} -desc {
  ...
  if {$filename == ""} {
    set list [_cell_file_box "Select spice file to load" *.sp]
    if {$list == ""} {
      # user hit cancel key
      return
    }
  ...
  foreach net [array names nets] {
	...
	continue
      ...
      lay_box $x $y [expr $x + [res] [expr $y + [res]]
      :paint $layer
      :label -kind hidden $label n $layer
...
proc laygen_init {} -desc {
    Add Layout Generator to Tool menu if license exists.
} {
    global MAX_DEVELOPER
    ...
    if { [license_check -check max_laygen] } {
