set RCSVERSION(stdcell.tcl) { $Revision: 1.32 $ }
# BUG:
...
proc stdcell_load_direct {filename} -desc {
  reads in a sim file and places fets and creates fly lines for circuit
	...
	"P" {
	    ...
	    if {! $fnd_comment } {
		max_error "Invalid .sim file: no device location comments found"
		puts "exiting..."
		return 0
	    }
	    ...
	    if { $l > 100000 || $w > 10000 } {
		max_error "Invalid .sim file: fets too huge!"
		puts "exiting..."
		return 0
	    }
	    if {! [_stdcell_is_on_grid $w]  || ! [_stdcell_is_on_grid $l] } {
		max_error "FET width or length not divisible by resolution = [res -mask]"
		puts "exiting..."
		return 0
	    }
...
if { $iotype == "io" } { set iotype "inout" }
    ...
    if {[llength $fet_sortlist] == 0} {
      max_error "No fets in input file"
      return 0
    }
