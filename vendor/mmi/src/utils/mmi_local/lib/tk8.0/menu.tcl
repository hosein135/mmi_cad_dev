set RCSVERSION(menu.tcl) { $Revision: 1.4 $ }
...
# Note: HP machine maps the roller on the middle mouse button to
# Button-4 and Button5.  But we dont want that to select menu items:
# it is very confusing.  So map the three buttons we want individually.
#bind Menu <ButtonPress> { tkMenuButtonDown %W }
bind Menu <ButtonPress-1> { tkMenuButtonDown %W }
bind Menu <ButtonPress-2> { tkMenuButtonDown %W }
bind Menu <ButtonPress-3> { tkMenuButtonDown %W }

#bind Menu <ButtonRelease> { tkMenuInvoke %W 1 }
bind Menu <ButtonRelease-1> { tkMenuInvoke %W 1 }
bind Menu <ButtonRelease-2> { tkMenuInvoke %W 1 }
bind Menu <ButtonRelease-3> { tkMenuInvoke %W 1 }
...
proc tkMenuNextMenu {menu direction} {
    ...
    # add for MMI
    _menu_warp_cursor [$mb cget -menu] _MENU_
}
...
# Uses the MMI _warp_cursor command

proc _menu_warp_cursor {win i} {
  ...
  if {$i == "_MENU_"} {
    ...
    # place cursor in the center of active

    # try to compute height of entry
    set yp [$win yposition $i]
    set ypn [$win yposition [expr $i + 1]]

    set height [expr $ypn - $yp]
    if {$height == 0} {
      # must be the last one, go the other way
      if {$i > 0} {
	set height [expr $yp - [$win yposition [expr $i - 1]]]
      }
      if {$height == 0} {
	# still lost
	set height 20
    ...
    set y [expr [winfo rooty $win] + $yp + $height/2]
  }

  catch "_warp_cursor $x $y"
}
...
proc tkFirstMenu w {
	...
	if {[winfo class $w] == "Menu"} {
	    ...
	    tkMbPost $w
	    tkMenuFirstEntry [$w cget -menu]
 	    _menu_warp_cursor [$w cget -menu] _MENU_
	}
    }
}
