...
;;;
;;;		| (test STRING)		;; Matches is C expression STRING
;;;					;; is true.
;;;
...
;;;					;; any of the following children do
;;;					;; not match.
;;;
;;;		| (repeat child+)	;; Match zero or more sequences of
;;;					;; children until of the doesn't match
;;;					;; or the end reached.  Advances to the
;;;					;; child that caused the repeat loop
;;;					;; to terminate.
;;;
;;;		| STRING		;; Don't match anything, just insert
;;;					;; the string contents as C code.  The
;;;					;; string may contain $ variables.
;;;
;;;		| =VARIABLE		;; Bind the specified variable to the
;;;					;; most recently matched child.
;;;		;
;;;
;;;
...
(defun rewrite-format-c-code (str)
  (rewrite-string-replace-match "$\\([A-Za-z0-9_]+\\)" str
				(rewrite-format-variable "\\1")))
...
(defun rewrite-pattern-begin (pattern)
  ...
  (insert "\n")
  (indent-to rewrite-current-indent 1)
  (insert (rewrite-declare-node "__prev" "NULL"))
  (insert "\n")
  (insert "\n"))
...
(defun rewrite-match-token-set (token-set)
  (let ((toks (rewrite-get-token-set token-set)))
    ...
    (while toks
	  ...
	  (progn
	    (insert " ||\n")
	    (indent-to (+ rewrite-current-indent 14) 1))))
    (insert ");\n")
...
(defun rewrite-bind-symbol (var value)
  (indent-to rewrite-current-indent 1)
  (insert
   (format "%s = %s;\n"
	   (rewrite-format-variable (rewrite-get-variable-name var)) value)))
...
(defun rewrite-repeat-begin ()
  ...
  (setq rewrite-current-indent (+ rewrite-current-indent 2))
  (indent-to rewrite-current-indent 1)
  (insert (rewrite-declare-node "__temp" "__prev"))
  (insert "\n")
  (indent-to rewrite-current-indent 1)
  (insert (rewrite-declare-node "__prev" "__temp"))
  (insert "\n")
  (indent-to rewrite-current-indent 1)
...
(defun rewrite-repeat-end (where)
  ...
  (insert "}\n")
  (indent-to rewrite-current-indent 1)
  (rewrite-note "have to set __prev to point to the beginning of the repeat")
  (indent-to rewrite-current-indent 1)
  (insert "__prev = ")
  (cond
...
(defun rewrite-generate-pattern-child (child optional rest where)
  (cond 
   ...
   ((and (symbolp child)
    ...
    (rewrite-bind-symbol child "__prev"))
   ...
   ((and (listp child)
	...
	(setl (optional rest where)
	      (rewrite-generate-pattern-child (car children)
						  ...
						  (cons 'parent-firstonly (cdr where))
						where)))
...
(defun rewrite-generate-test (rule)
  (if (not (and (listp rule) (eq (car rule) 'test)))
      (error "rewrite-generate-predicate: invalid predicate -> %s" rule)
...
(defun rewrite-construct-siblings (sibs next)
  (cond 
   ((null sibs)
    (indent-to rewrite-current-indent 1)
    (insert (format "__new_tree = %s;\n" next)))
   ((listp sibs)
    (rewrite-construct-siblings (cdr sibs) next)
    (indent-to rewrite-current-indent 1)
    (insert "__sibling = __new_tree;\n")
    (rewrite-save-var "__sibling")
    (rewrite-construct (car sibs))
    (rewrite-restore-var)
    (rewrite-if-begin "__new_tree != NULL")
    (indent-to rewrite-current-indent 1)
    (insert (rewrite-set "__new_tree" 'sibling "__sibling"))
    (indent-to rewrite-current-indent 1)
    (insert "\n")
    ;;(rewrite-if-else)
    ;;(indent-to rewrite-current-indent 1)
...
(defun rewrite-construct-parent (token)
  ...
  (let (tok)
	...
	(setq tok (rewrite-get (rewrite-format-variable (car field))
			       (cdr field)))))
    ...
    (insert "\n")
    (indent-to (+ rewrite-current-indent 2) 1)
    (insert (rewrite-declare-node "__child" "__new_tree"))
    (insert "\n")
    (indent-to (+ rewrite-current-indent 2) 1)
    (insert "__new_tree = ")
    (insert (rewrite-create-node "__token"))
    (insert ";\n")
    (indent-to (+ rewrite-current-indent 2) 1)
    (insert (rewrite-set "__new_tree" 'child "__child"))
    (insert "\n")
    (indent-to rewrite-current-indent 1)
    (insert "}\n")))
...
(defun rewrite-format-constructor-arg (arg)
  (if (and (symbolp arg)
	...
	(rewrite-get (rewrite-format-variable (car field))
		     (cdr field)))
    arg))
...
(defun rewrite-insert-node (what where)
  (cond
   ((eq (car where) 'pointer)
    (indent-to rewrite-current-indent 1)
    (insert (format "*%s = %s;" (cdr where) what))
    (insert "\n"))
   ((eq (car where) 'parent)
    (indent-to rewrite-current-indent 1)
    (insert (rewrite-set (cdr where) 'child what))
    (insert "\n"))
   ((eq (car where) 'parent-firstonly)
    ...
    (rewrite-if-else)
    (indent-to rewrite-current-indent 1)
    (insert (rewrite-set (cdr where) 'sibling what))
    (insert "\n")
    (rewrite-if-end))
   ((eq (car where) 'sibling)
    (indent-to rewrite-current-indent 1)
...
(defun rewrite-generate-replace (rule where)
  (let ((repl-rule (cadr rule))
    ...
    (indent-to rewrite-current-indent 1)
    (insert (rewrite-declare-node "__new_tree"))
    (insert "\n")
    ...
    (rewrite-insert-node "__new_tree" where)
...
(defun rewrite-generate-call (rule where)
  (let ((name (cadr rule)))
    (indent-to rewrite-current-indent 1)
    (insert (format "__tree = %s (__tree);\n" name))
    (rewrite-insert-node "__tree" where)))
...
(defun rewrite-generate-rule (rule where)
  (cond
   ...
   ((and (symbolp rule)
    ...
    (rewrite-match-non-null)
    (rewrite-if-match-begin)
    (rewrite-bind-symbol rule "__tree")
    (rewrite-if-end))
   ...
   ((eq (car rule) 'call-rewrite)
    (rewrite-generate-call rule where))
  ...
  (let ((table (make-vector 271 nil)))
    ...
    (mapatoms (flambda (var)
		(if var
		    (progn
		      (indent-to rewrite-current-indent 1)
		      (insert (rewrite-declare-node (symbol-name var)))
		      (insert "\n"))))
	      table)))


(defun rewrite-doit (name flags rules)
  (mapcar (flambda (flag)
...
(defmacro define-rewrite (name &rest body)
  (let ((bodyform body)
    ...
    `(rewrite-doit (quote ,name) (quote ,flags) (quote ,bodyform))))
