...
proc nl_write_def_nets {ofp connectivity routes specialnets design} {
    set count 0
    set has_zero 0
    set has_one 0
    ...
    nl_foreach_design_net -hierarchy -noassign -noconstant net $design {
...
proc nl_write_def_specialnets {ofp net_routes zero_name one_name design} {
    set nets [nl_list_nets -hierarchy -noassign -noconstant]
    set special_nets {}
...
proc nl_write_def_pins {ofp sort design} {
...
proc nl_write_def_components {ofp design} {
    set cells [nl_list_cells -unlinked -library -recursive -noassign]

    puts $ofp "COMPONENTS [llength $cells] ;"
    
    nl_foreach_design_cell -unlinked -library -recursive -noassign cell $design {
...
proc nl_write_def args {
    ...
    global nl_hierarchy_separator nl_current_design nl_write_def_dividerchar
	...
	if { $header != 0 } {
	    ...
	    set row_sites [nl_list_row_sites]
	...
	if { $components != 0 } {
	    nl_write_def_components $ofp $design
	}

        if { $pins != 0 } {
	    nl_write_def_pins $ofp $design
	}
	...
	if { $specialnets != 0 } {
	    nl_write_def_specialnets $ofp $net_routes $zero_name $one_name $design
	}

	if { $net_connectivity != 0 || $net_routes != 0 } {
	    nl_write_def_nets $ofp $net_connectivity $net_routes $specialnets $design
	}
