...
extern int def2pnl_pre_comment_mode;
>>
...
#token			"[A-Za-z0-9_]+"
#token			"#"		<< zzskip ();
					   def2pnl_pre_comment_mode = START;
					   zzmode (COMMENT);
					>>
#token DEF_BAD_KEYWORD	"[A-Za-z_][A-Za-z0-9_]*"
...
#token DEF_IDENT	"~[#\ \t\n]+"	<< zzmode (START); >>
#token			"#"		<< zzskip ();
					   def2pnl_pre_comment_mode = IDENTIFIER;
					   zzmode (COMMENT);
					>>
...
#token DEF_IDENT	"~[#\ \t\n]+"	<< zzmode (START); >>
#token			"#"		<< zzskip ();
					   def2pnl_pre_comment_mode = IDENT_NEW_OR_PLUS;
					   zzmode (COMMENT);
					>>
...
#token			"\n"		<< zzline++;
					   zzskip ();
					   zzmode (def2pnl_pre_comment_mode);
					>>
...
units_decl	: << char *name;
		  ...
		  number > [num]
		  DEF_SEMI!
			<< def2pnl_distance_units (name, num); >>
		;
...
row_decl	: << char *name;
		  ...
		  DEF_SEMI!
			<< if ( one != 1 ) {
			     def2pnl_error ("currently don't handle BY != 1 "
					    "in row site (got %d).", one);
			   }
			   def2pnl_row_site (name, core_name, x0, y0,
					     orient, count, width, height);
			>>
		;
...
pin_plus [pnl_port pport]
		: << pnl_loctype loctype;
		     int x, y;
		     pnl_orientation orient;
		  >>
		  DEF_PLUS!
		    ( placed_clause > [loctype, x, y, orient]
				<< def2pnl_port (pport, loctype, x, y, orient);
				>>
		    | net_clause
		    | dir_clause
		    | use_clause
...
route_branch [int special] > [pnl_branch branch]
		: << int x0;
		  ...
		  "\(" number > [x0] number > [y0] "\)"
				<< $branch = pnl_branch_create (layer, x0, y0, width);
...
layer_clause [pnl_port pport]
		: << char *name;
		  ...
		  coordinate > [x0, y0] coordinate > [x1, y1]
				<< pnl_port_set_geometry (pport, name,
							  x0, y0, x1, y1);
				>>
		;
