...
nl_attr_create (char *name, nl_object owner, nl_kind attr_of,
		nl_density density, int attr_size,
		nl_init_fun init_fun, nl_free_fun free_fun)
{
  nl_attr result = NULL;
  ...
  if ( density == nl_density_dense ) {
    ...
    dense_result->array = ar_alloc (num_elts, attr_size);

    ar_make_size (dense_result->array, num_elts);

    dense_result->init_fun = init_fun;
    dense_result->free_fun = free_fun;

    ar_for_all_pointers (dense_result->array, elt_ptr) {
      if ( init_fun != NULL ) {
	init_fun (elt_ptr, );
      }
      else {
	memset (elt_ptr, 0, attr_size);
      }
    } ar_end_for;
  }
  ...
  sparse_case:
    ...
    sparse_result->table = ht_alloc (16, ht_hash_ptr, NULL, NULL, NULL);
...
nl_attr_access (nl_attr attr, nl_object obj, int set, void *value_p)
{
  ...
  if ( density == nl_density_dense ) {
	...
	void *elt_ptr = ar_addr_of (dense_attr->array, i);

	if ( dense_attr->init_fun != NULL ) {
	  dense_attr->init_fun (elt_ptr);
	}
	else {
	  size_t elt_size = ar_elt_size (dense_attr->array);
	  memset (elt_ptr, 0, elt_size);
	}
      }
    }
    ...
    if ( set ) {
      ...
      ht_get_attribute_for_entry (sparse_attr->attr, hte, value_p);
    }
  }
  else {
    ASSERT (0);
...
{
  if ( attr->density == nl_attr_dense ) {
    nl_attr_access (attr, obj, 0, NULL);
  }
}
...
nl_design_attr_create (char *name, nl_design design, nl_density density,
		       int attr_size, nl_init_fun init_fun, nl_free_fun free_fun)
{
  nl_attr result 
    = nl_attr_create (name, (nl_object) design, nl_kind_design, density,
		      attr_size, init_fun, free_fun);
