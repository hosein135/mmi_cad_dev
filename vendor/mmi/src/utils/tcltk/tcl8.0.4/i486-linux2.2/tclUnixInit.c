 ...
 *
 * RCS: @(#) $Id: tclUnixInit.c,v 1.2 1999/01/13 01:41:51 pat Exp $
 */
    ...
    Tcl_Interp *interp;		/* Interpreter to source rc file into. */
{
	...
	if (fullName == NULL) {
		...
		if (Tcl_EvalFile(interp, fullName) != TCL_OK) {
		    ...
		    if (errChannel) {
			/* 11-98: Pat changed.  This is just wrong.
			 * If there is an error interpreting the
			 * start-up script, the error message
			 * is left in errorInfo, not interp->result.
			 * The new code below is the correct way.
			 * To see another example of how this works,
			 * see the call to Tcl_EvalFile in Tcl_Main.
			 */
			/* Old:
