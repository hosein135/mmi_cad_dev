 ...
 *
 * RCS: @(#) $Id: tkFont.c,v 1.2 1999/01/13 01:41:51 pat Exp $
 */
...
typedef struct LayoutChunk {
    ...
    void *cache;		/* Pat added: cache for rotated text */
} LayoutChunk;
...
typedef struct TextLayout {
    ...
    int width;			/* The maximum width of all lines in the
				 * text layout. */
    int rotate;			/* Pat added: rotation */
    int height;			/* Pat added: total height in pixels */
...
extern void Tk_RotTextXY _ANSI_ARGS_((int x0,int y0,int rot,int *px, int *py));
extern void Tk_NormalizeBox _ANSI_ARGS_((int *px1,int *py1,int *px2,int *py2));
...
#define MAX_LINES 50
	...
	layoutPtr->chunks[0].cache              = 0;
    }
    layoutPtr->height = layoutHeight;    /* pat added */
...
/* Like Tk_ComputeTextLayout, but adds:
 *    textLayout argument is textLayout returned by previous call to
 *        this function, or NULL on first call;
 *    rotate argument is text rotation (0, 90, 180, 270).
 *    recalc argument is True if recalculation is necessary.
 *        Recalculation flushes the rotated text cache, and slows down
 *        the redraw rate for rotated text.
 */
Tk_TextLayout
Tk_ComputeTextLayoutExt(textLayout, tkfont, string,
	numChars, wrapLength, justify, flags,
	rotate, recalc, widthPtr, heightPtr)
    Tk_TextLayout textLayout;  /* Pointer to previously computed text layout */
    Tk_Font tkfont;		/* see Tk_ComputeTextLayout */
    CONST char *string;		/* see Tk_ComputeTextLayout */
    int numChars;		/* see Tk_ComputeTextLayout */
    int wrapLength;		/* see Tk_ComputeTextLayout */
    Tk_Justify justify;		/* see Tk_ComputeTextLayout */
    int flags;			/* see Tk_ComputeTextLayout */
    int rotate;			/* Text Rotation */
    int recalc;			/* If True, do complete recalc */
    int *widthPtr;		/* Filled with width of string. */
    int *heightPtr;		/* Filled with height of string. */
{
    TextLayout *layoutPtr;
    ...
    if (textLayout == NULL) {
	textLayout = Tk_ComputeTextLayout(tkfont, string,
	    numChars, wrapLength, justify, flags, NULL, NULL);
    }
    layoutPtr = (TextLayout*) (textLayout);
    layoutPtr->rotate = rotate;
    if (widthPtr != NULL) {
	*widthPtr = layoutPtr->width;
    }
    if (heightPtr != NULL) {
...
/* Function to unrotate an (x,y) coordinate.  We do not rotate
 * the coordinates saved in the text chunks.  Instead, when we want
 * to compare an input (xi,yi) to a chunk (x1,y1) coordinate,
 * we unrotate the input (xi,yi) coordinate so we can compare it to the
 * chunk coordinates normally.
 * If the rotation is 90 or 270, we have to unrotate the (x,y)
 * back the other direction to compensate.
 */
static void UnRotate(int rotate, int *px, int *py)
{
    int fixRot;
    switch (rotate) {
    case 0:
	return;    /* no rotation to fix */
    case 90:
	fixRot = 270;
	break;
    case 180:
    ...
    Tk_TextLayout textLayout;	/* The text layout to be released. */
{
    ...
    layoutPtr = (TextLayout *) textLayout;
    if (layoutPtr != NULL) {
	chunkPtr = &layoutPtr->chunks[0];
	/* Free the rotated text cache, if any. (pat)
	 */
	for (i = layoutPtr->numChunks; i > 0; i--) {
	    Tk_FreeTextCache(chunkPtr->cache);
	    chunkPtr++;
	}
				 ...
				 * < 0 means to draw all characters. */
{
    ...
    LayoutChunk *chunkPtr;
    int textX, textY;   /* coords of chunk of text in this block of text */
	...
	if ((numDisplayChars > 0) && (firstChar < numDisplayChars)) {
	    ...
	    textX = chunkPtr->x + drawX;
	    textY = chunkPtr->y;
	    if (layoutPtr->rotate) {
		/* Pat added: rotate text, if necessary */
		Tk_RotTextXY(0,0,layoutPtr->rotate,&textX,&textY);
		Tk_DrawCharsRotate(display,drawable,gc,layoutPtr->tkfont,
			chunkPtr->start + firstChar, numDisplayChars-firstChar,
			x+textX, y+textY, layoutPtr->rotate,
			& chunkPtr->cache
			);
	    } else {
				 ...
				 * rectangular area, in pixels. */
{
    ...
    UnRotate(layoutPtr->rotate,&left,&top);
    UnRotate(layoutPtr->rotate,&right,&bottom);
    /* Unrotating the coordinates may swap the box top/bottom or left/right,
     * so turn the box back right-side up, if necessary.
     */
    Tk_NormalizeBox(&left,&top,&right,&bottom);
