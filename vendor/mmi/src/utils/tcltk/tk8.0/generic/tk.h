 ...
 *
 * RCS: @(#) $Id: tk.h,v 1.3 1999/02/01 23:00:44 doug Exp $
 */
...
/*
 * TkCanvas->tagTable, Tk_Item2Head, and Tk_Item2 lists form
 * a mapping from tags to items.
 */

typedef struct Tk_Item2  {
    struct Tk_Item2 *nextPtr;	/* list of items for a given tag */
    Tk_Item *itemPtr;		/* points to an item that has tag */
} Tk_Item2;

typedef struct Tk_Item2Header {
    Tk_Item2 *first;		/* first in list of items for given tag */
    Tk_Item2 *last;		/* last in list of items for given tag */
} Tk_Item2Header;
...
EXTERN Tk_TextLayout	Tk_ComputeTextLayout _ANSI_ARGS_((Tk_Font font,
			    ...
			    int *heightPtr));
EXTERN Tk_TextLayout	Tk_ComputeTextLayoutExt _ANSI_ARGS_((
			    Tk_TextLayout textLayout, Tk_Font font,
			    CONST char *string, int numChars, int wrapLength,
			    Tk_Justify justify, int flags, int rotate,
			    int recalc, int *widthPtr, int *heightPtr));
...
EXTERN void		Tk_DrawChars _ANSI_ARGS_((Display *display,
			    ...
			    int y));
EXTERN void		Tk_DrawCharsRotate _ANSI_ARGS_((Display *display,
			    Drawable drawable, GC gc, Tk_Font tkfont,
			    CONST char *source, int numChars, int x,
			    int y, int rot, void **pcache));
EXTERN void		Tk_FreeTextCache _ANSI_ARGS_((void *cache));
