 ...
 *
 * RCS: @(#) $Id: tclUnixPipe.c,v 1.3 2000/01/26 00:00:18 pat Exp $
 */
				 ...
				 * process. */
{
    ...
    /* MMI Mod: You cant use vfork and then reset the signals!
     * vfork runs in the process of the parent!
     * This code caused flex to crash when SIGALRM happened.
     * Must either use fork, or save/restore SIGALRM.
     * 2/2/00: Go back to fork, see if it fixes sue crashes.
     */
#define MMI_USE_VFORK 0
#if MMI_USE_VFORK
    sigaction(SIGALRM,NULL,&sigbuf);
    sighold(SIGALRM);	/* Dont want the signal before we call RestoreSignals */
    /* Also: look at the makefile, which may have -Dvfork=fork.
     */
    pid = vfork();
#else
    pid = fork();
...
#if MMI_USE_VFORK
    /* MMI mod: restore SIGARLM, and then let it happen if it occurred
     * while we were busy.
     */
    sigaction(SIGALRM,&sigbuf,NULL);
    sigrelse(SIGALRM);
#endif
