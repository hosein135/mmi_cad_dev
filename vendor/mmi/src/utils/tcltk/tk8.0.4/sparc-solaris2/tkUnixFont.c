 ...
 *
 * RCS: @(#) $Id: tkUnixFont.c,v 1.4 2000/06/30 16:45:20 pat Exp $
 */
...
/* This routine is called by the generic routines to free
 * the rotated text cache; they know nothing about what is
 * really in the cache, so this routine is located here.
 */
void Tk_FreeTextCache(void *cache)
{
    if (cache) {
	XDestroyImage((XImage*)cache);
    }
}


void
Tk_DrawCharsRotate(display, drawable, gc, tkfont, source, numChars, x, y,
    rot, pcache)
    Display *display;		/* Display on which to draw. */
    Drawable drawable;		/* Window or pixmap in which to draw. */
    ...
    Tk_Font tkfont;		/* Font in which characters will be drawn;
				 * must be the same as font used in GC. */
    CONST char *source;		/* Characters to be displayed. */
    int numChars;		/* Number of characters to display from
				 * string. */
    int x, y;			/* Coordinates at which to draw string. */
    int rot;			/* rotation (0,1,90,180,270: 1 == 270) */
    void **pcache;		/* Pointer to location to cache rotated text. */
{
    ...
    /* new by pat, based on code by LEEEEEE */

    height = fontPtr->font.fm.ascent + fontPtr->font.fm.descent;
    linewidth = XTextWidth(fontPtr->fontStructPtr, source, numChars);
    /* 3-00, pat: If font size is 1 pixel, XTextWdith sometimes returns 0
     * for reasons unknown, which crashes XCreatePixmap.
     */
    ...
    default:
    case 0:
	y -= fontPtr->font.fm.ascent;
	rotwidth = linewidth;
	rotheight = height;
	break;
    case 90:
	x -= fontPtr->font.fm.ascent;
	y -= linewidth;
	rotwidth = height;
	rotheight = linewidth;
	break;
    case 180:
	x -= linewidth;
	y -= fontPtr->font.fm.descent;
	rotwidth = linewidth;
	rotheight = height;
	break;
    case 1:
    case 270:
	x -= fontPtr->font.fm.descent;
	rotwidth = height;
	rotheight = linewidth;
	break;
    }
#define TEST_WITH_RECTANGLES 0
#if TEST_WITH_RECTANGLES
    /* Just place rectangles instead of rotated text, to demonstate
     * that this is what is causing rotated text to be slower than
     * normal text on Sun workstations.
     */
    if (rotBitmap == 0) {
    ...
    XChangeGC(display,bitmapGC,gcMask,&gcValues);
	...
	XDrawImageString(display, bitmap, bitmapGC, 0, fontPtr->font.fm.ascent, 
			 source, numChars);
	...
	*pcache = (void*) dest;
    ...
    /* XFreeGC(display, bitmapGC); No longer freed, ever. */
#endif
}
