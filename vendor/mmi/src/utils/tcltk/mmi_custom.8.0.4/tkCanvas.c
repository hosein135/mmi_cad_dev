 ...
 *
 * RCS: @(#) $Id: tkCanvas.c,v 1.5 1999/02/05 22:54:29 doug Exp $
 */
...
#ifdef TCLC
#undef Tk_CreateTimerHandler
#undef Tk_EventuallyFree
#undef Tk_Release
#undef Tk_CancelIdleCall
#undef Tk_DeleteTimerHandler
#undef Tk_DoWhenIdle
#undef Tk_Preserve
#undef Tk_BackgroundError
#undef Tk_TimerToken
...
typedef struct TagSearch {
    ...
    Tk_Item *lastPtr;		/* The item right before the currentPtr
				 ...
				 * beginning. */
    Tk_Item2 *currentPtr2;	/* Same as above, but for fast tag search */
    Tk_Item2 *lastPtr2;		/* Same as above, but for fast tag search */
    int searchOver;		/* Non-zero means NextItem should always
				 * return NULL. */
    Tk_Item2Header *entryHeader;/* entry in tag->item map */
    Tk_Uid searchAndTag;	/* second tag in tag intersection search */
} TagSearch;
...
static void		DoItem _ANSI_ARGS_((TkCanvas *canvasPtr,
			    Tcl_Interp *interp,
...
/*
 * I don't know how to get the canvas pointer to Tk_CanvasTagsParseProc
 * any other way.
 */
TkCanvas *HackCanvasPtr = NULL;
int dougdebug = 0;
    ...
    char **argv;		/* Argument strings. */
{
    ...
    Tcl_InitHashTable(&canvasPtr->tagTable, TCL_ONE_WORD_KEYS);
    ...
    char **argv;			/* Argument strings. */
{
    ...
    } else if ((c == 'c') && (strncmp(argv[1], "coordsall", length) == 0)
	    && (length >= 2)) {
        /* added by Leeeee */
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    argv[0], " coordsall tag\"",
		    (char *) NULL);
	    goto error;
	}
	for (itemPtr = StartTagSearch(canvasPtr, argv[2], &search);
		itemPtr != NULL; itemPtr = NextItem(&search)) {
	    /* first put in the id of the element */
	    char msg[30];
	    ...
	    result = (*itemPtr->typePtr->coordProc)(interp,
			(Tk_Canvas) canvasPtr, itemPtr, argc-3, argv+3);
	    /* and now a delimeter (cause I can't figure out sublists in C) */
	...
	for (i = 2; i < argc; i++) {
		...
		(*itemPtr->typePtr->deleteProc)((Tk_Canvas) canvasPtr, itemPtr,
			canvasPtr->display);
		Tk_DeleteAllTags(canvasPtr, itemPtr, 1);
	...
	for (itemPtr = StartTagSearch(canvasPtr, argv[2], &search);
		itemPtr != NULL; itemPtr = NextItem(&search)) {
	    Tk_DeleteTag(canvasPtr, itemPtr, tag);
	}
    ...
    char *memPtr;		/* Info about canvas widget. */
{
    ...
    for (tableEntry = Tcl_FirstHashEntry(&canvasPtr->tagTable, &searchPtr);
	tableEntry != NULL;
	tableEntry = Tcl_NextHashEntry(&searchPtr))
    {
	Tk_Item2Header *header = (Tk_Item2Header*)Tcl_GetHashValue(tableEntry);
	if (header != NULL) ckfree(header);
    }
    Tcl_DeleteHashTable(&canvasPtr->tagTable);
...

static Tk_Item2Header*
getTagHeader(TkCanvas *canvasPtr, Tk_Uid tag)
{
    Tcl_HashTable *tablePtr;
    Tcl_HashEntry *entryPtr;
    Tk_Item2Header *header;
    int newPtr;

    if (canvasPtr == NULL) canvasPtr = HackCanvasPtr;
    if (dougdebug && canvasPtr == NULL) fprintf(stderr, "oh nooooo\n");
    tablePtr = &canvasPtr->tagTable;
    entryPtr = Tcl_CreateHashEntry(tablePtr, tag, &newPtr);
    if (newPtr) {
	header = (Tk_Item2Header*)ckalloc(sizeof(Tk_Item2Header));
	Tcl_SetHashValue(entryPtr, (ClientData)header);
	header->first = NULL;
	header->last = NULL;
    } else {
	header = (Tk_Item2Header*)Tcl_GetHashValue(entryPtr);
    }
    return header;
}

 ...
 *	this list.
 *  Note - this above isn't true.  They didn't implement this.
 *  Code done during a search can safely delete the item that was
 *  returned.  If you delete any others, be sure you know what
 *  you're doing.  And if you do any searches within searches,
 *  you better know what you're doing if you delete any items at all.
 *
 *--------------------------------------------------------------
 */
					 ...
					 * will be initialized here. */
{
    ...
    if (0 == strcmp(tag, "turnondougdebug")) dougdebug = 1;
    if (0 == strcmp(tag, "turnoffdougdebug")) dougdebug = 0;
    amp = strchr(tag, '&');
    if (amp != NULL) *amp = '\0';
    searchPtr->tag = uid = Tk_GetUid(tag);
    if (amp != NULL) {
	searchPtr->searchAndTag = Tk_GetUid(amp+1);
	*amp = '&';
    } else {
	searchPtr->searchAndTag = NULL;
    }
    if (uid == allUid) {
    ...
    /*
     * None of the above.  Get first item with a matching tag.
     */

    searchPtr->entryHeader = header = getTagHeader(canvasPtr, uid);
    lastPtr2 = NULL;
    itemPtr2 = header->first;
    if ((uid2 = searchPtr->searchAndTag) != NULL) {
	while (itemPtr2) {
	    int i;
	    int n = itemPtr2->itemPtr->numTags;
	    Tk_Uid *t = itemPtr2->itemPtr->tagPtr;
	    for (i = 0; i < n; i++) {
		if (t[i] == uid2) break;
	    }
	    if (i < n) break;
	    lastPtr2 = itemPtr2;
	    itemPtr2 = lastPtr2->nextPtr;
	}
    }
    if (itemPtr2 == NULL) {
	searchPtr->searchOver = 1;
	searchPtr->currentPtr2 = NULL;
	if (dougdebug) fprintf(stderr, "no tag %d&%d\n", (int)uid,
	    (int)searchPtr->searchAndTag);
	return NULL;
    }
    searchPtr->lastPtr2 = lastPtr2;
    searchPtr->currentPtr2 = itemPtr2;
    if (dougdebug) {
	fprintf(stderr, "first tag %d&%d at item %d\n",
	    (int)uid, (int)searchPtr->searchAndTag, (int)itemPtr2->itemPtr);
    }
    return itemPtr2->itemPtr;
}

/*
 *--------------------------------------------------------------
					 ...
					 * progress. */
{
     ...
     * Must mess about like above because of possibly deleted item.
     */

    lastPtr2 = searchPtr->lastPtr2;
    if (lastPtr2 == NULL) {
	itemPtr2 = searchPtr->entryHeader->first;
    } else {
	itemPtr2 = lastPtr2->nextPtr;
    }
    if ((itemPtr2 == NULL) || (searchPtr->searchOver)) {
	searchPtr->searchOver = 1;
	if (dougdebug) fprintf(stderr, "no more tags\n");
	return NULL;
    }
    if (itemPtr2 != searchPtr->currentPtr2) {
	/*
	 * deleted item stuff like above.
	 */
    } else {
	lastPtr2 = itemPtr2;
	itemPtr2 = lastPtr2->nextPtr;
    ...
    searchPtr->currentPtr2 = itemPtr2;
    if (dougdebug) {
	fprintf(stderr, "next tag %d&%d at item %d\n",
	    (int)uid, (int)searchPtr->searchAndTag, (int)itemPtr2->itemPtr);
    }
    return itemPtr2->itemPtr;
}
...
static void
DoItem(canvasPtr, interp, itemPtr, tag)
    TkCanvas *canvasPtr;		/* canvas we're working on */
					 ...
					 * present for item, or NULL. */
{
    ...
    Tk_AddTag(canvasPtr, itemPtr, tag);
					 ...
					 * up to what's in argc/argv. */
{
	...
	if ((lastPtr != NULL) && (lastPtr->nextPtr != NULL)) {
	    DoItem(canvasPtr, interp, lastPtr->nextPtr, uid);
	}
	...
	for (itemPtr = canvasPtr->firstItemPtr; itemPtr != NULL;
		itemPtr = itemPtr->nextPtr) {
	    DoItem(canvasPtr, interp, itemPtr, uid);
	}
	...
	if (itemPtr->prevPtr != NULL) {
	    DoItem(canvasPtr, interp, itemPtr->prevPtr, uid);
	}
	...
	/* expanded by Leeeee */
	if ((argc < 3) || (((argc-3)/2)*2 != (argc-3))) {
	    Tcl_AppendResult(interp, "wrong # args:  must be \"",
		    cmdName, option, " closest x y -halo halo -start start -tag tag",
		    (char *) NULL);
	    return TCL_ERROR;
	}
	...
	for (i = 3; i < argc; i+=2) {
	  if (strcmp(argv[i],"-halo") == 0) {
	    ...
	    if (Tk_CanvasGetCoord(interp, (Tk_Canvas) canvasPtr, argv[i+1],
				  &halo) != TCL_OK) {
	      return TCL_ERROR;
	    }
	    if (halo < 0.0) {
	     ...
	     */
	    itemPtr = StartTagSearch(canvasPtr, argv[i+1], &search);
	    if (itemPtr != NULL) {
		startPtr = itemPtr;
	    }
	    ...
	    /* unknown switch */
	    Tcl_AppendResult(interp, "unknown switch \"",argv[i],"\"",
			     (char *) NULL);
	    return TCL_ERROR;
	  }
	}
	...
	while (1) {
		...
		if (itemPtr == NULL) {
		  ...
		  if (tag == 1) {
		    DoItem(canvasPtr, interp, closestPtr, uid);
		    return TCL_OK;
		  }
		  itemPtr = canvasPtr->firstItemPtr;
		}
					 ...
					 * MotionNotify. */
{
	...
	if ((itemPtr == canvasPtr->currentItemPtr) && !buttonDown) {
	    Tk_DeleteTag(canvasPtr, itemPtr, currentUid);
	}
	...
	DoItem(canvasPtr, (Tcl_Interp *) NULL, canvasPtr->currentItemPtr,
		currentUid);
	event = canvasPtr->pickEvent;
 ...
 * Tk_DeleteTagFromPos --
 *
 *	Delete a tag in a given position from an item.
 *
 * Results:
 *	None.
 *
 * Side effects:
 *	Tag is deleted.
 *	Parallel tag structure is updated.
 *
 *--------------------------------------------------------------
 */

void
Tk_DeleteTagFromPos(canvasPtr, itemPtr, pos)
    TkCanvas *canvasPtr;		/* canvas we're working on */
    Tk_Item *itemPtr;			/* Item to modify. */
    int pos;				/* position of tag to delete */
{
    Tk_Item2 *item2Ptr, *prev2Ptr;
    Tk_Uid tag = itemPtr->tagPtr[pos];
    Tk_Item2Header *header;

    /*
     * remove the tag from the item->tag mapping
     */

    if (dougdebug) {
	fprintf(stderr, "delete tag %d from item %d\n",
	    (int)itemPtr->tagPtr[pos], (int)itemPtr);
    }
    itemPtr->tagPtr[pos] = itemPtr->tagPtr[itemPtr->numTags-1];
    itemPtr->numTags--;

    /*
     * remove the item from the tag->item mapping
     */
    
    header = getTagHeader(canvasPtr, tag);
    for (item2Ptr = header->first, prev2Ptr = NULL; item2Ptr != NULL;
		prev2Ptr = item2Ptr, item2Ptr = item2Ptr->nextPtr) {
	if (item2Ptr->itemPtr == itemPtr) {
	    /* delete the item */
	    if (item2Ptr == header->first) {
		header->first = item2Ptr->nextPtr;
	    }
	    if (item2Ptr == header->last) {
		header->last = prev2Ptr;
	    }
	    if (prev2Ptr != NULL) {
		prev2Ptr->nextPtr = item2Ptr->nextPtr;
	    }
	    ckfree(item2Ptr);
	    break;
	}
 ...
 */
 ...
 Tk_DeleteTag(canvasPtr, itemPtr, tag)
    TkCanvas *canvasPtr;		/* canvas we're working on */
    Tk_Item *itemPtr;			/* Item to modify. */
    Tk_Uid tag;				/* tag to delete */
{
    int i;

    for (i = itemPtr->numTags-1; i >= 0; i--) {
	if (itemPtr->tagPtr[i] == tag) {
	    Tk_DeleteTagFromPos(canvasPtr, itemPtr, i);
	    break;
	}
    }
 ...
 * Tk_DeleteAllTags --
 *
 *	Delete all the tags of an item.
 *
 * Results:
 *	None.
 *
 * Side effects:
 *	All the tags are deleted.
 *	Dynamic tag space, if present, is freed, if ddts is true.
 *	Parallel tag structure is updated.
 *
 *--------------------------------------------------------------
 */
...
void
Tk_DeleteAllTags(canvasPtr, itemPtr, ddts)
    TkCanvas *canvasPtr;		/* canvas we're working on */
    Tk_Item *itemPtr;			/* Item to modify. */
    int ddts;				/* delete dynamic tag space? */
{
    while (itemPtr->numTags != 0) {
	Tk_DeleteTagFromPos(canvasPtr, itemPtr, 0);
    }
    if (ddts && (itemPtr->tagPtr != itemPtr->staticTagSpace)) {
 ...
 * Tk_AddTag --
 *
 *	Add a tag to an item, at the end of the tag list.
 *
 * Results:
 *	None.
 *
...
Tk_AddTag(canvasPtr, itemPtr, tag)
    TkCanvas *canvasPtr;		/* canvas we're working on */
    Tk_Item *itemPtr;			/* Item to modify. */
    Tk_Uid tag;				/* tag to add */
{
    Tk_Item2 *item2Ptr, *newItem2Ptr;
    Tk_Item2Header *header;
    ...
    if (dougdebug) {
	fprintf(stderr, "add tag %d to item %d\n", (int)tag, (int)itemPtr);
    }
    ...
    /*
     * Add tag to item->tag mapping
     */

    itemPtr->tagPtr[itemPtr->numTags] = tag;
    itemPtr->numTags++;

    /*
     * Add item to tag->item mapping
     */
    
    newItem2Ptr = (Tk_Item2*)ckalloc(sizeof(Tk_Item2));
    newItem2Ptr->itemPtr = itemPtr;
    newItem2Ptr->nextPtr = NULL;

    header = getTagHeader(canvasPtr, tag);
    if (header->first == NULL) {
	header->first = newItem2Ptr;
	header->last = newItem2Ptr;
    } else {
	header->last->nextPtr = newItem2Ptr;
	header->last = newItem2Ptr;
    }
}
