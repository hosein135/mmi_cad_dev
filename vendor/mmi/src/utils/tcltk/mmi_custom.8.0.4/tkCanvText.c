 ...
 *
 * RCS: @(#) $Id: tkCanvText.c,v 1.2 1999/01/13 01:41:51 pat Exp $
 */
...
typedef struct TextItem  {
  ...
  /* added by Leee for rotated text */
    /* For backward compatibility, rotate == 1 means rotate 270 */
    int rotate;         /* rotation: 0(default),90,180,270, or 1(means 270). */
    int baseX, baseY;   /* X,Y coords of upper left corner of text.
			 * It is always the corner nearest the first letter,
			 * but if text is rotated, it is no longer the
			 * upper left corner of the text bbox
			 */
...
static Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_PIXELS, "-rotate", (char *) NULL, (char *) NULL,
	"0", Tk_Offset(TextItem, rotate), 0},
					 ...
					 * x2, y2, ... */
{
	...
	ComputeTextBbox(canvas, textPtr, False);
    } else {
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
	...
	/* This is a change by Lee: set the background font color
	 * to the foreground canvas color.
	 */
	mask = GCForeground|GCFont|GCBackground;
	XGetGCValues(canvasPtr->display, canvasPtr->pixmapGC, mask, &gcValues);
	/* the foreground color of the canvas is our background color */
    ...
    ComputeTextBbox(canvas, textPtr, True);
    return TCL_OK;
}

/*
...
/* Pat added: Rotate (*px,*py) around the origin (x0,y0).
 * This rotation is queer because y increases down the page.
 */
void Tk_RotTextXY(int x0,int y0,int rot,int *px, int *py)
{
    /* translate (px,py) so origin is at text assigned (x,y) location */
    int x = *px - x0, y = *py - y0;
    /* perform rotation */
    switch (rot) {
    case 0:
    ...
    case 90:
	/* *px = -y; *py = x;      What it would be in normal space */
	*px = y; *py = -x;      /* What is when y increases downward */
	break;
    case 180:
	*px = -x;
	*py = -y;
...
/* Pat added: normalize a rectangle so that x1,y1 is the upper left
 * corner and x2,y2 is the lower right.
 */
void Tk_NormalizeBox(int *px1,int *py1,int *px2,int *py2)
{
    int tmp;
    if (*px1 > *px2) {
	tmp = *px1;
...
static void
ComputeTextBbox(canvas, textPtr, recalc)
    Tk_Canvas canvas;			/* Canvas that contains item. */
    TextItem *textPtr;			/* Item whose bbos is to be
					 * recomputed. */
    int recalc;				/* If True, recalc text layout, which
					 * also clears the rotated text cache
					 */
{
    Tk_CanvasTextInfo *textInfoPtr;
    int X, Y, leftX, topY, width, height, fudge;
...
#endif
    textPtr->textLayout = (Tk_TextLayout) Tk_ComputeTextLayoutExt(
	    textPtr->textLayout,
	    textPtr->tkfont, textPtr->text, textPtr->numChars, textPtr->width,
	    textPtr->justify, 0, textPtr->rotate, recalc, &width, &height);
    ...
    X = leftX = (int) (textPtr->x + 0.5);
    Y = topY = (int) (textPtr->y + 0.5);
    ...
    Tk_RotTextXY(X,Y,textPtr->rotate,&textPtr->baseX,&textPtr->baseY);
    Tk_RotTextXY(X,Y,textPtr->rotate,&textPtr->header.x1,&textPtr->header.y1);
    Tk_RotTextXY(X,Y,textPtr->rotate,&textPtr->header.x2,&textPtr->header.y2);

    /* We want to leave the bbox with (x1,y1) the upper left corner.
     * This may not be necessary, but there may be Tk routines
     * somewhere that assume this.
     */
    Tk_NormalizeBox(&textPtr->header.x1,&textPtr->header.y1,
	    &textPtr->header.x2,&textPtr->header.y2);
}

/*
 *--------------------------------------------------------------
					 ...
					 * must be redisplayed (not used). */
{
    ...
    /* Note: rotated text is not used for entries, so this code
     * never has to deal with rotated text. (pat)
     */
	...
	if (Tk_CharBbox(textPtr->textLayout, textPtr->insertPos,
		&x, &y, NULL, &height)) {
	    /* Added by pat: Rotate the x,y into the current rotated
	     * coordinate system of the text box, and offset by baseX,baseY,
	     * which is the start of the text in the rotated system.
	     * Also set cursorWidth and cursorHeight for a vertical
	     * or horizontal cursor.
	     */
	    Tk_RotTextXY(0,0,textPtr->rotate,&x,&y);
	    x += textPtr->baseX;
	    y += textPtr->baseY;
	    switch (textPtr->rotate) {
	    case 0:
		cursorWidth = textInfoPtr->insertWidth;
		cursorHeight = height;
		x -= cursorWidth / 2;
		break;
	    case 90:
		cursorWidth = height;
		cursorHeight = textInfoPtr->insertWidth;
		y -= cursorHeight / 2;
		break;
	    case 180:
		cursorWidth = textInfoPtr->insertWidth;
    ...
    Tk_CanvasDrawableCoords(canvas, (double) textPtr->baseX,
	    (double) textPtr->baseY, &drawableX, &drawableY);
    ...
    double *pointPtr;		/* Pointer to x and y coordinates. */
{
    ...
    return (double) Tk_DistanceToTextLayout(textPtr->textLayout,
	    (int) pointPtr[0] - textPtr->baseX,
	    (int) pointPtr[1] - textPtr->baseY);
}

/*
 *--------------------------------------------------------------
				 ...
				 * area.  */
{
    ...
    return Tk_IntersectTextLayout(textPtr->textLayout,
	    (int) (rectPtr[0] + 0.5) - textPtr->baseX,
	    (int) (rectPtr[1] + 0.5) - textPtr->baseY,
    ...
    int *indexPtr;		/* Where to store converted index. */
{
	...
	*indexPtr = textInfoPtr->selectLast;
	...
	/**** Pat fixed this apparent bug in Tk.  Here is the old code:
	 *    *indexPtr = Tk_PointToChar(textPtr->textLayout,
	 *	x + canvasPtr->scrollX1 - textPtr->baseX,
	 *	y + canvasPtr->scrollY1 - textPtr->baseY);
	 ****/
	*indexPtr = Tk_PointToChar(textPtr->textLayout,
		x - textPtr->baseX,
		y - textPtr->baseY);
					 ...
					 * final Postscript is being created. */
{
	...
	break;
    case 1:      /* Sue backward compatible */
    case 270:
	sprintf(buffer, "270 rotate\n%.15g %.15g [\n", 
	      -Tk_CanvasPsY(canvas, textPtr->y), textPtr->x);
	break;
    case 180:
	/* This is untested, and probably puts the text
	 * in the wrong place (pat)
	 */
	sprintf(buffer, "180 rotate\n%.15g %.15g [\n", 
	      textPtr->x, Tk_CanvasPsY(canvas, textPtr->y));
	break;
    case 90:
	sprintf(buffer, "90 rotate\n%.15g %.15g [\n", 
	      -Tk_CanvasPsY(canvas, textPtr->y), textPtr->x);
	break;
    }
