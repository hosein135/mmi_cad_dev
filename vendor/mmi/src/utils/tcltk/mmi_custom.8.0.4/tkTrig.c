 ...
 *
 * RCS: @(#) $Id: tkTrig.c,v 1.2 2001/05/10 22:40:31 pat Exp $
 */
					 ...
					 * x and y coordinates of point. */
{
    ...
    /* This is an MMI bug fix!  What if the number is negative?? */
    /* tmp = (int) (pointPtr[0] + 0.5); */
    tmp = (int) (pointPtr[0] < 0 ? (pointPtr[0] - 0.5) : (pointPtr[0] + 0.5));
    if (tmp < itemPtr->x1) {
	itemPtr->x1 = tmp;
    }
    if (tmp > itemPtr->x2) {
    ...
    tmp = (int) (pointPtr[1] < 0 ? (pointPtr[1] - 0.5) : (pointPtr[1] + 0.5));
    if (tmp < itemPtr->y1) {
	itemPtr->y1 = tmp;
    }
    if (tmp > itemPtr->y2) {
