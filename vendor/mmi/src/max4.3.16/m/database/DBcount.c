...
void
DBTreeCountPaint(CellDef *def, int (*count) (/* ??? */), int (*hiercount) (/* ??? */), int (*cleanup) (/* ??? */), ClientData cdata)
{
    struct countArg ca;
    int dbCountFunc(CellUse *use, struct countArg *ca), dbCountHierFunc(CellUse *use, struct countArg *ca);
	...
	(void) DBEnumChildren(def, dbCountFunc, (ClientData) &ca);

    /* Now the hiercount */
    ca.ca_hiercount = hiercount;
    (void) DBEnumChildren(def, dbCountHierFunc, (ClientData) &ca);
...
int
dbCountFunc(CellUse *use, struct countArg *ca)
{
    if ((*ca->ca_count)(use->cu_def, ca->ca_cdata) == 0)
	(void) DBEnumChildren(use->cu_def, dbCountFunc, (ClientData) ca);
    return (0);
}

int
dbCountHierFunc(CellUse *use, struct countArg *ca)
{
    int nx, ny;

    (void) DBEnumChildren(use->cu_def, dbCountHierFunc, (ClientData) ca);
    ...
    (*ca->ca_hiercount)(DBCellUseParent(use), 
			nx * ny, 
			use->cu_def, 
			ca->ca_cdata);
    return (0);
