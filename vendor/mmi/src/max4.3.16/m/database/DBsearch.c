                     	...
                     	/* Argument to pass to function */
{
  ...
  */
  ...
  if (!DBReadCell(scx->scx_use->cu_def)) return 0;
...
dbSearchLabelsFunc(register SearchContext *scx, register TreeFilter *fp)
{
    ...
    if(loc)
    {
      ...
      /* label hash uses both location and text now! */
      ASSERT(text,"DBSearchLabelsFunc");
      dummy = DBLabelAlloc(text);

      /* translate location to coords of this def */ 
      GeoInvertTrans(&scx->scx_trans, &inverseTrans);
      GEOTRANSRECT(&inverseTrans, loc, &dummy->lab_rect);
    ...
    {
      for(lab = IHashLookUp(def->cd_labelTextHash,text);
      lab;
	       ...
	       int flags)
{
    ...
    /* if path given, descend it recursively */
    {
      ...
      while(text && (space=strchr(text,' ')))
      {
	CellUse *cu;
	char id[BUFSIZ];
	char *p,*q;

	/* if space present, use exact path only */
	nonRecursive = TRUE;
		      ...
		      
{
    ...
    if(DBIsExpand(use, fp->tf_xmask))
    {
      ...
      if (!DBReadCell(use->cu_def)) goto this;
                   ...
                   int flags)
{
    ...
    if (!DBIsExpand(cellUse, xMask))
	return 0;
    if (!DBReadCell(cellUse->cu_def)) return 0;
