...
void dbChunk2(Plane *pl, TileType type, Rect initial)
{
  fprintf(stderr,"DEBUG dbChunk2 top. type=%s\n",
	  DBTypeLongNameTbl[type]);
  DumpRect("DEBUG dbChunk2, initial =", &initial);

  /* check that initial area is covered by type */
...
void dbChunkInit(void)
{
  ...
  DBNewYank("__DBCHUNK__", &dbChunkUse, &dbChunkDef);
				 ...
				 */
{
     ...
     * region into dbChunkDef.  Then find
    ...
    /* initial region */
    {
      int grow = MnTypicalWireWidth()*10;
      grow = 10; 

      GeoTransRect(&scx->scx_trans,&scx->scx_area,&initialRootArea);
      GEO_EXPAND(&scx->scx_area, grow, &scx->scx_area);
