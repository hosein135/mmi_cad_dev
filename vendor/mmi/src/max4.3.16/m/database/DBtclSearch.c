		 ...
		 TreeContext *cxp)
{
  ...
  TListAppendTPath(interp, p, tpath);
	       ...
	       TerminalPath *tpath)
{
  ...
  TListAppendTPath(interp, pl, tpath);
...
static bool dbSearchUserBBox = FALSE;
	    ...
	    TerminalPath *tpath)
{
  ...
  /* output bbox in rootcell user coords ( = Select cell coords) */
  {
    ...
    if(dbSearchUserBBox)
    {
      bbox0 = DBUserBBoxCellDef(use->cu_def);
    }
    else
    {
      bbox0 = DBBBoxCellDef(use->cu_def);
    }

    GeoTransRect(&scx->scx_trans, bbox0, &bbox);
  ...
  TListAppendTPath(interp, c, tpath);
	 ...
	 TerminalPath *tpath)
{
  ...
  /* output hierarchical instance path from root */
  TListAppendTPath(interp, wl, tpath);
		...
		bool cells)
{
	...
	if(cells && c=='u' && strncmp(*argv,"-user_bbox",length)==0)
        {
	  userBBox = TRUE;
	  argc--; argv++;
...
usage:
    if(cells)
    {
      MsgErrorF("usage:  %s [-cell def_name] [-any_cell] [-area xbot ybot xtop ytop] [-limit n] [-user_bbox]\n",cmdName); 
    }
    else
    {
...
Usage:  db_search_cells [-cell def_name] [-any_cell] [-area xbot ybot xtop ytop] [-limit n] [-user_bbox]
	  ...
	  the direction in which array elements are currently numbered.
       ...
       If -user_bbox user bounding box is given
       (instead of actual bounding box). 
...
Usage:  db_search_labels [-cell def_name] [-non_hier] [-area xbot ybot xtop ytop] [-limit n] [-kind kind] [-no_glob] [glob_pattern] 
			  ...
			  ClientData clientdata)
{
    ...
    /* output path */
    TListAppendTPath(interp, l, tPath);
		      ...
		      char **argv)
{
 ...
 usage:
  MsgErrorF("usage:  %s [-cell def_name] [-non_hier] [-area xbot ybot xtop ytop] [-limit n] [-kind kind] [-no_glob] [glob_pattern]",
	    cmdName); 
  CMD_RETURN(interp);
}
