...
/* copied into non-array cellUE's */
static ArrayInfo dbNullArray = { 0,0,0,0,0,0 };
...
dbUndoLabelForw(register labelUE *up)
{
    ...
    DBChangedArea(dbUndoLastCell, &up->lue_rect, NULL, DBCF_LABEL_ONLY);
...
{
    (void) DBLabelsEraseByContentG(dbUndoLastCell, 
				   &up->lue_rect,
				   up->lue_type, 
				   up->lue_text, 
				   up->lue_group);
...
DBUndoCellUse(register CellUse *use, int action)
{
    ...
    fprintf(stderr,"DEBUG DBUndoCellUse, action=%s subcell=%s parent=%s\n",
	    actions[action], use->cu_def->cd_name, DBCellUseParent(use)->cd_name);
    */

	    
    if (!UndoIsEnabled(DBCellUseParent(use))) return;
    ...
    up->cue_parent = DBCellUseParent(use);
...
dbUndoCellForw(register cellUE *up)
{
    ...
    ***/
	...
	case UNDO_CELL_PLACE:
	    if(up->cue_array.ar_xlo==up->cue_array.ar_xhi &&
	       up->cue_array.ar_ylo==up->cue_array.ar_yhi)
	    {
	      use = DBCellUseNew(up->cue_def, (char *) NULL);
	    }
	    else
	    {
	      use = DBCellUseNewArray(up->cue_def, (char *) NULL);
	    ...
	    DBChangedArea(up->cue_parent, &up->cue_bbox, NULL, DBCF_INSTANCE_ONLY);
