 ...
 * DBLabelNameCheck --
 *
 * Check that a prospective name is legal.
 *
 * Results:
...
bool
DBLabelNameCheck(char *name)
{
  if(!name)
  {
    MsgErrorF("Label name must be nonempty!\n");
    return FALSE;
  }

  return StrCheckChars(name, " ","Label name");
}
...
void DBLabelLink(CellDef *cellDef, Label *lab, int flags) 
{
  ...
  for(dup = (Label *) IHashLookUp(cellDef->cd_labelHash,lab);
      dup;
      dup = IHashLookUpNext(cellDef->cd_labelHash,dup))
  {
    if(dup->lab_group == lab->lab_group)
    {
      FREE(lab);
      return;
    }
  ...
  IHashAdd(cellDef->cd_labelTextHash, lab);
               			...
               			/* label to delete */
{
    ...
    IHashDelete(cellDef->cd_labelTextHash, lab);
    ...
    DBChangedArea(cellDef, &lab->lab_rect, NULL, DBCF_LABEL_ONLY);
    DBFlyLineNotifyLabelChange(cellDef, lab->lab_text);
    FREE((char *) lab);
                                      ...
                                      /* label group */
{
  ...
  /* label hash uses label rect and text */
  dummy = DBLabelAlloc(text);
  dummy->lab_rect = *rect;

  for(lab = IHashLookUp(def->cd_labelHash,dummy);
      lab;
      lab=next)  
  {
    /* get next before deleting lab! */
    next = IHashLookUpNext(def->cd_labelHash,lab);

    if (lab->lab_type == type && 
	lab->lab_group == group)
    {
      DBLabelErase(def,lab);
    }
  }
...
DBLabelsEraseByContent(
		       ...
		       char *text) 		
                                        /* label text */
{
    DBLabelsEraseByContentG(def,rect,type,text,def->cd_activeGroup);
}
...
DBLabelsClear(CellDef *cellDef)
{
  ...
  IHashClear(cellDef->cd_labelTextHash);
  IHashClear(cellDef->cd_labelHash);
}

/* compare label keys (for cd_labelHash) */
int dbLabelKeyEq(void *key1, void *key2)
{
  ...
  result = (GEO_SAMERECT(lab1->lab_rect,lab2->lab_rect) 
	    && strcmp(lab1->lab_text, lab2->lab_text)==0);

  /* DEBUG
  fprintf(stderr,"DEBUG dbLabelKeyEq lab1=%s lab2=%s result=%d\n",
	  lab1->lab_text, lab2->lab_text, result);
  */

  return result;
}

/* hash for label keys (for cd_labelHash) */
int dbLabelKeyHash(void *keyp)
{
  Label *lab = keyp;

  return (IHashStringKeyHash(lab->lab_text) ^ 
	  IHash4WordKeyHash(&lab->lab_rect));
}
	     ...
	     bool noLoad) /* if set don't read in cells */ 
{
	...
	*cp = csave;
	...
	def = scx.scx_use->cu_def;
        GeoTransRect(&scx.scx_trans, &def->cd_userBBox, &r);
	...
	def = scx.scx_use->cu_def;
	tmp.r_xbot = tmp.r_xtop = def->cd_userBBox.r_xbot;
	tmp.r_ybot = def->cd_userBBox.r_ybot;
	tmp.r_ytop = def->cd_userBBox.r_ytop;
	...
	def = scx.scx_use->cu_def;
	tmp.r_xbot = tmp.r_xtop = def->cd_userBBox.r_xtop;
	tmp.r_ybot = def->cd_userBBox.r_ybot;
	tmp.r_ytop = def->cd_userBBox.r_ytop;
	...
	def = scx.scx_use->cu_def;
	tmp.r_xbot = def->cd_userBBox.r_xbot;
	tmp.r_xtop = def->cd_userBBox.r_xtop;
	tmp.r_ybot = tmp.r_ytop = def->cd_userBBox.r_ybot;
	...
	tmp.r_xbot = def->cd_userBBox.r_xbot;
	tmp.r_xtop = def->cd_userBBox.r_xtop;
	tmp.r_ybot = tmp.r_ytop = def->cd_userBBox.r_ytop;
    ...
    for(lab = IHashLookUp(def->cd_labelTextHash,cp);
	lab;
	lab = IHashLookUpNext(def->cd_labelTextHash,lab))
    {
		...
		bool noLoad) /* if set don't read in cells */ 
{
  ...
  /* setup a temporary top level use */ 
  DBCellUseNewTemp(rootDef, &tempUse);

  return DBLabelFindByPathName(&tempUse, name, func, cdarg, noLoad);
}
