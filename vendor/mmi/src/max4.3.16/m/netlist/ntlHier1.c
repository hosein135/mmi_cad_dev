...
/* used to pass info to enum funcs */
 ...
 */
static int 
ntlInst2InstFunc2 (Tile * tile2, HierArg * ha)
{
    Rect r, r1;
 ...
 */
static int
ntlInst2InstFunc1 (SearchContext * scx, HierArg * ha)
{
    Rect  r;
...
/*
MsgInfoF("I2I: use1 = %s, use2 = %s pass2 = %x\n", 
					ha->ha_use1->cu_id, 
					use2->cu_id,
					ha->ha_pass2);
 */
     ...
     */
    if(use2->cu_flags & (ha->ha_pass2 ? CU_NTL_CHECKED2 : CU_NTL_CHECKED))
	return 0;
 ...
 *	calling use (use1)
 *
 * 	Searches the celldef containing use1. 
 *
...
/*
MsgInfoF("marking instance %s of celldef %s as checked\n", 
	    use1->cu_id, 
	    use1->cu_def->cd_name);
*/

    /* no other fields of dummyUse are filled in ! */
    dummyUse.cu_def = DBCellUseParent(use1);
