...
#define ntl_fet_DESC "define a fet device to the netlister"

#define ntl_fet_DOC "
Usage: 
  ntl_fet gate_layer source_drain_layer fet_name substrate_node_name
"


static int
ntlFetCmd(ClientData clientData, 
	  Tcl_Interp *interp, 
	  int argc, char **argv)
{
  char *cmdName;
  ...
  /* gate layer */
  if(!argc) goto usage;
  gateLayer = DBTechNameType(*argv);
  if (gateLayer < 0)
  {
    MsgErrorF("Unknown layer: %s\n", *argv);
    CMD_RETURN(interp);
  }
  if (gateLayer == TT_SPACE)
  {
    MsgErrorF("%s gate layer must not be 'space'!\n", cmdName);
    CMD_RETURN(interp);
  }
  argc--; argv++;
  ...
  if (sdLayer == TT_SPACE)
  {
    MsgErrorF("%s source/drain layer must not be 'space'!\n", cmdName);
    CMD_RETURN(interp);
  }
  argc--; argv++;
  ...
  /* substrate node */
  if(!argc) goto usage;
  substrateNodeName = *argv;
  argc--; argv++;

  /* should be no args left */
  if(argc>0) goto usage;

  ntlTransDefineFet(gateLayer,
		    sdLayer,
		    fetName,
		    substrateNodeName);
  CMD_RETURN(interp);
 ...
 usage:
  MsgErrorF("usage:  %s gate_layer sd_layer fet_name sub_node_name\n", 
	    cmdName);
  CMD_RETURN(interp);
}
	      ...
	      int argc, char **argv)
{
  ...
  {
    CellUse dummyUse;
    DBCellUseNewTemp(def, &dummyUse); 
    ntlMnNetlist(&dummyUse, (char *) NULL);
  }
...
{
  MnDocCreateCommand(interp, "ntl_fet", ntlFetCmd,
  ...
  MnDocLinkVar(interp, "NTL_USE_GLOBALS", 
	       ...
	       "controls output of .global lines by netlister",
	       "
...
");
  ...
  MnDocLinkVar(interp, "NTL_OUTPUT_TOPLEVEL_AS_SUBCKT", 
	       (char *) &ntlOutputToplevelAsSubckt, TCL_LINK_BOOLEAN,
	       "controls netlister output of toplevel def",
	       "
If reset (the default) the .SUBCKT and coresponding .ENDS lines for the
top level cell are output as comments.
");

  MnDocLinkVar(interp, "NTL_NO_IMPLICIT_PORTS", 
	       (char *) &ntlNoImplicitPorts, TCL_LINK_BOOLEAN,
	       "controls generation of implicit ports by netlister",
	       "
If reset (the default) the netlister will generate implicit ports
where ever connections between cells are not marked by explicit port labels.

If set, the netlister will generate warnings for all connections between
cells that do not occur at ports, and will NOT represent these connections
in its output.
");

  MnDocLinkVar(interp, "NTL_VERBOSE",
	       (char *) &ntlVerbose, TCL_LINK_BOOLEAN,
	       "if set, the netlister will be chatty.",
	       NULL);

  MnDocLinkVar(interp, "NTL_REPORT_BAD_DEVICES", 
	       (char *) &ntlReportBadDevices, TCL_LINK_BOOLEAN,
	       "if set, netlister warns on badly formed devices",
	       NULL);

  MnDocLinkVar(interp, "NTL_REPORT_SPLIT_NETS", 
	       (char *) &ntlReportSplitNets, TCL_LINK_BOOLEAN,
	       "if set, netlister warns when two nodes have the same name",
	       NULL);
}
