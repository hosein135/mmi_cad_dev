 ...
 * ie, points to MINFINITY, we skip the label.
 *
...
/* MsgInfoF( "ntlLable nodes: labels kind = %x, text = %s\n", lab->lab_kind, lab->lab_text); */
	...
	for (quad = 0; quad < 4; quad++)
	{
	    ...
	    if (ntlConnect(DBgetTileType(tp), lab->lab_type)
		&& ntlHasRegion(tp, (ClientData) MINFINITY))
	    {
	...
	for(poly = def->cd_polygons; poly != (Polygon *) NULL; 
		poly = poly->poly_next)
	    if( ntlConnect(poly->poly_type, lab->lab_type)
		&& poly->poly_client != (ClientData) MINFINITY
