...
void ntlSearchQueue(CellDef * def)
{
	...
	mask = &ntlConnectTbl[current->cq_type];
	...
	mask       = &ntlConnectTbl[ntlCurPoly->poly_type];
 ...
 */
static int 
ntlMarkTileFunc(Tile * tile, CellDef * def  )
{
    ConnectQueue * new;
    Rect area;
    TileType type;
...
static int 
ntlMarkPoly(Polygon * poly, CellDef * def  )
{
...
/* MsgInfoF("marking nodes of celldef %s \n", def->cd_name);  */
    ...
    /* all cells should already be in memory */
    ASSERT(def->cd_flags&CD_AVAILABLE,"ntlFindNodes");
