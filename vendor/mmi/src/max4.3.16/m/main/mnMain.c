...
mnGetEnvPath(char *name, char **resultp)
{
  ...
  *resultp = getenv(name);
  ...
  { 
    ...
    if(!PaTildeExpand(*resultp,buf,BUFSIZ))
    {
      fprintf(stderr,
	      "Max: Fatal Error, "
...
mnInitLibPaths()
{
  ...
  else
  {
    ...
    if(!PaTildeExpand(buf,buf2,BUFSIZ))
    {
      fprintf(stderr,
...
mnInitTcl(Tcl_Interp *interp)
{
  /* set tcl env variable to mmi library location */
  {
    ...
    /* try mmi_private first */
    sprintf(buf, "%s/lib/tcl%s", MnMMIPrivate, TCL_VERSION);
    sprintf(buf2,"%s/init.tcl", buf);
    fp = fopen(buf2,"r");
    if(fp) goto found;
    
    /* next try mmi_local */
    sprintf(buf, "%s/lib/tcl%s", MnMMILocal, TCL_VERSION);
    sprintf(buf2,"%s/init.tcl", buf);
    fp = fopen(buf2,"r");
    if(fp) goto found;

    /* finally default to mmi */ 
    sprintf(buf, "%s/lib/tcl%s", MnMMITools, TCL_VERSION);
    sprintf(buf2,"%s/init.tcl", buf);
    fp = fopen(buf2,"r");
    if(fp) goto found;

    /* no viable library found, complain and exit */
    fprintf(stderr,
...
be set to the null string");
  ...
  MnDocLinkVar(interp, "MAX_DEVELOPER", (char *) &MnDeveloper, 
	       ...
	       "
Max developer mode requires maxDev license, and can only be entered at startup.
...
mnInitTk(Tcl_Interp *interp)
{
  ...
  /* set tk env variable to mmi library location */
  {
    ...
    /* try mmi_private first */
    sprintf(buf, "%s/lib/tk%s", MnMMIPrivate, TK_VERSION);
    sprintf(buf2,"%s/tk.tcl", buf);
    fp = fopen(buf2,"r");
    if(fp) goto found;
    
    /* next try mmi_local */
    sprintf(buf, "%s/lib/tk%s", MnMMILocal, TK_VERSION);
    sprintf(buf2,"%s/tk.tcl", buf);
    fp = fopen(buf2,"r");
    if(fp) goto found;
...
#ifdef HIDE
    /* Determine technology, and load tech file */
    {
      char *techName = mnCmdLineTechnology;
      char *envTechName = getenv("MAX_DEFAULT_TECH");

      /* if no cmd line switch, try getting from last .max file on cmd line */
      if(!techName && lastFileArg)
      {
	techName = DBGetTech(lastFileArg);

	/* if file tech matches MAX_DEFAULT_TECH, and
	 * MAX_DEFAULT_TECH includes a variation part,
	 * use it!
	 *
	 * e.g if MAX_DEFAULT_TECH is 'scmos-foo' and file
	 * has tech scmos, use technolyg 'scmos-foo'.
	 */
	if(techName && envTechName && strchr(envTechName,'-'))
	{
	  char *envTech;
	  char *end;

	  /* split of tech part of env string */
	  envTech = StrDup(NULL, envTechName);
	  end = strchr(envTech,'-');
 	  *end = '\0';

	  if(strcmp(techName,envTech)==0) techName = envTechName;
	}
      }

      /* if still no techName, try  environment variable */
      if(!techName) techName = getenv("MAX_DEFAULT_TECH");

      /* If still no techName, default to mmi25 */
      if (!techName) techName = "mmi25";

      /* Split out tech and var parts */
      MnTech = StrDup(NULL, techName);
      MnTechVar = strchr(MnTech,'-');
      if(MnTechVar)
      {
	*MnTechVar = '\0';
	MnTechVar++;
      ...
      MnDocSetVar(MnInterp, "MN_TECH", MnTech, TCL_GLOBAL_ONLY,
		  "Technology name (without variation part)",
		  NULL);
      MnDocSetVar(MnInterp, "MN_TECH_VAR", MnTechVar?MnTechVar:"", 
		  TCL_GLOBAL_ONLY,
		  "Technology variation (if any)",
		  NULL);

      /* and finally load the technology! */
