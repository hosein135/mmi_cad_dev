				 ...
				 */
{
    ...
    newUse = DBCellUseNewCopy(selUse, realUse->cu_id, &newTrans);
                         	...
                         	/* Array characteristics desired. */
{
    ...
    newUse = DBCellUseNewArray(use->cu_def, (char *) use->cu_id);
				 ...
				 */
{
    ...
    /* If null case, just return */
    if(arrayInfo->ar_xlo == arrayInfo->ar_xhi && 
       arrayInfo->ar_ylo == arrayInfo->ar_yhi) return;
	      ...
	      bool group)  /* if set, don't erase or strectch material not in active group. */
{
	...
	FREE_TAG(selStretchList,"StrectchArea");
