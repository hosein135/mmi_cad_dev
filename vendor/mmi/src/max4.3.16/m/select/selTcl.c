...
-glob 
    treat last part of name as a glob pattern.
    selects all matching uses.  (Not yet implemented.)
...
/* handle glob case for selTclCmdCell */
static void
selTclCellGlob(CellUse *rootUse, char *id, bool less)
{
  MsgErrorF("sel_cell -glob option, not yet implemented.\n");
}
...
selTclCmdCell(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
{
  ...
  while(argc>0 && **argv=='-')
  {
    ...
    if(c=='g' && strncmp(*argv,"-glob",length)==0)
    {
      argc--; argv++;
      glob = TRUE;
      continue;
  ...
  /* id */
  if(argc==0) goto usage;
  if(!DBInstanceParsePath(*argv,id)) goto usage; 
  argc--;argv++;
  ...
  /* treat glob case separately */
  if(glob)
  {
    selTclCellGlob(w->lay_rootUse, id, less);
    CMD_RETURN(interp);
  }

  /* non glob case */
  bzero(&scx, sizeof(SearchContext));
  DBCellUseFindByPathName(id, 
			  w->lay_rootUse,
			  &scx,  /* fill in with use */
			  TRUE,  /* array subscripts optional */
			  FALSE); /* load cells dynamically
   ...
   * array as a whole, subtract off for the index of
   * the element.
   */
  {
    Transform tmp;
    
    GeoInvertTrans(DBGetArrayTransform(scx.scx_use, scx.scx_x, scx.scx_y), 
		   &tmp);
...
selTclCmdPolygons(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
{
  ...
  LayChangedDefSelection(SelectRootDef, &selPoly->poly_bbox, TRUE);
...
selTclCmdSave(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
{
  ...
  DBWriteCell(SelectDef, fileName, DBSuffix);
