...
usage: sel_what_cells [-boolean] [-edit_only non_edit_found_var_name] [-limit n] [-user_bbox]
...
If -user_bbox, gives user_bbox (rather than actual bbox). 
			...
			Tcl_Interp *interp)
{
  ...
  /* output bbox in rootcell user coords ( = Select cell coords) */
  {
    ...
    if(selTclUserBBox)
    {
      Rect *bbox0 = DBUserBBoxCellDef(selUse->cu_def);
      GeoTransRect(trans, bbox0,&bbox);
    }
    else
    {
      bbox = *DBBBoxCellUseNoUp(selUse);
    }
...
selTclCmdWhatCellsL(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
{
  ...
  while(argc>0 && **argv=='-')
  {
    ...
    if(c=='u' && strncmp(*argv,"-user_bbox",length)==0)
    {
      argc--; argv++;
      userBBox = TRUE;
      continue;
 ...
 usage:
  MsgErrorF("usage:  %s"
	    " [-boolean] [-edit_only non_edit_found_var_name] [-limit n] [-user_bbox]\n",
	    cmdName);
  CMD_RETURN(interp);
}
		    ...
		    char **argv)
{
  ...
  DBUpdate(SelectDef);
  if(!DBVStampSame(&SelectDef->cd_version,&layersStamp))
  {
    layersStamp = SelectDef->cd_version;
