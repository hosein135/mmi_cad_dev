                      		...
                      		/* Describes the current search. */
{
	...
	if ((*arg->sea_func)(&rootRect, arg->sea_type, arg->sea_cdarg) != 0)
	    return 1;
	FREE_TAG(arg->sea_rectList,"LinkedRect");
...

{
	...
	/* set up scx */
	DBCellUseNewTemp(SelectRootDef, &dummy); 
	scx.scx_use = &dummy;
...
selEnumCMatch(struct searg *arg, SearchContext *scx, CellUse *use)
{
  ...
  if(arg->sea_tPath)
  {
    ...
    s=tp->tp_first;
    for(s=tp->tp_first; s<tp->tp_next; s++)
    {
      *tpf->tp_next++ = *s;
    }
    *tpf->tp_next = '\0';
                      		...
                      		/* Describes what we're looking for. */
{
    ...
    {
      if(!DBIsArray(selUse) ||
	 use->cu_array.ar_xlo != selUse->cu_array.ar_xlo ||
    ...
    {
      if(DBIsArray(selUse)) goto checkChildren;
    }
    ...
    if (DBCellUseParent(use) == EditCellUse->cu_def)
    {
    ...
    if (DBCellUseParent(arg->sea_foundUse) == EditCellUse->cu_def)
    {
    ...
    if(!(use->cu_def->cd_flags & CD_AVAILABLE)) return 0;

    /* If editOnly, no point descending past edit cell */
    if(arg->sea_editOnly && 
       DBCellUseParent(use) == EditCellUse->cu_def) return 0;
                      		...
                      		/* Describes our search. */
{
	...
	/* get selection root use */
	parent = LayCurWindow()->lay_rootUse;

	if (parent == NULL || parent->cu_def != SelectRootDef)
	{
     ...
     */
    {
      ...
      scx2.scx_use = DBCellUseNewTemp(SelectRootDef, &dummy);
	...
	goto keepLooking;
       ...
       */
      if ((arg->sea_editOnly)
	  && (DBCellUseParent(arg->sea_foundUse) != EditCellUse->cu_def))
      {
	if (arg->sea_nonEdit != NULL) *(arg->sea_nonEdit) = TRUE;
	goto keepLooking;
      }
