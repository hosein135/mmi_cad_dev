...
extern bool StrCheckChars(char *string, char *illegal, char *msg); 
...
static __inline__ int UtlsStatMallocMem(int size) 
{
  ...
  */
  ...
  /* actually single word aligned */
  result = ((size + 3)/4)*4;

  /* two word overhead per malloc */
  /* result += 8;  no longer, using Jays memory manager */
