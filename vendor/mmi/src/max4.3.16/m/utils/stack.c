...
Stack *
StackNew(int sincr)
              		/* Number of entries by which to grow storage area */
{
    Stack *stack;
 ...
 */
void
StackPush(ClientData arg, Stack *stack)
{
 ...
 */
void
StackEnum(Stack *stack, int (*func) (/* ??? */), ClientData cd)
{
    int i, j;
    struct stackBody * sb;
 ...
 */
void
StackCopy(Stack *src, Stack **dest, int copystr)
{
    int stackCopyFn(ClientData stackItem, int i, ClientData cd);
...
/*ARGSUSED*/
int
stackCopyFn(ClientData stackItem, int i, ClientData cd)
{
    if(stackCopyStr)
