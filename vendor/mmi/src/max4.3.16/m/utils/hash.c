...
#ifndef lint
static char rcsid[] = "$Header: hash.c,v 6.0 90/08/28 19:00:47 mayo Exp $";
#endif  not lint
...
void
HashInitClient(HashTable *table, 
...
int
hash(register HashTable *table, register char *key)
{
...
HashEntry *
HashLookOnly(register HashTable *table, char *key)
                              	/* Hash table to search. */
              			/* Interpreted according to table->ht_ptrKeys
...
HashEntry *
HashFind(register HashTable *table, char *key)
                              	/* Hash table to search. */
              			/* Interpreted according to table->ht_ptrKeys
				 * as described in HashInit()'s comments.
				 */
...
next:
	...
	case HT_STRINGKEYS:
	    MALLOC_TAG(HashEntry *, 
		       h, 
		       (unsigned) sizeof(HashEntry)+strlen(key)-3,
		       "HashEntry (HT_STRINGKEYS)");
	...
	case HT_CLIENTKEYS:
		...
		MALLOC_TAG(HashEntry *, 
			   h, 
			   (unsigned) sizeof (HashEntry),
			   "HashEntry (HT_CLIENTKEYS)");
...
void
HashStartSearch(register HashSearch *hs)
...
HashEntry *
HashNext(register HashTable *table, register HashSearch *hs)
                              	/* Table to be searched. */
                            	/* Area used to keep state about search. */
{
    register HashEntry *h;
...
void
HashKill(HashTable *table)
                     	/* Hash table whose space is to be freed */
{
