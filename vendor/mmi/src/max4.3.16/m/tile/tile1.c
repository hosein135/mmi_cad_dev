 ...
 * TiStats --
 *
 * get statistics on tiles.
 *
 * returns total memory usage for tiles (including free list)
 *
 * --------------------------------------------------------------------
 */

int TiStats(int *allocs, 
	    int *frees, 
	    unsigned long *allocedMem, 
	    unsigned long *freeMem)
{
  int numFree = 0;
  Tile *ti;
  int tileSize = UtlsStatMallocMem(sizeof(Tile));

  for(ti=TileFreeList; ti; ti=ti->ti_client) numFree++;

  if(allocs) *allocs = TilesAlloced;
  if(frees)  *frees = TilesFreed;
  if(freeMem) *freeMem = numFree*tileSize;
  if(allocedMem) *allocedMem = (TilesAlloced-TilesFreed)*tileSize;

  return UtlsStatMallocMem(sizeof(Tile))*(TilesAlloced-TilesFreed+numFree);
}
