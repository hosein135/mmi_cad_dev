...
#ifndef lint
static char rcsid[] = "$Header: ExtTimes.c,v 1.2 91/04/01 21:57:44 mayo Exp $";
#endif  not lint
...
struct cellStats *extGetStats(CellDef *def);
...
ExtTimes(CellUse *rootUse, FILE *f)
{
    double clip, inter;
    HashSearch hs;
    HashEntry *he;

    /* pre load all cells rooted at rootUse */
     ...
     */
    MsgInfoF("Computing individual cell statistics:\n"); 
    HashStartSearch(&hs);
     ...
     * time (children).
     */
    MsgInfoF("Computing hierarchical and incremental statistics:\n"); 
    HashStartSearch(&hs);
    while (he = HashNext(&cellStatsTable, &hs))
     ...
     */
    MsgInfoF("Computing summary statistics:\n"); 
    HashStartSearch(&hs);
    while (he = HashNext(&cellStatsTable, &hs))
    {
	extTimesSummaryFunc((struct cellStats *) HashGetValue(he), f);
	FREE_TAG(HashGetValue(he),"cellStats");
    }
...
extTimesInitFunc(CellUse *use)
{
    ...
    /* Visit our children */
    (void) DBEnumChildren(def, extTimesInitFunc, (ClientData) 0);
    return (0);
}

/*
...
extTimesCellFunc(struct cellStats *cs)
{
    extern long extSubtreeTotalArea;
    extern long extSubtreeInteractionArea;
    extern long extSubtreeClippedArea;
    int extCountTiles(Tile *tile, struct cellStats *cs), extPaintOnly(CellDef *def), extHierCell(CellDef *def);
    TransRegion *transList, *tl;
    CellDef *def = cs->cs_def;
    int pNum;

    MsgInfoF("Processing %s\n", def->cd_name); 
	...
	(void) DBPlaneEnumAreaPaint((Tile *) NULL, def->cd_planes[pNum], &TiPlaneRect,
 ...
 * Called via DBPlaneEnumAreaPaint by extTimesCellFunc() above.
 *
 * Results:
 *	Returns 0 always.
 *
 ...
 */
    ...
    /*ARGSUSED*/
extCountTiles(Tile *tile, struct cellStats *cs)
               			/* UNUSED */
...
extTimesIncrFunc(struct cellStats *cs)
{
    /*
...
extTimesSummaryFunc(struct cellStats *cs, FILE *f)
{
...
extTimesParentFunc(CellDef *def, struct cellStats *cs)
{
    ...
    for(cp = def->cd_pars; cp; cp = cp->cp_next) 
      extTimesParentFunc(cp->cp_def, cs);
}

/*
 * ----------------------------------------------------------------------------
...
int
extTimesHierFunc(CellDef *def, struct cellStats *cs)
{
    int extTimesHierUse(CellUse *use, struct cellStats *cs);
    struct cellStats *csForDef;
    ...
    /* Visit our children */
    (void) DBEnumChildren(def, extTimesHierUse, (ClientData) cs);
    return (0);
}

extTimesHierUse(CellUse *use, struct cellStats *cs)
{
...
extTimesFlatFunc(CellDef *def, struct cellStats *cs)
{
    struct cellStats *csForDef;
    int extTimesFlatUse(CellUse *use, struct cellStats *cs);
    ...
    /* Visit our children */
    (void) DBEnumChildren(def, extTimesFlatUse, (ClientData) cs);
    return (0);
}

extTimesFlatUse(CellUse *use, struct cellStats *cs)
{
...
extTimeProc(int (*proc) (/* ??? */), CellDef *def, struct timeval *tv)
{
    int secs, usecs, i;
...
extCumInit(struct cumStats *cum)
{
...
extCumOutput(char *str, struct cumStats *cum, FILE *f)
              			/* Prefix string */
...
extCumAdd(struct cumStats *cum, double v)
{
...
ExtInterCount(CellUse *rootUse, int halo, FILE *f)
{
    double inter;

    /* preload cell tree root at rootUse */
    DBReadCellTree(rootUse->cu_def);
...
extInterAreaFunc(CellUse *use, FILE *f)
{
    static Plane *interPlane = (Plane *) NULL;
    CellDef *def = use->cu_def;
    int extInterCountFunc(Tile *tile, int *pArea);
    int area, interarea;
    double pctinter;
	...
	*  (def->cd_bbox.r_ytop - def->cd_bbox.r_ybot);
    ...
    /* Visit our children */
    (void) DBEnumChildren(def, extInterAreaFunc, (ClientData) f);
    return (0);
}

int
extInterCountFunc(Tile *tile, int *pArea)
{
    Rect r;
