...
#ifndef lint
static char rcsid[] = "$Header: ExtTest.c,v 1.4 91/04/01 21:57:37 mayo Exp $";
#endif  not lint
 ...
 */
void
ExtractTest(Layout *w, int argc, char *argv[])
{
	...
	case STEP:
	    ...
	    if (argc > 2)
	    {
	      MsgWarnF("To control extractor stepsize, see tcl variable"
		       " EXT_STEP_SIZE\n");
	    }
	    break;
    }
...
extShowTech(char *name)
{
	...
	for (s = 0; s < DBNumTypes; s++)
		...
		(void) fprintf(out, "    %-8.8s %-8.8s %8lf\n",
...
extShowEdge(char *s, Boundary *bp)
{
    ...
    int style = STYLE_FEEDBACK_SOLID;
...
/*    TODO move this to layout? can't do graphics outside of layout module!  */
    ...
    layRectToWindow(extDebugWindow, &edgeRect, &extScreenRect); 
...
#ifdef HIDEIT
    Rect tileRect;
    static int styles[] = { STYLE_FEEDBACK_PALE, STYLE_FEEDBACK_SOLID };
...
/* TODO remove can't do graphics outside of layout module! */
    ...
    layRectToWindow(extDebugWindow, r, &extScreenRect);
    if (DebugIsSet(extDebugID, extDebVisOnly))
    {
	Rect rclip;
...
extMore(void)
{
    ...
    fprintf(stderr,"extMore():  TODO read input from user!\n");
}
