...
#ifndef lint
static char rcsid[] = "$Header: ExtArray.c,v 1.3 90/04/02 17:38:42 mayo Exp $";
#endif  not lint
...
extern int extHardProc(SearchContext *scx, HardWay *arg);
...
char *extArrayRange(register char *dstp, register int lo, register int hi, int prevRange, int followRange);
char *extArrayTileToNode(Tile *tp, ExtTree *et, HierExtractArg *ha, int doHard);
LabRegion *extArrayHardNode(Tile *tp, CellDef *def, HierExtractArg *ha);
char *extArrayNodeName(NodeRegion *np, HierExtractArg *ha, ExtTree *et1, ExtTree *et2);

char *
extArrayNodeName(NodeRegion *np, HierExtractArg *ha, ExtTree *et1, ExtTree *et2)
{
    register Tile *tp;

    NODETOTILE(np, et1, tp);	/* tp = extHierLookTile(np, et1); */
    if (tp && DBgetTileType(tp) != TT_SPACE && extHasRegion(tp, extUnInit))
...
char *
extArrayTileToNode(Tile *tp, ExtTree *et, HierExtractArg *ha, int doHard)
             
                
                       
                	/* If TRUE, we look up this node's name the hard way
			 ...
			 */
{
    ...
    if(DBIsArray)
    {
      hasX = (use->cu_xlo != use->cu_xhi);
      hasY = (use->cu_ylo != use->cu_yhi);
    }
	...
	LayFeedbackAdd(&r, errorStr, ha->ha_parentUse->cu_def, 1,
				STYLE_FEEDBACK_MEDIUM);
    return ("(none)");
...
char *
extArrayRange(register char *dstp, register int lo, register int hi, int prevRange, int followRange)
                        
                    
                   	/* TRUE if preceded by a range */
                     	/* TRUE if followed by a range */
{
    if (!prevRange) *dstp++ = '[';
    if (hi < lo)
...
void extArrayHardSearch(CellDef *def, 
			HardWay *arg, 
			SearchContext *scx, 
			int (*proc) (/* ??? */))
{
...
LabRegion *
extArrayHardNode(Tile *tp, CellDef *def, HierExtractArg *ha)
{
    TileType type = DBgetTileType(tp);
    char labelBuf[4096];
    SearchContext scx;
    HardWay arg;
	...
	DBLabelLink(def, arg.hw_label, DBLL_PREPEND); 
	return (lreg);
    }
...
void
extArrayAdjust(HierExtractArg *ha, ExtTree *et1, ExtTree *et2)
{
 ...
 * extArrayPrimaryFunc --
 *
 * Called by DBEnumArrayElements, which should only find a single array element.
 ...
 *
 * Results:
 *	Returns 1 to cause DBEnumArrayElements to abort and return 1 itself.
 *	This is so the caller of DBEnumArrayElements can tell whether or not
...
int
extArrayPrimaryFunc(CellUse *use, Transform *trans, int x, int y, HierExtractArg *ha)
                 	/* Use of which this is an array element */
                     	/* Transform from coordinates of use->cu_def to those
			 * in parent, for the array element (x, y).
			 */
             		/* X, Y indices of this array element */
 ...
 * extArrayInterFunc --
 *
 * Called by DBEnumArrayElements, which should find all array elements inside
 ...
 *
 * Results:
 *	Returns 0 to cause DBEnumArrayElements to continue.
 *
 * Side effects:
...
int extArrayInterFunc(CellUse *use, 
                 	/* Use of which this is an array element */
		      Transform *trans, 
                     	/* Transform from use->cu_def to parent
			 * coordinates, for the array element (x, y).
			 */
		      int x, 
		      int y, 
             		/* X, Y of this array element in use->cu_def coords */
		      HierExtractArg *ha)
{
...
Void
extArrayProcess(HierExtractArg *ha, Rect *primary)
                       
                  	/* Area guaranteed to contain only the primary
			 ...
			 */
{
    ...
    if (DBEnumArrayElements(use, primary, extArrayPrimaryFunc, (ClientData) ha) == 0)
    {
	LayFeedbackAdd(primary,
     ...
     */
    (void) DBEnumArrayElements(use, &ha->ha_interArea, extArrayInterFunc, (ClientData)ha);
...
static int
extArrayFunc(SearchContext *scx, HierExtractArg *ha)
                       	/* Describes first element of array */
                       	/* Extraction context */
{
     ...
     * and top-right.
     */
    primary.r_xbot = bbox->r_xbot;
    primary.r_xtop = bbox->r_xbot + 1;
    primary.r_ybot = bbox->r_ybot;
    primary.r_ytop = bbox->r_ybot + 1;
    ha->ha_subUse = use;

    /* Area A */
    if (ysep <= ysize)
    {
	ha->ha_clipArea.r_xbot = bbox->r_xbot;
	ha->ha_clipArea.r_xtop = bbox->r_xbot + xsize + halo;
	ha->ha_clipArea.r_ybot = bbox->r_ybot + ysep - halo;
	ha->ha_clipArea.r_ytop = bbox->r_ybot + ysize + halo;
    ...
    if (xsep <= xsize)
    {
	ha->ha_clipArea.r_xbot = bbox->r_xbot + xsep - halo;
	ha->ha_clipArea.r_xtop = bbox->r_xbot + xsize + halo;
	ha->ha_clipArea.r_ybot = bbox->r_ybot;
	ha->ha_clipArea.r_ytop = bbox->r_ybot + ysize + halo;
	ha->ha_interArea = ha->ha_clipArea;
	 ...
	 */
	primary.r_ybot = bbox->r_ytop - 1;
	primary.r_ytop = bbox->r_ytop;
	ha->ha_clipArea.r_xbot = bbox->r_xbot + xsep - halo;
	ha->ha_clipArea.r_xtop = bbox->r_xbot + xsize + halo;
	ha->ha_clipArea.r_ybot = bbox->r_ytop - ysize - halo;
	ha->ha_clipArea.r_ytop = bbox->r_ytop - ysep + halo;
	ha->ha_interArea = ha->ha_clipArea;
...
Void
extArray(CellUse *parentUse, FILE *f)
{
    SearchContext scx;
    HierExtractArg ha;
    ...
    scx.scx_trans = GeoIdentityTransform;
    scx.scx_area = TiPlaneRect;
    (void) DBSrChildren(&scx, extArrayFunc, (ClientData) &ha);
