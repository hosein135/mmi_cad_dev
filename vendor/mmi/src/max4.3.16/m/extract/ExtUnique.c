...
#ifndef lint
static char rcsid[] = "$Header: ExtUnique.c,v 1.2 91/05/06 17:08:03 mayo Exp $";
#endif  not lint
 ...
 *
 * Side effects:
 *	May modify the label list of def, and mark def as CD_MODIFIED.
...
int
extUniqueCell(CellDef *def, int allNames)
{
    ...
    if (nwarn)
	MsgErrorF("%s: %d warnings\n", def->cd_name, nwarn);
    return (nwarn);
}

int
extMakeUnique(CellDef *def, LabelList *ll, LabRegion *lreg, LabRegion *lregList, HashTable *labelHash, int allNames)
{
    static char *badmesg =
	...
	for (ll2 = lp2->lreg_labels; ll2; ll2 = ll2->ll_next)
	{
		...
		LayFeedbackAdd(&r, message, def, 1, STYLE_FEEDBACK_MEDIUM);
	    }
	}
    }
    return nwarn;
