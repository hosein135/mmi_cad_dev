...
ExtInit()
{
	...
	*(debugFlags[n].di_id) =
    ...
    DBNewYank("__EXTRACT__", &extYuseCum, &extYdefCum);

    /* Create the dummy use also used in hierarchical extraction */
    extParentUse = DBCellUseNewTop(extYdefCum, (char *) NULL);
...
extDefPushFunc(CellUse *use)
{
    ...
    if (def->cd_client || (def->cd_flags&CD_INTERNAL))
	return (0);
 ...
 *	at rootUse->cu_def, and mark the cells as CD_MODIFIED.
...
extDefParentAreaFunc(CellDef *def, CellDef *baseDef, CellUse *allButUse, Rect *area)
{
    ...
    {
      CellKid *kid;
      for(kid = cp->cp_def->cd_kids; kid; kid->ck_next) 
      {
	CellUse *parent;
	for(parent = kid->ck_uses; parent; parent = parent->cu_next) 
	{
	  int xlo = 0;
	  int xhi = 0;
	  int xsep = 0;
	  ...
	  for (x = xlo; x <= xhi; x++)
	  {
	    for (y = ylo; y <= yhi; y++)
	    {
	      ...
	      extDefParentAreaFunc(DBCellUseParent(parent), 
				   baseDef,
				   parent, 
				   &parArea);
	    } /* y */
