 ...
 *
 * rcsid "$Header: extractInt.h,v 1.6 95/02/05 14:14:44 stefanos Locked $"
 *
 *     ********************************************************************* 
...
extern CapValue extGetCapValue(HashEntry *he);
extern void extSetCapValue(HashEntry *he, CapValue value);
extern void extCapHashKill(HashTable *ht);
...
typedef struct nreg
{
    ...
    int		 nreg_class;	/* Type of tile that contains nreg_ll */
...
#define	extMakeNodeNumPrint(buf, plane, coord) \
    sprintf((buf), "%s_%s%d_%s%d#", DBPlaneShortName(plane), \
...
typedef struct extstyle
{
    ...
    PlaneList		 *exts_overlapOtherPlanes[NT];
    ...
    PlaneList		 *exts_overlapShieldPlanes[NT][NT];
	 ...
	 */
    PlaneList		 *exts_sideOverlapOtherPlanes[NT][NT];
    TileTypeBitMask	 exts_sideOverlapOtherTypes[NT][NT];
    PlaneList		 *exts_sideOverlapShieldPlanes[NT][NT];
...
extern Region *ExtFindRegions(CellDef *def, Rect *area, TileTypeBitMask *mask, TileTypeBitMask *connectsTo, ClientData uninit, Region *(*first) (/* ??? */), int (*each) (/* ??? */));

/* Filter functions for ExtFindRegions() */
extern Region *extTransFirst(Tile *tile, FindRegion *arg);		extern int extTransEach(Tile *tile, int pNum, FindRegion *arg);
extern Region *extResFirst();		extern int extResEach();
extern Region *extNodeFirst();		extern int extNodeEach();
extern Region *extHierLabFirst(Tile *tile, FindRegion *arg);	extern int extHierLabEach(register Tile *tile, register int pNum, FindRegion *arg);
...
extern char *extNodeName(LabRegion *node);
extern NodeRegion *extBasic(CellDef *def, FILE *outFile);
extern NodeRegion *extFindNodes(CellDef *def, Rect *clipArea);
extern ExtTree *extHierNewOne(void);
extern int extNbrPushFunc(register Tile *tile, register Rect *tileArea);
extern int extScaleDim(int dim);
extern int extScaleArea(int area);
...
#define	extTileConn(tp1, tp2, tbl) \
	extConnectsTo(DBgetTileType(tp1), DBgetTileType(tp2), (tbl))

/* -------------------------------------------------------------------- */

/* sizesize for hierarchical processing, in typical wire widths 
 * linked to tcl var EXT_STEP_SIZE 
 */
extern int extStepSize;
