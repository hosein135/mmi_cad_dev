...
/*
 * maMain.c -- malloc/free accessed through routines in this file.
 */
#include <stdlib.h>
#include <string.h>
#include "magic.h"
 ...
 */
int memNumMalloc = 0;       /* number of malloc calls */
int memNumFree = 0;       /* number of free calls */
int memMallocedBytes = 0;           /* total bytes malloced 
...
memMalloc(unsigned nbytes)
{
  ...
  if (memDelayedFree) 
  {
    MemMM ? MM_Free(memDelayedFree) : mem_free(memDelayedFree);
    memDelayedFree=NULL;
  }

  /* do the malloc */
  p = MemMM ? MM_Malloc(nbytes) : mem_malloc(nbytes);
   ...
   */
  if(memReserve) mem_free(memReserve);
...
void memFree(void *p)
{
  ASSERT(p, "memFree, passed 0 pointer\n");   
 ...
 */
void *
memMallocTag(unsigned nbytes, char *msg)
{
  if(MemAudit) fprintf(stderr, "%s: ", msg);
  
  return memMalloc(nbytes);
}
 ...
 */
void *
memCallocTag(unsigned nbytes, char *msg)
{
  if(MemAudit) fprintf(stderr, "%s: ", msg);
  
  return memCalloc(nbytes);
}
 ...
 * ----------------------------------------------------------------------------
 */
void memFreeTag(void *p, char *msg)
{
  if(MemAudit) fprintf(stderr, "%s: ", msg);
