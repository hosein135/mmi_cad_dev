 ...
 *
 *  If a new subcell is created, its CD_AVAILABLE is left FALSE.
 *
 * Results:
				    ...
				    */
{
	...
	if (def == NULL)
	{
	    ...
	    if(readFromDisk)
	    {
	      DBReadOpenQuiet++;  /* suppress load message */
	      DBReadCell(def);
	      DBReadOpenQuiet--;
	    }
	}
...
calmaElementSref(void)
{
    ...
    if(gdsReadNoDRC && !(def->cd_flags & CD_AVAILABLE))
    {
    ...
    if (isArray)
    {
      use = DBCellUseNewArray(def, propIName?propIName:gdsInstanceName);
      DBMakeArray(use, &GeoIdentityTransform, xlo, ylo, xhi, yhi, xsep, ysep);
    }
    else
    {
      use = DBCellUseNew(def, propIName?propIName:gdsInstanceName);
    }
     ...
     /* if set, ignore cells not in gdsReadCellNameHash */
{
	...
	cifReadCellDef->cd_flags |= CD_INTERNAL;
    ...
    else
    {
      ...
      if(cifReadCellDef->cd_flags&CD_READ_ONLY)
      {
	...
	/* fix drc vstamp */
	if(!(cifReadCellDef->cd_flags & CD_AVAILABLE))
	{
    ...
    if(gdsReadNoDRC)
    {
      ...
      for(kid = cifReadCellDef->cd_kids; kid; kid = kid->ck_next)
      {
	kid->ck_vDRC = cifReadCellDef->cd_vDRC;
      }
    }
