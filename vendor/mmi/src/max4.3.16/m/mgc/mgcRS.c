 ...
 *
 * Side effects:
 *      If new_name is specified, renames cell.
 *	Writes the cell out to disk.
 *      f specified, or the file
...
void CmdSave(Layout *w, TxCommand *cmd)
{
  ...
  if (cmd->tx_argc > 2)
  {
    MsgErrorF("Usage: %s [new_cell_name]\n", cmd->tx_argv[0]);
    return;
  }
  if (cmd->tx_argc == 2) newName = cmd->tx_argv[1];
  ...
  {
    /* valid cell name? */
    if(!DBCellNameCheck(newName)) return;

    if (!DBCellRenameDef(def, newName))
    {
      /* This should never happen */
      MsgErrorF("Cell not saved, rename to \"%s\" failed.\n",
		newName);
      return;
    }
  }
  ...
  /* and write the cell */
  if(DBWriteCell(def, NULL, DBSuffix)) def->cd_flags &= ~CD_MODIFIED;
}
