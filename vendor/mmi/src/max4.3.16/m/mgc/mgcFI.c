...
CmdFeedback(Layout *w, TxCommand *cmd)
{
	...
	case SAVE:
	    if (cmd->tx_argc != 3) goto badusage;
	    f = PaOpen(cmd->tx_argv[2], "w", NULL, ".", NULL);
	    if (f == NULL)
	    {
                   	...
                   	/* Describes the command that was invoked. */
{
    ...
    /* check for read-only */
    if(!DBAccessModifyMask(EditCellUse->cu_def,&maskBits)) return;
	...
	FREE_TAG(cmdFillList,"cmdFillArea");
                     		...
                     		/* Describes state of search. */
{
    ...
    MALLOC_TAG(struct cmdFillArea *,
	       cfa,
	       sizeof(struct cmdFillArea),
	       "cmdFillArea");
...
CmdFlush(Layout *w, TxCommand *cmd)
{
    ...
    /* do it */
    {
      ...
      if (DBCellUseParent(EditCellUse) == def)
      {
                             ...
                             /* Describes command arguments. */
{
    ...
    /* Create the new use. */
    newUse = DBCellUseNew(def, (char *) NULL);

    /*
    fprintf(stderr,"DEBUG CmdGetCell after DBCellUseNew\n");
    */
    ...
    DBChangedArea(EditCellUse->cu_def, bbox, &DBAllButSpaceBits, DBCF_INSTANCE_ONLY);
...
CmdIdentify(Layout *w, TxCommand *cmd)
{
    ...
    if(!DBInstanceNameCheck(cmd->tx_argv[1])) return;
                		...
                		/* New id for cell use. */
{
    ...
    DBCellUseParent(use)->cd_flags |= CD_MODIFIED;
