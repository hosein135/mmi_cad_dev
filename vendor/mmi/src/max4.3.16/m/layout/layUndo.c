...
LayUndoOldEdit(CellUse *editUse, CellDef *editRootDef, Transform *editToRootTrans, Transform *rootToEditTrans)
{
    ...
    ep->e_parentDef = DBCellUseParent(editUse);
 ...
 *	Redisplay the old edit cell and the new one.
 *
 * ----------------------------------------------------------------------------
 */
...
layUndoChangeEdit(register editUE *ep)
{
    ...
    DBChangedArea(EditRootDef, &area, &DBAllButSpaceBits, DBCF_DISPLAY_ONLY);
    ...
    if(!parent)
    {
      ...
      /* rootUse, search window list for it */
      for (w = layTopWindow; w != NULL; w = w->lay_next)
      {
	use = w->lay_rootUse;
	if (strcmp(use->cu_id, ep->e_useId) == 0) break;
      }
    }
    else
    {
      /* not a root use, lookup by id */
      {
	char *idp = &ep->e_useId[0];
	use = IHashLookUp(parent->cd_idHash, &idp);
	ASSERT(use,"layUndoChangeEdit");
      }
    }
