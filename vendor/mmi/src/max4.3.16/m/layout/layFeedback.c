...
LayFeedbackClear(void)
{
	...
	if (currentRoot != fb->fb_rootDef)
	{
		...
		LayChangedDefHL(currentRoot, &area, TRUE);
	    area = GeoNullRect;
	}
	FREE(fb->fb_text);
	fb->fb_text = NULL;
...
int
layfbGetTransform(CellDef *rootDef, 
                 			/* A root def that is an ancestor
					 * of cellDef in LayFeedbackAdd.
					 */
		  Transform *transform, 
                         		/* Transform up from cellDef 
					 * to use. 
					 */
		  Transform *cdarg)
					 ...
					 */
{
    ...
    /* skip internal cells */
    if (rootDef->cd_flags & CD_INTERNAL) return 0;

    /* if not root window, skip it */
    if (rootDef != LayCurWindow()->lay_rootUse->cu_def) return 0;

    layfbRootDef = rootDef;
    *cdarg = *transform;
    return 1;
}
				 ...
				 */
{
	...
	new = (Feedback *) memMalloc((unsigned) layfbSize*sizeof(Feedback));
	MALLOC_TAG(Feedback *,
		   new,
		   layfbSize*sizeof(Feedback),
		   "Feedback Array");
	...
	if (layfbArray != NULL)
	    FREE_TAG(layfbArray, "Feeback Array");
	layfbArray = new;
    }
    ...
    LayChangedDefHL(fb->fb_rootDef,&(fb->fb_rootArea),FALSE);
}
...
layFeedbackReportChanges(void)
{
	...
	{
	    if (currentRoot != NULL)
		LayChangedDefHL(currentRoot, &area, FALSE);
	    area = GeoNullRect;
	}
	(void) GeoInclude(&fb->fb_rootArea, &area);
	currentRoot = fb->fb_rootDef;
