...
DRCClean(CellDef *def)
{
  ...
  /* declare instance drc versions up-to-date */
  {
    CellKid *kid;

    for(kid=def->cd_kids; kid; kid=kid->ck_next) 
    {
      kid->ck_vDRC = def->cd_vDRC;
    }
  }

#ifdef HIDE
  /* don't even check uses of this def */	
  {
    CellParent *cellpar;
    ...
    for(cellpar = def->cd_pars; cellpar; cellpar=cellpar->cp_next)
    {
      ...
      CellKid *kid = IHashLookUp(parent->cd_kidHash, &def); 
