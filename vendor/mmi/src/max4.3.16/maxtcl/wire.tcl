...
set RCSVERSION(wire.tcl) { $Revision: 1.52 $ }
...
proc _wire_init {} {
    ...
    if { $varsNotFound != "" } {
	msg "warning: could not find values in the technology file for the following: [join $varsNotFound ", "]; using defaults\n"
    }
    if { $varsWrong != "" } {
	msg "warning: current wire width differs from minimum width\
...
proc wire_grid_menu {} -desc {
  View the wire grid and width menu
    ...
    lappend prop_list [list "WIRE GRID (pitch <or> pitchx pitchy)" "" -label]
    foreach layer "$WIRE(config_layers)" {
	set WIRE($layer,snap) [use_first WIRE($layer,snap) '1]
	lappend prop_list [list "$layer pitch" WIRE($layer,snap) -entry]
    }
...
proc wire_get_rc {{-width 0} layer} -desc {
  ...
  set errors ""
  if {$rpersq == ""} {
    append errors "error: No RESISTANCE RPERSQ values loaded for wire layer $layer, using 0 - maybe LEF file not read in?\n"
    set rpersq 0
  }
  if {$cpersq == ""} {
    append errors "error: No CAPACITANCE CPERSQ values loaded for wire layer $layer, using 0 - maybe LEF file not read in?\n"
    set cpersq 0
  }
  if {$cedge == ""} {
    if {$cpersq == 0} {
      append errors "error: No CAPACITANCE EDGE values loaded for wire layer $layer, using 0 - maybe LEF file not read in?\n"
    }
    set cedge 0
  }

  # Only pop this up the first time through.
  if {$errors != ""} {
    global _WIRE_GET_RC_DONE
    if {[use_first _WIRE_GET_RC_DONE] == 1} {
      msg $errors
    } else {
      max_error -buffer $errors
    }
    set _WIRE_GET_RC_DONE 1
  }
