...
set RCSVERSION(grid.tcl) { $Revision: 1.32 $ }
...
init_global IGRID(toggle) -default 0 -desc {
  ...
  IGRID($g,user,sizex) IGRID($g,user,sizey) - User grid.\
    sizex used in both dirs if prop_mode==normal.\

  In the following: xx is one of:
    "coarse" for the coarse visible grid,
    "fine" for the fine visible grid,

  IGRID($g,$xx,sizex) IGRID($g,$xx,sizey) - Grid size.\
    sizex used in both dirs if prop_mode==normal.\

  IGRID($g,$xx,originx) IGRID($g,$xx,originy) - Grid origin.\
    originx used in both dirs if prop_mode==normal\
...
proc grid_init {} -desc {
  ...
  for {set g 1} {$g <= $GRID(ngrids)} {incr g} {
    ...
    set IGRID($g,user,sizex) [use_first IGRID($g,user,sizex) GRID(userx)]
    set IGRID($g,user,sizey) [use_first IGRID($g,user,sizey) GRID(usery)]
...
proc grid_update {} -desc {
  ...
  set GRID(userx) $IGRID($g,user,sizex)
  if { $IGRID($g,prop_mode) == "detailed" } {
    set GRID(usery) $IGRID($g,user,sizey)
  } else {
    # User x grid specifies both x and y grids.
    set GRID(usery) $IGRID($g,user,sizex)
  }
}
...
proc grid_get {{-name ""} {-number ""}} -doc {
  ...
  if {![info exists IGRID($gridno,user,sizex)]} { return "" }

  # Note: originx/originy not supported yet.
  set userx $IGRID($gridno,user,sizex)
  set usery $IGRID($gridno,user,sizey)
...
proc grid_set {{-name ""} {-number ""} {-rename ""} {-user ""} {-coarse ""} {-fine ""}
  {-coarse_origin ""} {-fine_origin ""} {-coarse_visibility ""} {-fine_visibility ""}} -type user -desc {
  Sets various grids.
} -doc {
  This is the preferred method to set grids.

  If -number, set the specified grid number,
  else if -name, set the grid with that name,
  else set currently selected grid.

  The -user, -coarse and -fine take a single number (for square grid)
  or a list of two numbers (for assymetric grid) to set the specified grid.
  The "user" grid controls snapping during movement.

  The -coarse_origin and -fine_origin require a list of two numbers to
  set the specified grid origin.

  -coarse_visibility and -fine_visibility can be "off" "lines" or "dots.
  These visibilities only apply if the visibility for the grid as a whole is on.
  See also toggle_grid to turn the grid on and off.

  -rename allows you to add a text name for the grid, which shows up
  in the Grid Menu as a comment.

  Example:
  grid_set -number 4 -rename fplan-block -user "4.8 4.8" \
	-coarse "4.8 4.8" -fine "1 1" \
	-coarse_visibility lines -fine_visibility off
} {
  global IGRID GRID
  # If no -name or -number, set current grid.
  set gridno $IGRID(current)
  ...
  if {$rename != ""} {
    set IGRID($gridno,name) $rename
  }

  foreach thing "user coarse fine" {
    set grid_spec [set $thing]
    if {$grid_spec != ""} {
      if {[llength $grid_spec] > 2} { error "grid_set: grid must be one or two numbers" }
      setl {gridx gridy} $grid_spec
      set IGRID($gridno,$thing,sizex) $gridx
      if {$gridy == ""} { set gridy $gridx }
      set IGRID($gridno,$thing,sizey) $gridy
      if {$gridx != $gridy} {
  ...
  foreach thing "coarse fine" {
    set origin [set ${thing}_origin]
    if {$origin != ""} {
      if {[llength $origin] != 2} {error "grid_set: origin must be two numbers"}
      setl {originx originy} $origin
      set IGRID($gridno,$thing,originx) $originx
      set IGRID($gridno,$thing,originy) $originy
      if {$originx != $originy} {
	set IGRID($gridno,prop_mode) "detailed"
      }
    }

    set vis [set ${thing}_visibility]
    if {$vis != ""} {
      if {$vis != "off" && $vis != "lines" && $vis != "dots"} {
	error "grid_set: visibility must off, lines, or dots"
      }
      set IGRID($gridno,$thing,visibility) $vis
    }
  }

  if {$gridno == $IGRID(current)} {
    grid_update
  }
}
...
proc grid_menu {} -desc {
  ...
  while {1} {
	...
	append gridnums "$i "
      ...
      lappend prop_list \
	  "{User Design Grid X} IGRID($g,user,sizex) $user_grid_prop \
      ...
      lappend prop_list \
	  "{User Design Grid Y} IGRID($g,user,sizey) $user_grid_prop \
	  -when {\$IGRID($g,prop_mode) == {detailed}} \
	  -help {points and rectangles entered by the mouse will \
	      snap to this resolution in the Y direction}"
      lappend prop_list \
	  "{User Design Grid} IGRID($g,user,sizex) $user_grid_prop \
	...
	return
      ...
      set bad_grid 0
      if {$IGRID($g,prop_mode) == "detailed"} {
	set list [list user,sizex user,sizey]
      } else {
	set list [list user,sizex]
      }
      foreach name $list {
	set ongrid [max $GRID(mask) \
