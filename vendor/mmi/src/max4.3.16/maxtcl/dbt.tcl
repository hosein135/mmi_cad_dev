...
set RCSVERSION(dbt.tcl) { $Revision: 1.8 $ }

# This is a file full of things that should be written in C.

proc dbt_listize_pin_path {{-cell ""} pinpath} -desc {
  Like dbt_listize_cell_path, but path is either
  a port or pin name.
} {
  set l [string last "/" $pinpath]
  if {$l == -1} {
    # It is a port.
    return $pinpath
  } else {
    set subcell [string range $pinpath 0 [expr $l-1]]
    set subpath [dbt_listize_cell_path $subcell]
    if {$subpath == ""} {return ""}
    set pin [string range $pinpath [expr $l+1] end]
    return [concat $subpath [list $pin]]
  }
}

proc dbt_listize_cell_path {{-cell ""} cellpath} -desc {
  Convert slash based pathname to max path list.
} -doc {
  Find a maximal path for which a subcell name exists.
  Eg: for a/b/c, look for cell a/b/c,
  then for cell a/b containing c,
  then for cell a containing b/c
  then for cell containing b containing c.
  Return "" if not found.
} {
  return [cellinfo_name [dbt_find_cell -fast 1 -cell $cell $cellpath]]

  # The following code works, but obsoleted by dbt_find_cell.
...
proc dbt_find_cell {{-list 0} {-fast 1} {-partial 0} {-no_flat 0} {-cell ""} cellpath} -desc {
  Return cell info (in db_search_cells format) given path name.
} -doc {
  Find a maximal path for which a subcell name exists.
  Eg: for a/b/c, look for cell a/b/c,
  then for cell a/b containing c,
  ...
  If -partial 0, return the cell_info for the specified cell,
  or "" if the cell is not found.
  
  If -partial 1, look for a partial match and return the matching cellinfo.

  If -fast 1, dont bother to compute accurate transform or location information,
  but the name and path in the cell_info will be correct.
} {
  if {$cell == ""} {set cell [lay_rootcell]}
  ...
  # Find maximal part of cellpath that exists as a subcell in this cell.
  set subcellid $cellpath
  while {1} {
    set cell_list [db_instances -cell $cell -id $subcellid]
    if {[llength $cell_list] > 0} {
      # We found a matching cell.
      if {$no_flat == 0 || [fplan_db_inst -cell $cell getprop $subcellid] != "flatten"} {
	# Use it.
	break
      }
    }
    set l [string last "/" $subcellid]
    if {$l == -1} {
      # Subcell not found.
      return ""
    }
    set subcellid [string range $subcellid 0 [expr $l-1]]
  }

  # Subcell Found.
  set cell_info [lindex $cell_list 0]
  if {$l == -1} {
    # We swallowed the entire path.
    return $cell_info
  } else {
    set subdef [cellinfo_def $cell_info]
    set subcellinfo [dbt_find_cell -cell $subdef [string range $cellpath [expr $l+1] end]]
    if {$subcellinfo == ""} {
      # We found a partial match.
      if {$partial} {
	return $cell_info   ;# return as much as found.
      } else {
	return ""
      }
    }
    set subcellpath [cellinfo_path $subcellinfo]
    if {$fast} {
      # Dummy up the path in the cellinfo to use the path we found.
      # Note that all the x,y and tranform info will be bogus.
      return [lreplace $subcellinfo 6 6 [concat [list $subcellid] $subcellpath]]
    } else {
      # Return accurate cell_info.
      sel_cell $subcellpath
      return [sel_what_cells]
    }
  }
}

proc dbt_find_label {{-hch /} {-fast 0} {-cell ""} path} -desc {
  Return label info given path name.
} -doc {
  If -cell specified, use it as the root of the path.
  If -fast 1, result has the correct label name and path, but
	      does not have accurate x, y or transform info,.
  The -hch option specifies the hierarchy character.

  This function uses a longest match algorithm to find labels given
  a hierarchical path where some of the intervening cells might have been flattened.
  Example:   Given a/b/c, it searches for:
    label a/b/c
    label c in a/b
    label b/c in a
    label c in b in a
  ...
  #regsub -all {(^-)|([][\*?])} $path {\\&} quoted_labid

  set lab_list [db_search_labels -no_glob -non_hier -limit 1 -cell $cell $path]
  if {[llength $lab_list] > 0} {
    return [lindex $lab_list 0]
  }

  # No label.  Try interpreting this as a path to a cell containing a label.
  set lp [string last $hch $path]
  set subcellid [string range $path 0 [expr $lp-1]]
  ...
  while {1} {
    set cell_list [db_instances -cell $cell -id $subcellid]
    if {[llength $cell_list] > 0} break
    set l [string last $hch $subcellid]
    if {$l == -1} {
      # Subcell not found.
      return ""
    }
  ...
  set labid [string range $path [expr $l+1] end]

  # Look for the label in this cell.
  set lab_info [dbt_find_label -hch $hch -fast 1 -cell [cellinfo_def $cell_info] $labid]
  ...
  set labpath [concat [list $subcellid] [labinfo_path $lab_info]]

  if {$fast} {
    # Dont return accurate x,y coords.
    # Just dummy up the path.
    return [lreplace $lab_info 7 7 $labpath]
  } else {
    set fullpath [concat $labpath [labinfo_text $lab_info]]
    # 2/15: db_search_labels has a bug that it does not take a list,
    # but it seems to work ok if you join it into a string first.
    return [lindex [db_search_labels -no_glob -cell $cell [join $fullpath]] 0]
  }
}
...
#puts "dbt_flyline $args"
  ...
  if { $init } {
    ...
    if { $push_def == "" } {
      set push_def [cellinfo_def [dbt_find_cell $arg1]]
      assert { $push_def != "" }
    }
...
USAGE:
  ...
  if {[lay_editcell] == [lay_rootcell]} {
    ...
    # TODO: Remove this when the db_chunk, above, works for edit-in-place.
...
proc dbt_kids {{-hierarchy} {-nogcells} {cell ""}} -desc {
  Return list of child cell defs.
} -doc {
  If -hierarchy, return list of all cells in tree.
  If cell not specified, start at editcell.
} {
  if {$cell == ""} {
    set cell [lay_editcell]
  }

  set kids [db_kids $cell]
  foreach kid $kids {
    if {$nogcells && [string match {#*} $kid]} {continue}
    set kid_hash($kid) 0
  }

  if {$hierarchy} {
    foreach kid $kids {
      foreach subkid [dbt_kids -hierarchy $kid] {
	if {$nogcells && [string match {#*} $subkid]} {continue}
	set kid_hash($subkid) 0
      }
    }
  }
  return [array names kid_hash]
}
...
# This should be in C.
# It should also have a -cell option.
# It should really just be part of db_search_l labels.
proc dbt_search_label {labelpath} -desc {
  Return info on a label given a path to the label.
} {
  set slash [string last "/" $labelpath]
  if {$slash == -1} {
    # No path.  Just look for the label in the cell.
    return [lindex [db_search_l labels -non_hier -exact $labelpath] 0]
  } else {

    # The path includes the trailing slash.
    set path [string range $labelpath 0 $slash]
    set text [string range $labelpath [expr $slash+1] end]

    if {$path == "./"} {
      return [lindex [db_search_l labels -non_hier -exact $text] 0]
    }

    # 6/29: Update: the lay_internals IS NOT NECESSARY.  db_search labels
    # documentation is incorrect.  If you specify -cell, it
    # searches the entire hiearchy under that cell,
    # regardless of lay_internals.
    set labels [db_search_l labels -any_cell -cell [lay_editcell] -exact $text]
    return [lindex $labels 0]
  }
}
