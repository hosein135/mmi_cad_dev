...
set RCSVERSION(techinfo.tcl) { $Revision: 1.26 $ }
...
proc _techinfo_translate_layer {layer} -desc {
  translate mmi layer name to customer tech file layer name.
    ...
    # Let use set up LAYER_NAME to specify explicit translations.
...
proc techinfo {what {l1 ""} {l2 ""} {flags ""}} -desc {
  ...
  switch -- $what {
    ...
    "layer_exists" {
      # The dbt procs should really be here in techinfo
      return [expr {[dbt_short_name $l1] == $l1}]
    }
    ...
    "layer" {
      ...
      if { [lsearch -exact $order $newl1] >= 0 } {
	return $newl1
      }

      # Try lower case.
      set newl1 [_techinfo_translate_layer [string tolower $l1]]
      if { [lsearch -exact $order $newl1] >= 0 } {
	return $newl1
      }

      # Try upper case.
      set newl1 [_techinfo_translate_layer [string toupper $l1]]
      if { [lsearch -exact $order $newl1] >= 0 } {
	return $newl1
      }
