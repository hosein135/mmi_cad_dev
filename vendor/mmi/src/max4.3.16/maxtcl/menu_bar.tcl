...
set RCSVERSION(menu_bar.tcl) { $Revision: 1.76 $ }
...
proc menu_add_cmd {args} -desc {
  ...
  Deletes any pre-existing entry with same label.
    ...
    # Invalidate the cache of menu stuff
    global _MENU_BAR_CMD_CACHE
    catch {unset _MENU_BAR_CMD_CACHE}
...
proc menu_bar_command {label} -desc {
  ...
  if {! [info exists _MENU_BAR_CMD_CACHE] } {
    ...
    foreach thing $menu_info {
      setl {menulabel menucommand} $thing
      set fix_label [menu_bar_fix_label $menulabel]
      set _MENU_BAR_CMD_CACHE($fix_label) $menucommand
    }
  }
...
proc api_menu_bar_invoke {label} -type user -desc {
  Invoke the specified item from the max menu.
} -doc {
  Runs a command from the menu bar as if that item had been selected with the mouse.
  The argument is the complete list of menus clicked, separated by "->",
  and with spaces replaced with underscore.
  Example:

    api_menu_bar_invoke File->User_Preferences->Display_Options...

  Will open the Display Options menu.
} {
  set cmd [menu_bar_command $label]
  if {$cmd == ""} {
    error "No menu bar command found for: $label"
  }
...
proc menu_bar_build {w} -desc {
  ...
  menu_add_cmd $menu "Load Cell Hierarchy"	cell_load_hierarchy \
    -desc "Load current cell's descendents into memory."
  menu_add_cmd $menu "Print Cell Hierarchy" fplan_print_hier
  menu_add_cmd $menu "Edit Cell Path" cell_path_edit
  ...
  menu_add_cmd $menu "Edit Text Properties..."      edit_label T -nowrap \
      -desc "View/Change text or net properties"

  menu_add_cmd $menu "Edit Cell Properties..."      cell_lbox C -nowrap \
      -desc "View/Change cell properties"
  ...
  menu_add_cmd $menu "Edit Text Menu..."       change_labels \
      -desc "Change multiple selected labels at once via menu"
  ...
  menu_add_separator $menu
  menu_add_cmd $menu "Display Options..." view_display_options O \
    -desc "Control visibility of database objects and window components"
  ...
  menu_add_cmd $menu "Select by Name..."     select_by_name n \
      -desc "Select text, nets or cells by name"
  ...
  menu_add_cmd $menu "Select Cell Probe..." selc_mode_enter  F -nowrap
...
proc menu_add {args} -type user -desc {
  ...
  USAGE:
  menu_add [-menu (File|Edit|View|Tool|Misc|Local|Help)]
	   [-label <name>]
	   [-command <command>]
	   [-hotkey <hotkey>]
	   [-help <string>]
  ...
  if {$hotkey != "" && [mode_stack] == ""} {
    # The hotkey binding is saved in the array, but it has not taken effect yet.
    # Re-enter main mode to make it happen now.
    # Note: This is an expensive operation.  It would be better to defer it if
    # multiple menu_add were being called, but this should only happen when
    # the user is creating a custom menu, so dont worry about it.
    mode_change main
  }
}
...
proc menu_see_cif_update {} -desc {
    ...
    #$menu add command -label " " -columnbreak 1
    # The tear-off indicator does not stretch
