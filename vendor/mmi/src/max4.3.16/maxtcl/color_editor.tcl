...
set RCSVERSION(color_editor.tcl) { $Revision: 1.10 $ }
...
# 12/14/01, pat: I hate to take out the unsmiley face,
# but I needed room for something actually useful: solid stipple.
lappend SAMPLE_STIPPLES "\
    00000000 \
    01100110 \
    01100110 \
...
proc _make_stipple_edit {w} -desc {
  ...
  for {set i 0} {$i < 8} {incr i} {
    ...
    for {set j 0} {$j < 8} {incr j} {
      ...
      bind $bit <Button-3> "_color_edit_mark_bit $bit"
    }
...
proc _color_edit_mark_bit {w} {
  set oldtext [$w cget -text]
  set newtext [expr {$oldtext == "X" ? " " : "X"}]
