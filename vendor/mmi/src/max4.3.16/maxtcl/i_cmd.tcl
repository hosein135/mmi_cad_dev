...
set RCSVERSION(i_cmd.tcl) { $Revision: 1.23 $ }
...
collection of actions to take between interactive commands
    ...
    # allow read retrys on NOTFOUND cells every interactive
    # command.
    #(cell files  may have been added to path by other tools)
    catch {db_read_retry}
...
proc i_cmd_between_undos {} -desc {
  actions to take between interactive commands and after undos
    ...
    if {$LUCKY_VARIABLE != 0} {
      warning {The MMI license server is not responding. \
	Restart the license server, save your modified buffers,\
	and restart MAX to stop these messages.}
    }}
