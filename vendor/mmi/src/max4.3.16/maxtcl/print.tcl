...
set RCSVERSION(print.tcl) { $Revision: 1.27 $ }
...
# These are the plotting routines to search for.
# We will not show these to the user unless a routine named print_$type actually exists.
# The default type will be the first type in the list for which a print_$type routine exists.
set PRINT(types) "postscript"

proc print_cell {{type ""}} -desc {
  ...
  # Using the list of print types from PRINT(types),
  # search for print routines that have been loaded, either
  # in this file or from the $MMI_LOCAL max.rc startup file.

  set types ""
  foreach type $PRINT(types) {
    if {[info commands print_$type] != ""} {
  ...
  if {[llength $types] == 0} {
    msg "Aborting, can't find any print routines for $PRINT(types).\n"
    return 0
  } elseif {[llength $types] == 1} {
    # If only one found, just launch it.
    eval print_[lindex $types 0]
    return 1
  } else {
    # Post a menu for user to choose print type.
    # The type selected will be sticky within this max session.
    use_init PRINT(type) [lindex $types 0]
    set prop_list [list [list "Print Type:" PRINT(type) -radio $types]]
    if {![prop_menu2 -title "Choose Print Type" $prop_list]} {
      return 0
    }
    eval print_$PRINT(type)
  }
}
...
proc _gdsplot_setup {} -desc {
  ...
  #The -s option does not do anything at the end of a spool!
  #set default_cmd "cat %s | lpr -s -P%p"
  set default_cmd "cat %s | lpr -P%p"
...
proc _UNUSED_print_get_default_printer {} -desc {
  Try to find a default printer for this user.
} {
  global env
  if {[info exists env(LPDEST)]} { return $env(LPDEST) }
  if {[info exists env(PRINTER)]} { return $env(PRINTER) }
  set result [exec lpstat -d]
  return [lindex end $result]
}


# create postscript for the given max cell and write to file
# If user wants to print, just lpr -r it (-r deletes file afterward).

use_init PS(print_to_file) 0

# largest, portrait, or landscape
use_init PS(orient) largest

# print ports?
use_init PS(ports) 1
use_init PS(port_scale) 1

# in inches
use_init PS(page_x) 8.5
use_init PS(page_y) 11.0
use_init PS(margin) 0.5
use_init PS(dpi) 72.0

# fill with no fill at 0 to solid at 1.
use_init PS(fill) 0

use_init PS(boundary) prb

use_init PS(print_command) "lpr"

proc print_postscript {} {
  global PS
  global MAX_VERSION MAX_COMPILE_TIME MAX_VERSION_TAG

  set filename "[file rootname [cell_file [lay_rootcell]]].ps"

  set prop_list ""
  lappend prop_list [list orientation PS(orient) -choice {largest landscape portrait} -help "If largest, the orientation that fits the layout with the largest scale is chosen."]
  foreach thingy "print_command boundary page_x page_y margin dpi" {
    lappend prop_list [list $thingy PS($thingy)]
  }

  lappend prop_list [list "display ports" PS(ports) -binary -help "If true, displays the port text."]
  lappend prop_list [list "port scale factor" PS(port_scale) -number 0.0 -incr 0.1 -entry  -help "To scale the port text size up or down."]

  lappend prop_list [list "fill" PS(fill) -number 0.0 1.0 -incr 0.1 -entry -help "0 for no filling.  1 for solid black.  0.1 produces a light shading, etc."]

  lappend prop_list [list "print to file" PS(print_to_file) -binary -help "If true only writes a postscript file, otherwise prints the file to the printer with the print_command."]
  lappend prop_list [list "Print File" filename]

  if {![prop_menu2 -title "Postscript Print Setup" $prop_list]} {return}

  if {$PS(print_to_file)} {
    puts "Writing postscript file $filename ..."
  } else {
    puts "Writing postscript file for [lay_rootcell] to printer ..."
    set filename __[lay_rootcell]__.ps
  }
  ...
  # add header stuff and definitions
  # resets printer
  puts $FILE_ID "%\04"
  puts $FILE_ID "%!PS-Adobe-3.0"
  ...
  puts $FILE_ID "%% Creator: $message"
  puts $FILE_ID "%% Pages: 1"
  puts $FILE_ID "%% EndComments"

  puts $FILE_ID ""

  puts $FILE_ID "/box \{"
  puts $FILE_ID "    /y2 exch def"
  puts $FILE_ID "    /x2 exch def"
  puts $FILE_ID "    /y1 exch def"
  puts $FILE_ID "    /x1 exch def"

  puts $FILE_ID ""
  puts $FILE_ID "    x1 y1 moveto"
  puts $FILE_ID "    x1 y2 lineto"
  puts $FILE_ID "    x2 y2 lineto"
  puts $FILE_ID "    x2 y1 lineto"
  puts $FILE_ID "    closepath"
  if {$PS(fill) > 0.0001} {
    puts $FILE_ID "    gsave [expr 1 - $PS(fill)] setgray fill grestore"
  }

  puts $FILE_ID "    stroke"
  puts $FILE_ID "\} bind def\n"

  # add text
  puts $FILE_ID "/center_text {"
  puts $FILE_ID "    /rot exch def"
  puts $FILE_ID "    /size exch def"
  puts $FILE_ID "    /text exch def"
  puts $FILE_ID ""
  puts $FILE_ID "    gsave"
  puts $FILE_ID ""
  puts $FILE_ID "    /Helvetica findfont size scalefont setfont"
  puts $FILE_ID ""
  puts $FILE_ID "    text stringwidth pop /width exch def"
  puts $FILE_ID ""
  puts $FILE_ID "    % Compute the baseline offset"
  puts $FILE_ID "    0 0 moveto (TXygqPZ) false charpath flattenpath pathbbox"
  puts $FILE_ID "    /baseline exch def pop pop pop newpath"
  puts $FILE_ID ""
  puts $FILE_ID "    % Translate coordinates using x,y on stack and then center adjust"
  puts $FILE_ID "    translate"
  puts $FILE_ID "    rot rotate"
  puts $FILE_ID "    width -0.5 mul baseline -0.5 mul moveto"
  puts $FILE_ID "    text show"
  puts $FILE_ID ""
  puts $FILE_ID "    grestore"
  puts $FILE_ID ""
  puts $FILE_ID "} bind def"
  puts $FILE_ID ""

  puts $FILE_ID "/add_port {"
  puts $FILE_ID "    /justify exch def"
  puts $FILE_ID "    /degrees exch def"
  puts $FILE_ID "    /size exch def"
  puts $FILE_ID "    /text exch def"
  puts $FILE_ID ""
  puts $FILE_ID "    gsave"
  ...
  puts $FILE_ID "    /Helvetica findfont size scalefont setfont"
  puts $FILE_ID ""
  puts $FILE_ID "    0 0 moveto (+) false charpath flattenpath pathbbox"
  puts $FILE_ID "    /ph exch def /pw exch def pop pop"
  puts $FILE_ID ""
  puts $FILE_ID "    text stringwidth pop /width exch def"
  puts $FILE_ID ""
  puts $FILE_ID "    translate"
  puts $FILE_ID "    degrees rotate"
  puts $FILE_ID ""
#  puts $FILE_ID "    -1 -1 moveto"
#  puts $FILE_ID "    1 1 lineto"
#  puts $FILE_ID "    stroke"
#  puts $FILE_ID "    -1 1 moveto"
#  puts $FILE_ID "    1 -1 lineto"
#  puts $FILE_ID "    stroke"
#  puts $FILE_ID ""
  puts $FILE_ID "    width justify mul justify 0.5 add pw mul sub ph -0.5 mul moveto"
  puts $FILE_ID "    text show"
  puts $FILE_ID ""
  puts $FILE_ID "    grestore"
  puts $FILE_ID ""
  ...
  puts $FILE_ID "%%BeginSetup"
  puts $FILE_ID "%%IncludeResource: font Helvetica"
  puts $FILE_ID "%%EndSetup"
  puts $FILE_ID ""
  puts $FILE_ID "%%Page: 1 1"
  puts $FILE_ID "save"
  puts $FILE_ID ""

  puts $FILE_ID "/Helvetica findfont 10 scalefont setfont"

  # TODO -- want this to be just bbox of cells -- not paint or anything else
  # TODO -- should take into account port text size
  setl {x1 y1 x2 y2} [lay_bbox]
  ...
  # example:
  # x from 0 to 120
  # width is 8.5 * 72 = 612 / 120 = 5.1

  set scalex [expr 1.0 * ($PS(page_x) - 2*$PS(margin)) * $PS(dpi) / ($x2-$x1)]
  set scaley [expr 1.0 * ($PS(page_y) - 2*$PS(margin)) * $PS(dpi) / ($y2-$y1)]

  set rscalex [expr 1.0 * ($PS(page_y) - 2*$PS(margin)) * $PS(dpi) / ($x2-$x1)]
  set rscaley [expr 1.0 * ($PS(page_x) - 2*$PS(margin)) * $PS(dpi) / ($y2-$y1)]

  set scale [min $scalex $scaley]
  set rscale [min $rscalex $rscaley]

  # do rotation if better and called for
  if {($PS(orient) == "largest" && $rscale > $scale) || \
	  $PS(orient) == "landscape"} {
    # rotate
    puts $FILE_ID "90 rotate"
    set scale $rscale

    puts $FILE_ID "30 -20 moveto ([cell_file [lay_rootcell]]    [clock format [clock seconds]]) show\n"

    # now translate to center of page
    set tx [expr -$x1 + ( $PS(dpi)*$PS(page_y)/$scale - ($x2-$x1) ) / 2.0]
    set ty [expr ( $PS(dpi)*$PS(page_x)/$scale - ($y2-$y1) ) / 2.0]

    # make up for rotation
    set ty [expr - $ty - ($y2 - $y1) - $y1]
    ...
    set tx [expr -$x1 + ( $PS(dpi)*$PS(page_x)/$scale - ($x2-$x1) ) / 2.0]
    set ty [expr -$y1 + ( $PS(dpi)*$PS(page_y)/$scale - ($y2-$y1) ) / 2.0]

#    puts "$tx $ty --> [expr ( $PS(dpi)*$PS(margin)/$scale )]"

  }

  puts $FILE_ID [format "%g %g scale" $scale $scale]
  puts $FILE_ID [format "%g %g translate" $tx $ty]

  puts $FILE_ID ""

  puts $FILE_ID [format "%g setlinewidth" [expr 0.5/$scale]]

  puts $FILE_ID "0 setlinecap"
  puts $FILE_ID "1 setlinejoin"
  puts $FILE_ID "0.0 0.0 0.0 setrgbcolor"
  ...
  foreach label [db_search_labels] {
    if {[lindex $label 0] == $PS(boundary)} {
      # found a label on boundary layer
      setl {ll lx1 ly1 lx2 ly2} $label
      if {$lx1 == $lx2 || $ly1 == $ly2} {
	# not a box
	continue
      }
      lappend coords [lrange $label 1 4]
    }
  }

  foreach paint [db_search_paint $PS(boundary)] {
    lappend coords [lrange $paint 1 4]
  }

  puts $FILE_ID "gsave"
  puts $FILE_ID "\[3 3\] 0 setdash"
  foreach coord $coords {
    setl {lx1 ly1 lx2 ly2} $coord
    puts $FILE_ID "$lx1 $ly1 moveto $lx2 $ly1 lineto $lx2 $ly2 lineto $lx1 $ly2 lineto closepath stroke"
  }
  puts $FILE_ID "grestore\n"

  # minimum width for text -- 10 chars @ 5x scale
  set mintext [expr 40.0/$scale]
#  set mult [expr $scale / 5.0]
  set mult 1

  set count 0

  # add cells
  foreach cell [db_search_cells -user_bbox] {
#    set coords [fplan_bbox -cellid [lindex $cell 0] -grid mask -parent]
    set coords [lrange $cell 2 5]

    puts $FILE_ID "$coords box"

    if {[expr [incr count] % 10000] == 0} {
      puts "...$count..."
    }

    # eliminate text early if box too small -- performance
    if {[expr [lindex $coords 2] - [lindex $coords 0]] > $mintext || \
	    [expr [lindex $coords 3] - [lindex $coords 1]] > $mintext} {
      # add text since big enough
      ...
      set kind [lindex $cell 1]

#puts "$kind $dx $dy  mult=$mult  mintext=$mintext"

      # rotate if needed
      if {$dy > [expr 2 * $dx]} {
	# rotated

	set yc [expr $y1 + $dy * 0.5]
	set xb [expr $x1 + $dx * 0.67]
	set xt [expr $x1 + $dx * 0.33]

	# size text based on block size (approx)
	set size [min [expr $dx / 4] [expr $dy / [string length $kind]]]
	if {$size > 0.5} {
	  puts $FILE_ID "$xt $yc ($kind) [format %g [expr $size * $mult]] 90 center_text"
	}

	set size [min [expr $dx / 4] [expr $dy / [string length $name]]]
	if {$size > 0.5} {
	  puts $FILE_ID "$xb $yc ($name) [format %g [expr $size * $mult]] 90 center_text"
	}

      } else {
	# not rotated

	set xc [expr $x1 + $dx * 0.5]
	set yt [expr $y1 + $dy * 0.67]
	set yb [expr $y1 + $dy * 0.33]

	# size text based on block size (approx)
	set size [min [expr $dy / 4] [expr $dx / [string length $kind]]]
	if {$size > 0.5} {
	  puts $FILE_ID "$xc $yt ($kind) [format %g [expr $size * $mult]] 0 center_text"
	}

	set size [min [expr $dy / 4] [expr $dx / [string length $name]]]
	if {$size > 0.5} {
	  puts $FILE_ID "$xc $yb ($name) [format %g [expr $size * $mult]] 0 center_text"
	}
      }
    }
  }    
  ...
  if {$PS(ports) == 1} {
    # 115 45 (+< HOWDY) 2.7 0 0 add_port
    set size [format "%g" [expr $PS(port_scale) * [min 8 [expr 6.0 / $scale]]]]

    # add labels
    foreach label [db_search_labels] {
      switch [lindex $label 9] {
	input { set dir < }
	output { set dir > }
	inout { set dir <> }
	default { continue ; set dir "" }
      }

      switch [lindex $label 5] {
	EAST { 
	  set text "+$dir [lindex $label 6]"
	  set rotate 0
	  set justify 0
	}
	WEST { 
	  set text "[lindex $label 6] ${dir}+"
	  set rotate 0
	  set justify -1
	}
	NORTH { 
	...
	default { 
      ...
      puts $FILE_ID "[lrange $label 1 2] ($text) $size $rotate $justify add_port"
    }
  }
  ...
  if {!$PS(print_to_file)} {
    set stat [catch {eval exec cat $filename | $PS(print_command)} msg]
