...
set RCSVERSION(max.tcl) { $Revision: 1.40 $ }
...
# This returns TRUE for new versions of max that use a tcl list
# for paths instead of slash separated paths.
# Needs to be defined before reading .rc files.
proc use_list_path {} {
  global MAX_VERSION
  return [expr {[string range $MAX_VERSION 2 end] >= 3.13}]
}

set LAY_MAX_FONT_SIZE 12
...
# cursor_udpate not needed; we just did cursor_busy, above.
# cursor_update

# Load up the keymap.
keymap_init

# Source the project file again, so max commands that were not defined
# previously can happen now.
...
proc max_source_maxrc {} -desc {
    ...
    global argv MN_PATH_SYS_LIB doc_source

    call_keyword -nocase $argv {{norc}}
    if {$norc} {
      # -norc option suppresses reading the project.rc or max.rc files.
      msg "note: -norc option found: skipping search for project or max.rc files\n"
      return
    }
...
proc max_parse_args {} -desc {
  ...
  set cells [call_keyword -nocase -append -dash 0 $argv \
	       {{norc} {project ""} {set ""} {command ""} {iconify ""} {batch ""} {tech ""} {technology ""}}]
...
# This can error out in the post-processing command
# hooks invoked as a result of things done in the max.rc file.
# Catch it, or max will die.
if {[msg_catch i_cmd_between a b c]} {
  global errorInfo
  msg "$errorInfo\n"
  msg "$a $b $c\n"
}
...
msg_flush	;# Make sure we flush any errors during start up.
