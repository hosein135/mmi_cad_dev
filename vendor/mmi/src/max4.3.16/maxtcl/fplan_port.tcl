...
set RCSVERSION(fplan_port.tcl) { $Revision: 1.8 $ }
...
proc _port_snap {x y lay} {
  setl {snapx snapy offsetx offsety} [wire_get_grid $lay]
...
proc _UNUSED_word {str n} -desc {
  Return nth space separated word from str.
} {
  return [lindex [_fplan_words $str] $n]
}
...
proc _fplan_find_label {cell px py layer} -desc {
  ...
  return [db_search_l labels -cell $cell -non_hier \
...
proc _fplan_port_eval_layer {cell port reg} {
  ...
  # Evaluate the layer, locspec and bitloc.
...
#puts "snap $port $px $py -> [_fplan_snap_to_region $reg $px $py]"
  ...
  if {$placetype == "expr"} {
    ...
    setl {loc_keywords loc1} [_fplan_parse_port_spec $cell $port $bitnum locspec]
    setl {bitloc_keywords bitloc} [_fplan_parse_port_spec $cell $port $bitnum bitloc]

    if {$loc1 != ""} {
      # Move the port to the user specified location.

      # The locspec property is a request to place it somewhere.
  ...
  while {1} {
    ...
    set fnd_lab [lindex [_fplan_find_label $cell $px $py $newlayer] 0]
...
proc fplan_port_legalize {{-center_unplaced 1} {-resize ""} cell {port_list ""}} -desc {
  Update port placement in max based on port properties.
} -type user -doc {
  The <cell> is required.

  If <port_list> is unspecified, legalize all i/o ports in cell.

  The -resize option is used internally by the floorplanner immediately after resizing a cell;
  the value of -resize is the previous bounding box; by specifying the old bounding box,
  the legalizer can make a better guess about which side of the cell the ports used to be
  on before the cell was resized.

  The -center_unplaced defaults to 1.  If -center_unplaced 0, unplaced ports are left alone, I think.
  Not sure.
} {
  # If this fails, it will have screwed up or deleted the ports, so undo the damage.
  msg "fplan_port_legalize -center_unplaced $center_unplaced -resize $resize $cell [list $port_list]\n"

  undo_delim
  if {[catch {_fplan_leg_ports_int -center_unplaced $center_unplaced -resize $resize $cell $port_list} result]} {
#DEBUG
#    undo_to_delim
...
proc _fplan_leg_ports_int {{-center_unplaced 1} {-resize ""} cell port_list} -doc {
  ...
  # Delete the hidden labels so we dont have any conflicts
  ...
  foreach lab_info [db_search_labels -cell $cell -non_hier] {
	...
	set port_info($id) $lab_info
	lappend all_ports $id
      }
    }
  }
  ...
  foreach port $port_list {
	...
	#setl {lx ly lkind curlayer} [fplan_db_pin2 -usecache -cell $cell $port]
	setl {lx ly} [labinfo_loc $port_info($port)]
	set locspec [fplan_db_pin -cell $cell getprop $port locspec]
	switch -- [lindex $locspec 0] {
	  left -
	  right -
	  top -
	  bottom {
	    if {[llength $locspec] == 1} {
	      # Only a side was specified.  Effectively, this is just a placed port.
	      set side $locspec
	      set m [_fplan_port_eval_layer $cell $port $locspec]
	      lappend place_ports([lindex $locspec 0],$m) [list $port $lx $ly]
	    } else {
	      # User specified an expression. We will put it there and not move it.
	      lappend user_expr_ports([lindex $locspec 0]) [list $port $lx $ly]
	    }
	    _fplan_sel_label -more $port
	  }
	  "" {
	    setl {lx ly} [labinfo_loc $port_info($port)]
	    # No expression in the "locspec" property.
	    # If the port is in a region, keep it in that region.
	    # If the port is right on the corner, its hard to figure out which
	    # region it belongs in, so first try using the port text orientation.
	    # The port may not be in a region due to any number of causes.
	    # If not in a region, just snap it to the nearest region.
	    set region ""
	    set fuzz 1  ;# Amount it can be off and still be considered in the region.
	    setl {bx1 by1 bx2 by2} $cell_rect
	    switch [string toupper [labinfo_pos $port_info($port)]] {
	      EAST {
		if {abs($lx - $bx2) <= $fuzz} {set region right}
	      }
	      WEST {
		if {abs($lx - $bx1) <= $fuzz} {set region left}
	      }
	      TOP {
		if {abs($ly - $by2) <= $fuzz} {set region top}
	      }
	      BOTTOM {
		if {abs($ly - $by1) <= $fuzz} {set region bottom}
	      }
	    }
	  ...
	  default {
	    max_error -buffer "error: cell $cell port $port unrecognized \"locspec\" property: $locspec"
	    # And leave it alone.
	  }
	}
      }
  ...
  foreach port $fixed_ports {
    ...
    set fnd_labs [_fplan_find_label $cell ${l.x1} ${l.y1} ${l.layer}]

    # See if there are any other ports at this location besides the fixed port.
    if {[llength $fnd_labs] >= 2} {
      foreach lab_info $fnd_labs {
	if { [labinfo_text $lab_info] != $port} {
	  max_error -buffer "warning: cell $cell fixed port $port has same location as port ${l.text}"
	  break  ;# Just report the first.
	}
      }
    }
  }
...
#puts "unplaced=$unplaced_ports"
  foreach port $unplaced_ports {
    ...
    set lab_info $port_info($port)
    db_label -kind [labinfo_kind $lab_info] -pos n [labinfo_layer $lab_info] $port $cx $cy
...
#puts "$place_ports($reg,$mlayer) -> $portslots"
	...
	set lab_info $port_info($port)
	db_label -kind [labinfo_kind $lab_info] -pos $_FPLAN_PORT(lab_pos,$reg) $mlayer $port $px $py
      }
	...
	set lab_info $port_info($port)
	db_label -kind [labinfo_kind $lab_info] -pos $_FPLAN_PORT(lab_pos,$reg) $mlayer $port $cx $cy
      }
    }
  }
  ...
  foreach reg $region_list {
    if {$region_overflow($reg)} {
      max_error -buffer "warning: ports did not fit in region: $reg, extra ports moved to center"
    }
  }
  util_prof end
}
...
proc _fplan_port_make_buckets {cell reg mlayer new_ports} -desc {
  ...
  set width [wire_info width $mlayer]
  set sep [techinfo sep $mlayer]
...
#puts "cell [lay_editcell] reg $reg $rx1 $ry1 $rx2 $ry2 -layers $mlayer labels=$labels"
  ...
  foreach lab_info [lsort -real -index $sindex $labels] {
    ...
    # If port is not on grid, it may obstruct two locations!
    # Obstruction starts at wire center - width/2 (obstructing wire) - sep - width/2 (next wire).
    set pstart [expr $val - $width - $sep]
    set pstop [expr $val + $width + $sep]
    set istart [expr int(ceil(($pstart-$offset)/$snap))]
    set istop [expr int(floor(($pstop-$offset)/$snap))]
  ...
  if {[catch {db_search_l paint -any_cell -cell $cell -area $rx1 $ry1 $rx2 $ry2 ${mlayer}_obs} obs_paintballs]} {
    set obs_paintballs ""
  }
  foreach paint_info $obs_paintballs {
    struct max_paint p $paint_info
    if {$ori == "v"} {
      set istart [expr int(ceil(( (${p.x1}-$width/2.0-$sep)  -$offset)/$snap))]
      set istop [expr int(floor(( (${p.x2}+$width/2.0+$sep) -$offset)/$snap))]
    } else {
      set istart [expr int(ceil(( (${p.y1}-$width/2.0-$sep) -$offset)/$snap))]
      set istop [expr int(floor(( (${p.y2}+$width/2.0+$sep) -$offset)/$snap))]
    }
...
proc fplan_port_opt_outside {{-verbose 0} {-withplace unplaced} {-adjacent 0} {-best_side 0} cellid} -desc {
  Find preferred location for pin based on stuff outside cellid.
} -type user -doc {
  The <cellid> is the cell instance id of the cell to process.
  Assumes we are editing the parent cell of cellid.

  For each pin in cellid, find nearest other pin/port to which it is attached.
  If -adjacent is 1, move pin to horizontal/vertical extension from that pin.
  If -adjacent is 0, move pin to edge of cell on a line between center of the two cells.

  The -withplace specifies a subset of ports to optimize based on the place prop of the ports.
  The values of this option can be: "unplaced placed", "unplaced" (default), or "placed"

  If -best_side is 1, ports are moved to the best side.  If 0, ports are moved to the
  best location on the side they currently are on.

  This function may place lots of ports in the same place;
  run fplan_port_legalize to fix them.
} {
  global FPLAN _FPLAN_PORT
  set hch [nlt_hier_char]

  util_prof begin  fplan_port_opt_outside
  ...
  set submodi [fplan_db_inst celli2modi $cellid]
  ...
  # TODO: (2/4/02): For hierarchical traversal, need to specifically look up the
  # instance object instead of just passing in $submodi.*
  foreach subpin [nl_find_pins ${submodi}${hch}* $topmod] {
    set stuff [split $subpin $hch]
...
#puts "pinname=$pinname"
	...
	setl {c2x1 c2y1 c2x2 c2y2} [fplan_bbox -cell [cellinfo_def [lindex $cell_stuff2 0]]]

	# Default: use center of cellid2
	set x [expr (${c2x1}+${c2x2})/2.0]
	set y [expr (${c2y1}+${c2y2})/2.0]

	if {$adjacent} {
	  # Align to adjacent pins on neighboring cell.

	  # Get location of the other pin, which is pin2 in cell2.
	  set pintext ${modi2}${hch}${pin2}
...
#puts "locations=$locations"
  ...
  util_prof end  fplan_port_opt_outside
}
...
proc _fplan_port_placetype {{-best_side 0} {-withplace ""} cell port} -desc {
  ...
  if {$placeprop=="placed" || $placeprop=="user"} {
    ...
    if {$locspec == ""} {
      set placekey "placed"
    } elseif {[llength $locspec] == 1 && ($locspec == "left" || $locspec == "right" || \
  ...
  switch -- $placekey {
    ...
    "user_reg" {
      set reg [fplan_db_pin -cell $cell getprop $port "locspec"]
    }
    "" - "unplaced" {
      set reg "unknown"
    }
...
proc fplan_port_opt_inside {{-verbose 0} {-withplace unplaced} {-best_side 1} cell {ports "*"}} -desc {
  Find preferred location for pin in specified region based on cell contents.
} -doc {
  Preferred location for each port is on horizontal/vertical extension
  from closest subcell pin inside cell.
  This function may place lots of ports
  in the same place; run fplan_port_legalize to fix them.

  If <ports> are unspecified, optimize all ports.
  ...
  BUG: This function currently has difficulty figuring out the side of a port if it is near a corner.
} {
  global _FPLAN_PORT
  ...
  foreach port $ports {
    ...
    foreach pin [nl2_get_net_pins -hier 1 $lay_mod $net] {
      if {$pin == $port} {continue}
      # This happens if a pin is connected to some cells, eg stdcells,
      # that were not placed in the cell.
      if {[catch {setl {px py} [fplan_db_pin2 -xform $pin]}]} {
	max_error -buffer "warning: cant find pin $lay_mod.$pin (net $net) maybe cell not placed?"
	continue
      }
...
proc fplan_db_cache {{-cell ""} {-cells 0} {-clear}} -doc {
  ...
  if {1 || $cells} {
    ...
    foreach cell_info [db_search_cells] {
  ...
  # NOTE: Dont use this to try to get hierarchical transforms - I dont think it works.
      msg "warning: fplan_db_cache -all may not do hierarchical transforms\n"
...
proc fplan_db_pin_list {{-cell ""} {-usecache} {-global}}  -desc {
  ...
  if {$usecache} {
    ...
    foreach lab_info [db_search_labels -cell $cell -non_hier] {
      set t [labinfo_kind $lab_info]
      if {$global ? ($t=="global") : ($t=="input"||$t=="output"||$t=="inout")} {
	lappend port_list [labinfo_text $lab_info]
      }
    }
    return $port_list
  }
...
proc fplan_db_pin2 {{-hch "."} {-usecache} {-fixname} {-xform} {-cell ""} pin} -desc {
  Return label data.
} -doc {
  ...
  if {[use_list_path]} {
    ...
    # There no longer is any name fixing, so we dont need
    # to break out the label from the cell to fix them separately.

    if {$usecache} {
      return $FPLAN_PORT_CACHE($cell,$pin,info)
    }

    set lab_info [dbt_find_label -hch $hch -cell $cell $pin]
    if {$lab_info == ""} {
      error "Can not find pin $pin (cell $cell)"
    }
    setl {x y} [labinfo_loc $lab_info]
    return [list $x $y [labinfo_kind $lab_info] [labinfo_layer $lab_info] [labinfo_text $lab_info]]
  }


  # OLD CODE PRIOR TO USING dbt_find_label, 2/19/02:
...
proc fplan_port_save_props_to_nl {{-cell ""}} -desc {
  Save current props in nl.
} {
  ...
  foreach cell $cell_list {
    ...
    set propnames [fplan_db_pin -cell $cell listprops]
	...
	output -
	inout { 
	  set portname [labinfo_text $lab_info]

	  # Save location
	  setl {lx ly} [labinfo_loc $lab_info]
	  nl2_set_port_location $mod $portname $lx $ly

	  # If you dont set an orientation, it comes out "none" next time.
	  nl2_set_port_orientation $mod $portname N

	  # Save layer
	  nl2_set_port_geometry $mod $portname [labinfo_layer $lab_info] 0 0 0 0

	  # Save any other properties.
	  foreach propname $propnames {
	    set val [db_prop -def $cell PORT($portname,$propname)]
	    if {$val != ""} {
	      nl2_set_port_attr $mod $portname $propname $val
	    }
	  }
	}
      }
...
proc fplan_db_pin {{-cell ""} action {pin ""} {propname ""} {value ""}} -desc {
  ...
  switch -- $action {
    listprops {
      # Return a list of defined port properties.
      set ret ""
      foreach pair $FPLAN_PORT_OPTIONS {
	lappend ret [lindex $pair 0]
      }
      return $ret
    }
    delete {
...
proc _fplan_check_missing_ports {cell other_ports} -desc {
  ...
  setl {extra_max_ports extra_nl_ports} [utils_ldiff $max_ports $other_ports]
...
proc _fplan_list_selected_ports {{-use_sel 1} bus} -desc {
  ...
  if {$use_sel} {
    ...
    if {[llength $cell_list] != 0} {
      error "There are cells selected"
    }
    if {[llength $lab_list] == 0} {
	...
	lappend portlist ${l.text}
      ...
      # Use the selected cell, if any:

    if {[llength $cell_list] > 0} {
      if {[llength $cell_list] > 1} {
	error "Select just one cell to process"
      }
      struct max_cell c [lindex $cell_list 0]
      set cell ${c.def}
      set cellid ${c.id}
      ...
      # No cells selected.  Do ports in current cell.
      set cell [lay_editcell]
      set cellid .
  ...
  if {$bus} {
    ...
    foreach port $portlist {
      set busnames([nlt_bus $mod $port]) 0
    }
    return [list $cell $cellid [array names busnames]]
  } else {
...
proc fplan_edit_selected_ports {{-editcell 0}} -desc {
  ...
  if {$editcell} {
    ...
    setl {cell cid bitlist} [_fplan_list_selected_ports -use_sel 0 0]
  }
  ...
  do {
	...
	specified by the "locspec" and "bitloc" properties, if specified,.
    ...
    lappend prop_list [list "PIN LOCATION LEGALIZER" "" -label]
    lappend prop_list [list "run legalizer" run_spreader -binary]

    lappend prop_list [list "" "" -separator]
    lappend prop_list [list "Common Options" "" -label]
    lappend prop_list [list "verbose" verbose -binary]
    lappend prop_list [list "process selected ports only" use_selection -binary \
      -help {if set, process selected ports in current cell.  Otherwise, process all ports\
	in the selected cell, or the current cell if none selected}]


    if {0} {
      # Note: This code works, but now I want people to use the label_lbox code.
      ...
      setl {cell cid portlist} [_fplan_list_selected_ports -use_sel $use_selection $aggbus]
  ...
  if {$ret != 0} {
    ...
    setl {cell cid bitlist} [_fplan_list_selected_ports -use_sel $use_selection 0]

    if {$run_optimizer} {
      msg "Optimize Ports $cell -verbose $verbose -withplace \"$place_opts\" -best_side $best_side\n"

      if {$cell == [lay_editcell]} {
	fplan_port_opt_inside -verbose $verbose -withplace $place_opts -best_side $best_side $cell $bitlist
      } else {
	fplan_port_opt_outside -verbose $verbose -withplace $place_opts -adjacent $adjacent -best_side $best_side $cid
      }
    }

    if {$run_spreader} {
      fplan_port_legalize $cell $bitlist
    }
  }
...
proc _fplan_edit_port_list {cell portlist} -desc {
  ...
  while {1} {
    ...
    lappend prop_list [list "" "" -break 20]
    foreach port $portlist {
      set port_info($port,locspec) [fplan_db_pin -cell $cell getprop $bitcache($port) locspec]
      lappend prop_list [list locspec port_info($port,locspec) -popup {left right top bottom ""}]
      if {$show} {
  ...
  foreach port $portlist {
    foreach bit [nlt_bus_explode $port] {
      foreach propname {locspec bitloc place layerspec} {
	...
	if {$num1 == -1 && $num2 == -1} {
	  ...
	  if {! $bus} {
	    ...
	    foreach single_port $port_bits {
	      set bus_name [nlt_bus -lowest $mod $single_port]
	      if {$bus_name != ""} {
...
proc fplan_write_ports {{cell ""}} -desc {
  Write sue-compatible .ports file for cell.
} {
  ...
  if {$cell == ""} {
	...
	TODO: Where should x and y come from?
	set x [fplan_db_pin2 -cell $cell getprop $pin x]
	set y [fplan_db_pin2 -cell $cell getprop $pin y]
	set portinfo [format "port %-*s %10s %10s" $maxlen $pin $x $y]
	foreach propname [fplan_db_pin -cell $cell listprops] {
	  set val [fplan_db_pin -cell $cell getprop $pin $propname]
	  if {$propname == "port_x" || $propname == "port_y"} {
	    # These are placed in the ports file above.
	    continue
	  }
	  if {$val != ""} {
	    append portinfo " -$propname [list $val]"
	  }
	}
	puts $fd $portinfo
      }
...
proc fplan_read_ports {{-merge "ask_me"} {cell ""}} -desc {
  Read a .ports file, in format similar to .ports file used by sue.
} -doc {
  USAGE:
...
proc _fplan_add_label {portname ix iy kind layer reg} {
  ...
  #TODO: THIS IS CAUSING MAX CORE DUMPS, so disabled.
...
proc _fplan_sel_label {{-more} portname} {
  ...
  #BUG: THE sel_check WAS CAUSING MAX CORE DUMPS, so disabled.
...
proc OLD_fplan_save_props {{-cell ""}} -desc {
