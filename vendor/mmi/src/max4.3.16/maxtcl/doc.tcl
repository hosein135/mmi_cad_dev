...
proc doc_box {} -desc {
  ...
  global MAX_FULL_DOC env


  # Check for full documentation or not.
  # If the user sets MAX_FULL_DOC, but does not have
  # a maxDev license, the documentation will not actually be loaded
  # into the arrays, so they will not see it.
  if {$MAX_DEVELOPER == 1 || [use_first MAX_FULL_DOC env(MAX_FULL_DOC) '0] == 1} {
    set MAX_FULL_DOC 1
  } else {
    set MAX_FULL_DOC 0
  }
...
proc _doc_box_update {} -desc {
    ...
    global doc_box doc_user MAX_DEVELOPER MAX_FULL_DOC
...
#    } 
    ...
    # (could be stuff in doc_user array that no longer exists)
    if { $MAX_FULL_DOC == 0 } {
	set matches_old $matches
	set matches ""

	foreach name $matches_old {
	    # If the name contains "*" or "?", info commands will fail,
	    # because MMI does not permit info commands with wildcards
	    # unless MAX_DEVELOPER is set.
	    if {[catch {info commands $name} result] == 0 && $result != "" } {
    ...
    foreach name $matches {
      ...
      set sep "-"
      if { $MAX_FULL_DOC == 1 && [info exists doc_user($name)]} {
	set sep "U"
      }
...
proc _doc_box_variables_fill {} -desc {
  ...
  global _doc_at_db doc_user_var MAX_FULL_DOC doc_box
  ...
  foreach name $matches {
    ...
    set sep "-"
    if { $MAX_FULL_DOC == 1 && [info exists doc_user_var($name)]} {
      set sep "U"
    }
    set entry [format "%-25s %s %s" $name $sep $desc]
