...
set RCSVERSION(message.tcl) { $Revision: 1.11 $ }
# message handling.
...
init_global MSG(WARN_MAX) \
	-type INTEGER \
	-default 50 \
	-desc { Max number of messages printed per interactive command, or -1 to disable message supression. }

set MSG(TOTAL_COUNT) 0  ;# Number of warnings during current command.
set MSG(warn_suppress_count) 0
...
proc _msg_open_log {} -desc {
  ...
  for {set i 1} {$i < 100} {incr i} {
    ...
    set MSG(log_file_name) $file_name
  ...
  puts $MSG(log_fd) [help_max_get_max_id]
  puts $MSG(log_fd) [help_max_get_config]
  puts ""
...
proc _msg_print {s} -desc {
  ...
  if {[string first "\n" $s]} { incr MSG(TOTAL_COUNT) }

  if {$MSG(logging_on) && $MSG(log_fd) != ""} {
      puts -nonewline $MSG(log_fd) $s

      if {$MSG(TOTAL_COUNT) >= $MSG(WARN_MAX) && $MSG(WARN_MAX) > 0 } {
	if { $MSG(TOTAL_COUNT) == $MSG(WARN_MAX) } {
	  set tmp "**** additional messages suppressed, see log file $MSG(log_file_name) ****"
	  puts ""; puts $tmp
	}
	return   ;# Do not puts the message.
      }
  }
...
proc msg_put_log {s} -desc {
  Put the string directly to the log file.
} {
  global MSG
  if {$MSG(logging_on) && $MSG(log_fd) != ""} {
      puts -nonewline $MSG(log_fd) $s
...
proc _msg_warn {s} -desc {
  ...
  # You can set MSG(WARN_MAX) to -1 to disable it.
  if {$MSG(logging_on) && $MSG(log_fd) != "" &&
      $MSG(TOTAL_COUNT) >= $MSG(WARN_MAX) && $MSG(WARN_MAX) > 0 } {
    if { $MSG(TOTAL_COUNT) == $MSG(WARN_MAX) } {
      set tmp "**** additional messages suppressed, see log file $MSG(log_file_name) ****"
      puts ""; puts $tmp
      append MSG(buffer) $tmp
    }
    incr MSG(warn_suppress_count)
  } else {
    puts -nonewline $s
    append MSG(buffer) $s
  }
...
proc msg_log_file_menu {} {
  ...
  # MSG(log_file) is the pattern to generate the file name.
  # MSG(log_file_name) is the actual chosen file name.
  if {$log_clear && [info exists MSG(log_file_name)]} {
    catch {file delete $MSG(log_file_name)}
  }
...
proc msg_flush {} -desc {
  ...
  if {$MSG(warn_suppress_count)} {
    msg "Warning: $MSG(warn_suppress_count) warnings suppressed - see file $MSG(log_file)\n"
    set MSG(warn_suppress_count) 0
  }
}


proc msg_run {args} -desc {
  Log tcl comand to log file, and execute it.
} {
  msg "$args\n"
  return [eval $args]
}
