...
set RCSVERSION(box.tcl) { $Revision: 1.25 $ }
...
proc box_dim_edit {} -desc {
  ...
  while {1} {
    ...
    lappend prop_list [list "Mark this box" annotate -binary]
    lappend prop_list [list "Clear box marks" "" -button "lay_line -tag box -clear" -separator]
	...
	default {
      ...
      if {$annotate} {
	layt_rect -tag box $xbot $ybot $xtop $ytop
      }
...
proc box_goto_coords {} -desc {
  ...
  lappend prop_list "units units -choice {microns nanons half_nanons} \
    -help {Specify units in microns (10 ** -6 meter) or nanons (10 ** -9 meter)}"
  lappend prop_list "{Zoom In} GOTO_COORDS(zoom) -binary -separator \
    -help {if set, max will move and zoom the window to center the specified point}"

  lappend prop_list [list {Mark this point} annotate -binary \
    -help {if set, max will draw a cross at the point locations}]
  lappend prop_list [list "Clear point marks" "" -button "lay_line -tag point -clear"]
  ...
  } elseif {$units == "half_nanons"} {
    setl {x y} [uusnap -mask [expr $x / 2000.0] [expr $y / 2000.0]]
  } else {
  ...
  if {$annotate} {
    set tlen 1  ;# Ticks are arbitrarily 2 microns high.
    eval lay_line -tag point [expr $x - $tlen] $y [expr $x + $tlen] $y
    eval lay_line -tag point $x [expr $y - $tlen] $x [expr $y + $tlen]
  }
