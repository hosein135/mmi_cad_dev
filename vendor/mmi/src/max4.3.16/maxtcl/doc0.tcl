...
set RCSVERSION(doc0.tcl) { $Revision: 1.15 $ }
...
# Check to see if we want to put full documentation into the arrays.
# The _MAX_DEV_LICENSE variable is used only here, and only when
# reading in the tcl code at startup.
# Later on, we will display it only if MAX_DEVELOPER or MAX_FULL_DOC are set.
# This must be done right up front so we can use it while building the documentation,
# to prevent internal documentation from being put into an array where unauthorized
# users could find it.
# Note that the full tcl source code has not been read in yet.
proc _doc0_check_dev_license {} {
  global _MAX_DEV_LICENSE MAX_DEVELOPER
  if {$MAX_DEVELOPER} {
    # Max already checked for a maxDev license before setting MAX_DEVELOPER variable.
    set _MAX_DEV_LICENSE 1
  } else {
    set _MAX_DEV_LICENSE 0
    if {[info commands "license_check"] == "license_check"} {
      set _MAX_DEV_LICENSE [license_check -check maxDev]
    }
  }
}
_doc0_check_dev_license
...
if { [info commands proc_unrestricted] != ""} {
	...
	# Special case for "set", which can be renamed without
	# goofing with the documentation.  This is needed
	# because the doc_at commands use "set", and an
	# infinite loop is otherwise created.
	# This occurred when reading the project.rc file.
	if {$old == "set" || $new == "set"} {
...
_doc_proc_orig doc_add_cmd {name desc doc {internal ""}} {
    global _MAX_DEV_LICENSE doc_user doc_source doc_developer
