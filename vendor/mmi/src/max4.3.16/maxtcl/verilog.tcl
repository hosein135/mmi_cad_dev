...
set RCSVERSION(verilog.tcl) { $Revision: 1.6 $ }
...
proc _verilog_new_hier_fill {} {
  ...
  global _VERILOG_HIER_MENU
  #set win .verilog_hier
  #set plist $win.items.list
  set plist $_VERILOG_HIER_MENU(listbox_win)

  if {[$plist size] > 0} {
      $plist delete 0 end
  }

  # Note: we are using the islibcell flag in nl
  # to determine which cells are LEF cells.
  proc _verilog_new_list_tree2 {mod level} {
    global _VERILOG_HIER_RESULT _VERILOG_HIER_MENU
    incr level
    if {[llength $_VERILOG_HIER_RESULT] > 1000} {
      return  ;#overflow
    }
    foreach inst [nl2_list_cells -hierarchy [expr !$_VERILOG_HIER_MENU(hier_show_lef)] $mod] {
    ...
    foreach ref [array names refcnt] {
      # Output some space, def name, optional number of instances.
      set str [format "%*s%s" $level " " $ref]
      if {$refcnt($ref) >= 2} { append str " #$refcnt($ref)" }
      lappend _VERILOG_HIER_RESULT $str
      _verilog_new_list_tree2 $ref $level
    }
  }
  ...
  foreach mod [verilog_find_top] {
    ...
    if {[llength $_VERILOG_HIER_RESULT] > 1000} {break}
    nl2_link $mod
    _verilog_new_list_tree2 $mod 0
  }
  ...
  if {[llength $_VERILOG_HIER_RESULT] >= 1000} {
    $plist insert end "***overflow, list truncated***"
  }
}


# 2/25: Rewritten to expand and to use prop_menu2.
proc _UNUSED_verilog_new_hier_menu {} -desc {
  display verilog hierchy from previously read verilog in a listbox
} {
  global LISTBOX_FONT _VERILOG_HIER_MENU
  ...
  bind $plist <Button-1> { \
	  _verilog_hier_button %W %x %y Button-1;break}
  bind $plist <Button-2> { \
	  _verilog_hier_button %W %x %y Button-2; break }
  ...
  set _VERILOG_HIER_MENU(hier_show_lef) 0

  # Disable this:  If user clicks it, the X server hangs until it completes.
  # TODO: Make it work again.
  #checkbutton $win.lef -text "Show LEF cells" -padx 3 -pady 3 \
  #  -variable _VERILOG_HIER_MENU(hier_show_lef) \
  ...
  button $win.print -text "puts selection to text window" -padx 3 -pady 3 \
    -command "_verilog_hier_cmd puts_selected"
  pack $win.print -side top -fill x -expand 1

  button $win.copy -text "Copy verilog to file..." -padx 3 -pady 3 \
    -command "_verilog_hier_cmd copy"
  pack $win.copy -side top -fill x -expand 1
  ...
  set helpmsg {Module names are shown indented by their hierarchical \
    ...
    Button-1 selects/deselects tree starting at selected module.\
    Button-2 selects/deselects just the individual module.}
  ...
  grab set $win
  cursor_wait $win 1 "Verilog Hierarchy Display"
  tkwait window $win
  #update idletasks
  if {$old_grab != ""} {
...
proc verilog_new_hier_menu {} -desc {
  ...
  set _VERILOG_HIER_MENU(total_count) 0
  set _VERILOG_HIER_MENU(selected_count) 0
  set _VERILOG_HIER_MENU(listbox_win) ""	;# Init to avoid an error when window first posted.
  set _VERILOG_HIER_MENU(hier_show_lef) 0

  set prop_list ""
  lappend prop_list [list "Total Modules:" _VERILOG_HIER_MENU(total_count) -label]
  lappend prop_list [list "Selected" _VERILOG_HIER_MENU(selected_count) -label]

  # The break is necessary to keep the listbox from processing
  # the mouse buttons after we are done with them.
  set listbox_bindings [list \
    <Button-1> "_verilog_hier_button %W %x %y Button-1;break" \
    <Button-2> "_verilog_hier_button %W %x %y Button-2;break" \
    <Map> "_verilog_hier_cmd save_win %W; _verilog_new_hier_fill" ]
  lappend prop_list [list "" "" -listbox -bind $listbox_bindings -reload]
  lappend prop_list [list "Clear Selection" "" -button {$_VERILOG_HIER_MENU(listbox_win) selection clear 0 end}]
  lappend prop_list [list "Edit Verilog Text" "" -button "_verilog_hier_cmd edit"]
  lappend prop_list [list "Copy verilog to file..." "" -button "_verilog_hier_cmd copy"]
  ...
  prop_menu2 -title "Verilog Hierarchy" -buttons "Close=0=cancel" $prop_list
}


proc verilog_copy {outputfile modlist} -desc {
  Copy verilog for selected modules into a specified file.
} {
  # Make a list of files to process.
  global VERILOG_MODULE2FILE
  foreach modname $modlist {
    set filelist($VERILOG_MODULE2FILE($modname)) 1
  }

  # Convert modlist to an array
  foreach thingy $modlist {
    set keepers($thingy) 1
  }

  set outfd [open $outputfile w]
  unwind_catch {
    foreach filename [array names filelist] {
      set pf [open $filename r]
      set keepflag 0	;# Set to 1 to copy lines to file.
	...
	switch -- [lindex $stuff 0] {
	  "module" {
	    set keepflag [info exists keepers([lindex $stuff 1])]
	    if {$keepflag} {
	      puts $outfd $line
	    }
	  }
...
proc _verilog_hier_cmd {cmd {savewin ""}} {
  global _VERILOG_OPT
  global _VERILOG_HIER_MENU
  #set win .verilog_hier
  #set plist $win.items.list
  set plist [use_first _VERILOG_HIER_MENU(listbox_win)]
  switch $cmd {
    "save_win" {
      set _VERILOG_HIER_MENU(listbox_win) $savewin
      set plist $savewin
    }
    "select_n" {
    ...
    "copy" {
      # Copy the verilog itself to a new file.
      if {[$plist size] == 0} {
	error "nothing selected"
      }

      set prop_list ""
      set output_filename [use_first outputfile 'tmp.vg]
      lappend prop_list [list "output filename" output_filename \
	-filename [list -message "Copy verilog" -filename $output_filename -pattern *.vg]]
      set copy_invert 0
      lappend prop_list [list "output:" copy_invert -enum {"selected verilog" "non-selected verilog"}]
      if {![prop_menu2 -title "Copy selected verilog" $prop_list]} {
	return ;# cancelled.
      }
	...
	if {[$plist selection includes $i] == $copy_invert} {
	  continue
	}
	set line [$plist get $i]
	set modname [string trim $line]
	regsub { .*} $modname "" modname
	lappend modlist $modname
      }

      verilog_copy $output_filename $modlist
    }
    "edit" {
      if {[$plist size] == 0} {
	error "nothing selected"
    ...
    "_UNUSED_edit" {
      # Note: 2/25: rewritten to use verilog_copy.
  ...
  set _VERILOG_HIER_MENU(total_count) [$plist size]
  set _VERILOG_HIER_MENU(selected_count) [llength [$plist curselection]]
}

proc _verilog_hier_button {win lx ly action} -desc {
...
proc _UNUSED_verilog_list_tree2 {mod level} {
  global _VERILOG_HIER_RESULT
...
proc verilog_make_maps {} -doc {
  ...
  foreach mod [array names _VERILOG_MODULES] {
	...
	if {$n1 != $n2 && $bus_aggregated} {
	  set netbus [nlt_bus -lowest $mod $net]
	  if {$netbus != ""} {
	...
	if {[llength $bitlist] == 1} {
	  ...
	  if {$bus_aggregated} {
	    foreach bit $bitlist {
	      set netbus [nlt_bus -lowest $mod $bit]
...
proc nl2_get_cell_ref {mod inst} -desc {
  ...
  if {$FPLAN(use_nl_shell)} {
    ...
    return [nlt_log {nl_get_cell_reference $nlcell}]
  }
  struct verilog_module m $_VERILOG_MODULES($mod)
...
proc nl2_list_cells {{-hierarchy 0} mod} -desc {
  In this case the -hiearchy flag means to list only hierarchical cells.
} {
  global FPLAN _VERILOG_MODULES
  if {$FPLAN(use_nl_shell)} {
    ...
    lappend cmd $mod
    if {[nlt_log {catch $cmd ret}]} {return ""}
...
proc nl2_list_nets {{-hier 0} {-bus 0} mod} -doc {
  If -bus 1, return aggregated bus names.
} {
  global FPLAN _VERILOG_MODULES

  if {$FPLAN(use_nl_shell)} {
    set cmd "nl_list_nets [expr {$hier ? "-hierarchy" : ""}] -noassign -noconstant [list $mod]"
    nlt_log {set result [eval $cmd]}
    return $result
  }
  ...
  if {$bus} {
    foreach bit $wire_list {
      set bus_names([nlt_bus $mod $bit]) 0
    }
    return [array names bus_names]
  } else {
    return $wire_list
  }
}
...
proc nl2_list_ports {{-bus 0} {-incl_kind 0} mod} -desc {
  ...
  if {$bus} {
	...
	setl {bit kind} $thing
	set result([list [nlt_bus $mod $bit] $kind]) 0
      }
...
proc nl2_get_net_pins {{-hch ""} {-bus 0} {-hier 0} curmod net} -doc {
  ...
  the net object returned from another nl command.
  ...
  if {$FPLAN(use_nl_shell)} {
    ...
    if {$hier} {
      nlt_log {set inet [nl2_find_inet $curmod $net]}
    } else {
      nlt_log {set inet [nl2_find_net $curmod $net]}
    }
    if {$inet == ""} {
      # This can happen when doing flylines, for example, if the verilog
      # does not match the cell for some reason.
      msg "Warning: net $net not found\n"
      return ""
    }
    ...
    if {$hier} {append cmd " -hierarchy"}
    append cmd { $inet}
    nlt_log {set pin_list [eval $cmd]}
  ...
  if {$bus} {
	...
	lappend result "$port\[[nlt_list_compress $cons($port)]\]"
      ...
      #set portbits [nl2_get_net_pins <some_module> $netbit]
      #set result ""
...
proc nl2_get_pin_net {curmod pin} -doc {
  ...
  if {$FPLAN(use_nl_shell)} {
    ...
    if {[string first $hch $pin] > 0} {
      #regsub "/" $pin "." pin
      set ipin [nl2_find_ipin $curmod $pin]
    } else {
...
proc verilog_squish {} -desc {
  ...
  set orig_hier_sep $nl_hierarchy_separator
  ...
  foreach mod [nl2_list_designs] {
    set re_names ""
    foreach submodi [nl2_list_cells -hierarchy 1 $mod] {
      global submod
	...
	if {[fplan_cell_info -is_lef $subsubmod]} {
	  ...
	  nlt_log {set nl_hierarchy_separator "@"}  ;# Temporary value.
	  nlt_log {nl_ungroup $submodi}
	  nlt_log {set nl_hierarchy_separator $orig_hier_sep}
	  ...
	  nlt_log {set cell_obj [nl2_find_cell $old_name $mod]}
...
proc nl2_read_verilog {{-include ""} {-flags ""} filename} -type local -desc {
  ...
  global nl_current_design FPLAN _VERILOG_OPT

  set old [cursor_busy 1]
  ...
  if {$FPLAN(use_nl_shell)} {
    ...
    set cmd [concat nl_read_verilog $flags [list $file_list]]
    msg "$cmd\n"

    if {[nlt_log {catch $cmd result}]} {
      ...
      nlt_log {eval "nl_read_verilog -ports_only $flags $file_list"}
    }
    ...
    # If there were not designs at all in the verilog file, then nl_current_design may be unset,
    # which would cause nl_link to fail.
    if {[use_first nl_current_design] != ""} {
      #nl_link -silent
      nl2_link $nl_current_design
    }
