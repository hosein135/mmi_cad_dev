...
set RCSVERSION(mode.tcl) { $Revision: 1.39 $ }
...
#    }
    ...
    if { $mode_name == "main" } {
	cursor_mode 0
	# 1/31/02: Moved this from cursor_mode to here; and added test for $pop == pop
	# This was added to fix bugs where the cursor was sometimes "lost"
	# if a mode was aborted, so if we are poping to main mode, we always
	# want to restore the main cursor.
	if {$pop == "pop"} { cursor_busy 0 }
...
proc mode_change {{-force} {-enter} mode_name} -desc {
