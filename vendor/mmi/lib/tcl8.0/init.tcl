# init.tcl --
set RCSVERSION(init.tcl) { $Revision: 1.6 $ }
#
...
# Pat added: max will fail with a seemingly unrelated error message if
# the tclIndex does not exist, so lets print a reasonable error message first.
if {[info exists env(TCL_LIBRARY)] &&
	! [file exists "$env(TCL_LIBRARY)/tclIndex"]} {
	puts "warning: Missing file: $env(TCL_LIBRARY)/tclIndex"
}
...
proc unknown args {
	...
	if {$ret != 0} {
	    # pat: if you auto-load something and it fails, we want
	    # to make sure we print a message, rather than just failing.
	    # This can happen if you forget to use the mmi
	    # proc program, and try to autoload a file that has
	    # proc -desc or proc -doc in it.
	    puts "Failure auto-loading source code for: $name"
	...
	# MMI, mha - catch "info commands" since MMI version may not work here
	# MMI, pat added to Tcl8.0
	# Pat, 2/4/02: Disable command completion.  It is extremely confusing to
	# have a command work on the command line, then put the same line
	# in a file and source it, and have it fail.
	set ret [catch "info commands $name" cmds]
    ...
    # pat: if the tclIndex is missing in one of the main directories,
    # then when a command is not found, this just returns an error code.
    # When firing up sue with wish, this can terminate the startup script
    # in the middle, with no error message.  So print an error message.
    # pat: took it back out.  Lee uses catch {garbage command} extensively,
    # and each one prints a message.
    # pat: the above comment may no longer be true - I fixed a bug in wish
    # where it did not print error messages from startup scripts.
    # puts "invalid command name: \"$name\""
    # flush 1
    return -code error "invalid command name \"$name\""
}
...
proc auto_load_index {} {
    global auto_index auto_oldpath auto_path errorInfo errorCode TCLINDEX
	...
	set f ""
        if {[info exists TCLINDEX($dir)]} {
	    # use the cached version -- Added by Lee for SUE
            catch $TCLINDEX($dir)
        } elseif {$issafe} {
...
proc auto_reset {} {
    ...
    # MMI, mha - modified to not use unrestricted "info procs" 
    # MMI, pat added to Tcl8.0
    if {info exists auto_index} {
	foreach p [array names auto_index] {
	    if {[info procs $p] && ($p != "unknown")
