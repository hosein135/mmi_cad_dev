set RCSVERSION(scrlbar.tcl) { $Revision: 1.1 $ }
...
proc tkScrollSelect {w element repeat} {
    ...
    if {$repeat == "again"} {
	set tkPriv(afterId,scrollbar) [after [$w cget -repeatinterval] \
		tkScrollSelect $w $element again]
    } elseif {$repeat == "initial"} {
	set delay [$w cget -repeatdelay]
	if {$delay > 0} {
	    set tkPriv(afterId,scrollbar) [after $delay tkScrollSelect $w $element again]
	}
    }
}
