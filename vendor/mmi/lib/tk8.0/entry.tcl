set RCSVERSION(entry.tcl) { $Revision: 1.2 $ }

# 7/00: MMI Modification to fix tk bug:
# The entry.tcl and scrlbar.tcl code use a common variable named
# tkPriv(afterId).  This causes a bug in prop menus whereby
# the scrollbar on a number entry, once clicked, continues to
# advance forever.  The fix is to change one of the names
# from tkPriv(afterId) to tkPriv(afterId_somethingelse)
...
proc tkEntryAutoScan {w} {
    ...
    # MMI Modification:
    #OLD CODE: set tkPriv(afterId) [after 50 tkEntryAutoScan $w]
    set tkPriv(afterId,entry) [after 50 tkEntryAutoScan $w]
}
